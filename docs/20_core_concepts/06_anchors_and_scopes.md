...
(AnchorIncubator[N,i0,s,+,*,i1,i2,id,i3,i4,i5,i6,i7,i8]) — 14 slots
(AnchorFTA[...])                     — (TBD per current FTA ladder)
```

Every anchor's body is a small MPL file in `files/definitions/Anchor*.txt`. The body unpacks the anchor's claim about its arguments. For `AnchorPeano.txt`:
...
The renaming stage (stage 8, `process_proof_graphs.py`) then rewrites `x<N>` names to `<digit>_copy` where `<digit>` is the anchor-slot index that the `x` was a copy of:

```text
raw: (AnchorPeano[N,x1,x2,x3,x4,x5,x6])
...
Resolving raw bound-variable indices to anchor-slot names happens in stage 8 (the processor), not in the prover. The prover emits raw-index chapters via `visualizer.cpp`'s `buildStack`/`generateRawProofGraph` ([`visualizer.cpp`](../../GL_Quick_VS/GL_Quick/src/visualizer.cpp), [`visualizer.cpp`](../../GL_Quick_VS/GL_Quick/src/visualizer.cpp)), and the processor applies Priority 1 (anchor-slot mapping from the theorem expression) as the first renaming pass.

This means: the `anchor handling` tag row exists as an explicit row in the raw proof graph too — stage 8 just renames its arguments. The row is emitted by the prover when the anchor-handling step occurs during proof.
...
1. **Statement** — `replacedAnchor` is appended to `encodedStatements`, `wholeExpressions`, `intKnownStatements`, `localEncodedStatements*`, and `statementLevelsMap` so the axed form is treated as a derivable fact at this LB.
2. **History line** — `addOrigin(mb->exprOriginMap, replacedAnchor, ("anchor handling", [originalAnchor]))` plus, since [I-?? — see below](#recursion-lb-history-line), `addOrigin(mb->mailOut.exprOriginMap, …)` so the entry propagates to descendant LBs as the standard mail merge runs.
...
Mechanism: `prehandleAnchor` writes the history line to **both** `mb->exprOriginMap` and `mb->mailOut.exprOriginMap` on the non-recursion LB where the axed anchor is generated. The standard mail propagation in `performElementaryLogicalStep` then copies the entry into descendant LBs' `mailIn.exprOriginMap` and merges it into their local `exprOriginMap` via the existing flow at [`prover.cpp`](../../GL_Quick_VS/GL_Quick/src/prover.cpp). The recursion-LB descendant ends up with the origin entry — but no statement, no `axedVariables` insert — exactly the state buildStack needs to resolve cross-scope citations.
