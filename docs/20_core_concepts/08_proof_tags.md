...
30 distinct tags + 31 `TAG_CHECKERS` registry entries (one shared checker). Sorted alphabetically here; the verifier iterates in registry order (see [`10_pipeline/08_verifier.md`](../10_pipeline/08_verifier.md)).
...
- [`mirrored from`](#mirrored-from) — this theorem is a mirror of the cited source.
...
- `applyEquivalenceClass` at `prover.hpp` — the main eq-class rewrite over `encodedStatements`. `rest[0]` is the original encodedStatement before args were mapped via the current class.
...
**Purpose.** Incubator-only. An operator-equality theorem (e.g. `(=[+[a,b], c])`) is back-reformulated into the direct operator form `(in3[a,b,c,+])`.

**Checker.** `check_incubator_back_reformulation` at [`verifier.py`](../../verifier.py).

---

## `mirrored from`

**Purpose.** This theorem is the mirror of the cited source — output-variable premise swapped with head, non-anchor premises permuted.

**Checker.** `check_mirrored_from` at [`verifier.py`](../../verifier.py). Helper: `_check_mirror` at [`verifier.py`](../../verifier.py).

**Example (10_mirrored_statement.txt):**
...
**Validation.** Disintegrate both theorems. Swap output-matched premises. Permute non-anchor premises. Normalise. Compare.
...
**Prover-side level gate (commit, `prover.cpp`).** Before emitting a vacuous-truth on an induction-step LB at `validityName=main` contradiction, at least one of the two contradicting ingredients (`expr` / `negation`) must carry `mb.level` in `statementLevelsMap` — i.e. must be rooted in THIS LB's own inferences (not purely anchor-inherited). If both ingredients are anchor-only derivations, the contradiction is between two anchor-inherited facts and the LB is not actually vacuously true under its recursion assumption; emission is skipped and the LB stays active.
