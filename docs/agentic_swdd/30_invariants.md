...
**Spot.** A wipe that removes a subkey while at least one un-wiped implication still references it. Downstream symptom: `preEvaluateFromEncoded` or `growBaseCandidates` returns `false` for a query that should hit, because the subkey is gone. Manifests as missing hash inferences rather than as a crash — hard to spot without instrumentation.
...
**Scope.** Prover request generation. `ExpressionAnalyzer::ownerSetUSatisfied` (predicate) and its call sites — the same owner-set match sites as the D-105 scope prune: `preEvaluateFromEncoded` (grow-continuation + merge, singles/pairs/CE), `filterIntEncodedStatements{,CE}`, `growBaseCandidates`, the singles/pairs seeds in `generateEncodedRequestsStatic{,Pairs}`, and `makeMandatoryEncodedStatementLists2Static`. The insert-side producer is `recordUSignature`, called at every owner insert in `addToHashMemory` and `makeNormalizedKeysForAdmission`. The firing-side mirror is `checkLocalEncodedMemoryStatic`'s `encodedMap` (`ignoreU=true`) lookup.
...
**Scope.** Prover / CE filter. `BurstSink::canAccept` and the CE request generator `generateEncodedRequestsStaticCE`.
...
**Code.** `prover.hpp::BurstSink::canAccept` (the `self->ceFilteringActive ||` short-circuit); `filter.cpp::generateEncodedRequestsStaticCE`.
...
<a id="i-76"></a>
## I-76 the burst early-exit is honored only at `g_splitCount == 1`; split parts run to completion (determinism)

**Scope.** Prover. `BurstSink::canAccept` (the `stop` check) and `BurstSink::consume` (the `burstDeactivates` → `stop` set) in `prover.hpp`, driven by `performElem2`.

**Rule.** The phase-2 burst early-exit ([D-121](40_decisions.md#d-121)) is gated on `g_splitCount`: honored ONLY when the LB runs UNSPLIT (`g_splitCount <= 1`). At `g_splitCount > 1` neither the SET (`consume` skips the `burstDeactivates` loop — no `stop->store`, no early `return false`) nor the BAIL (`canAccept` skips the `stop->load` check) fires; every split part runs its slice to completion.

**Why.** Under split the N parts are siblings on one shared LB. One part firing a deactivating head and setting the shared `stop`, with the other parts bailing in `canAccept`, is a **thread-timing race**: a sibling that has not yet generated its (useful) firing is cut, so the firing set — and the proved-theorem set — depends on scheduling. That is non-deterministic output, which breaks GL's core determinism claim. It surfaced as a heisenbug — the hashburst dump's own mutex-serialized latency reliably flipped a lost Peano `1·x=x`-mirror theorem (`(>[v1](in[v1,N])(>[v2](in3[i1,v2,v1,*])(=[v1,v2])))`) from absent at 100-split back to present — and was confirmed: gating the early-exit on `g_splitCount` makes two 100-split full runs byte-identical (`proved_theorems` + `global_theorem_list`), recovering the dropped Peano and Gauss theorems with the verifier airtight. At `g_splitCount == 1` there are no siblings (a single part bailing on its own deactivating head is deterministic), so the early-exit stays valid — and load-bearing for the unsplit incubator and the CE filter ([I-73](#i-73), which still halts on this stop because CE runs split==1).

**How to spot.** Two same-config split runs (N>1) differing in `proved_theorems` / `global_theorem_list`; a theorem present at `disable_lb_split` / `fixed_number_splits=1` but absent at higher split counts; `canAccept` or `consume` honoring `stop` without a `g_splitCount` guard.

**How to fix on violation.** Keep both gates: `canAccept` returns true when `g_splitCount > 1` regardless of `stop`; `consume`'s `burstDeactivates` loop is wrapped in `if (g_splitCount <= 1)`. The skipped requests at split>1 are wasted-once-doomed, so unsplit and split produce the same firing set — minus the race.

**Code.** `prover.hpp::BurstSink::canAccept` (the `g_splitCount > 1 ||` guard), `prover.hpp::BurstSink::consume` (the `if (g_splitCount <= 1)` wrap).

**See also.** [D-121](40_decisions.md#d-121), [I-66](#i-66), [I-73](#i-73), [I-83](#i-83).

---

<a id="i-75"></a>
## I-75 the adaptive re-split fires at most once per LB per iteration; only an UNSPLIT LB that hit the cap escalates

**Scope.** Prover. `proveKernel`'s phase-2 adaptive re-split loop and the pure helper `adaptiveSplitDecision`.

**Rule.** `adaptiveSplitDecision` returns `redoNow == true` (escalate to `fixedParts` and re-run this iteration) ONLY when `currentParts <= 1` (the LB was unsplit) AND `maxSubmatches >= submatchCap` (its single part hit the cap). On every other call `redoNow` is false: a split LB (`currentParts > 1`) either coarsens to 1 (fall-back) or holds, never redoes. Therefore `proveKernel`'s `while (!toRun.empty)` re-split loop runs at most twice — pass 1 over all active LBs collects the escalators; pass 2 runs them at `numberOfParts == fixedParts > 1`, where the redo gate is structurally false, so pass 3's `toRun` is empty.

**Why.** Termination plus a bounded cost. The discard-and-redo ([D-111](40_decisions.md#d-111)) must not loop: if a split LB could re-escalate, a monster LB could re-split unboundedly within one iteration. Gating the redo on `currentParts <= 1` makes escalation a one-way 1 → `fixedParts` step per iteration; at `fixedParts` the LB runs to its per-part cap and truncates-and-regenerates like the old fixed-100 path (no further escalation), so the wasted work is bounded to a single capped unsplit pass per escalation.

**How to spot.** `proveKernel`'s re-split loop running > 2 passes; a hang where `toRun` never empties; `adaptiveSplitDecision` returning `redoNow` with `currentParts > 1` (the `adaptive_split_decision_bang_bang` unit test pins the no-redo-at-N>1 cases).

**How to fix on violation.** Keep the `currentParts <= 1` guard on the escalate branch of `adaptiveSplitDecision`; never let the fall-back or hold branches set `redoNow`. The escalation target must be `> 1` so the re-run pass cannot re-trigger.

**Code.** `prover.cpp::ExpressionAnalyzer::adaptiveSplitDecision` (the `currentParts <= 1 && maxSubmatches >= submatchCap` escalate branch), `prover.cpp::proveKernel` (the `toRun` while-loop + the post-finalize `redo[li]` collection).

**See also.** [D-111](40_decisions.md#d-111), [I-74](#i-74).

---

<a id="i-74"></a>
## I-74 a discarded unsplit burst leaves no residue on the LB; the re-run at full split is byte-identical to a fresh split burst

**Scope.** Prover. `proveKernel`'s phase-2 finalize (the escalate branch that skips `performElemPhase2`) and the read-only phase-2 executor `performElem2`.

**Rule.** When a main-path unsplit LB escalates (cap-hit), `proveKernel` sets `numberOfParts = fixedParts`, flags `redo[li]`, and does NOT call `performElemPhase2` for it that pass — so the LB undergoes NO delta-clear, NO `applyFiringRecords`, and NO admission/integration drain from the truncated burst. The truncated burst's `FiringRecord`s (the proveKernel-local `taskRecords`) are simply dropped. Because phase 2 (`performElem2`) is read-only on the LB ([D-116](40_decisions.md#d-116)), the LB's containers are exactly as they were before the burst; the re-run at `fixedParts` in the next pass therefore sees the same LB state a fresh fixed-split burst would, and applies a COMPLETE burst.

**Why.** This is what keeps the re-run deterministic and theorem-faithful. If the discarded burst left any partial deposit (a stray statement, an admission key, a delta entry), the re-run would start from a contaminated state and could diverge run-to-run or drop work. The cleanliness rests entirely on phase 2 being read-only: the finalize is the ONLY place an LB is mutated, and skipping it is a complete no-op on LB state — so the re-run at the full split is byte-identical to a fresh split burst on the un-escalated LB. (Adaptive is NOT expected to match the fixed-100 proof graph: at N=1 the early-exit reshapes a deactivating LB's auxiliary derivations, so the theorem set is preserved but the proof graph differs — [I-76](#i-76).)

**How to spot.** Two adaptive runs diverging (run-to-run non-determinism) traced to an LB that escalated; a stray statement / origin / admission entry left on an escalated LB by the discarded burst; any LB-mutating call added to `performElem2` (which would break the read-only precondition).

**How to fix on violation.** Keep `performElem2` read-only on the LB (deposits go only to the caller's `firingRecords`; `NameMap` reads via `lookup`, never `encode`). Keep the escalate branch skipping `performElemPhase2` entirely — never "partially apply then roll back".

**Code.** `prover.cpp::proveKernel` (the finalize `if (d.redoNow) { redo[li] = 1; continue; }` skip), `prover.cpp::performElem2` (read-only on the LB).

**See also.** [D-111](40_decisions.md#d-111), [D-116](40_decisions.md#d-116), [I-75](#i-75), [I-66](#i-66).
...
**Scope.** Statification deload ([`memory_infra/lb_deload.hpp`](../GL_Quick_VS/GL_Quick/src/memory_infra/lb_deload.hpp) / `.cpp`); the `LbMemory` aggregate and every container that joins it.
...
**Fix.** Add the missing touch point as an explicit `ensureLoaded` call (and document it in the lifecycle list); never weaken the assert.

**Code.** `LbArena::resolve` / `markDeloaded` / `markResident`; `Memory::deloadStaticContainers` / `ensureLoaded` (`memory.cpp`).
...
**Scope.** The active-LB deload/reload policy — the working-set pager ([D-161](40_decisions.md#d-161)). Each of the three phases opens a steward window (`beginPhaseWindow` / `endPhaseWindow`) over its dispatch cursor + the active vector; while open, `maintainWorkingSet` PREFETCHES the lookahead window `[cursor+workers, cursor+workers+kLookahead)` (reload a cold upcoming LB; reshuffle a fragmented resident one) and DRAINS the deloadable LBs outside the working set `[cursor−workers, cursor+kLookahead)` — mass-evict to the working set above the 3/4 hard bound, the one-for-one exchange (one biggest deloadable LB per reload this pass, `pickBiggestDeloadable`) between 1/2 and 3/4, nothing below 1/2. "Deloadable" = active, unsplit, resident, `Idle` (not in flight), outside the kept window. **The claim word is the correctness guarantee, the index window the churn optimization:** the steward CASes `Idle → Busy` before any deload, so a worker that claimed the LB (`WorkerOwned`) makes that CAS fail — an in-flight LB is NEVER deloaded regardless of the cursor's staleness.

**Worker side.** A worker makes the LB it must process resident through the unified handshake `claimAndLoadForWork` (claim → if cold, make room via the load↔evict exchange, then `ensureLoaded`) and **releases the claim (`Idle`) the moment it is done with the LB**, so the LB becomes deloadable again — this is the load-bearing half: a processed LB that stays `WorkerOwned` is never drained, so the resident set grows to ALL active LBs and the pool exhausts (the first-run `IncubatorPeano1` depletion). Phases 1 and 3 release at the body end. Phase 2's executor — where one LB has N concurrent parts — releases via a per-LB remaining-parts counter: the part that decrements it to zero (the last to seal, so no part still reads the LB) flips the claim to `Idle`; the finalize then re-claims + reloads it. All three phases use the same handshake (CE-filter clones run with no steward — always resident — and skip it). Genuine exhaustion (the concurrent working set alone exceeds the pool) asserts at `acquireBlock` naming `static_pool_bytes`, never a silent wait; every `Busy`-claim wait is deadline-bounded ([I-113](#i-113)).

**Determinism.** The eviction set is timing-dependent (the gitignored `.deload/` file set varies run-to-run — accepted, as for the retired [I-115](#i-115)); proof output stays deterministic because deload/reload is content-invisible ([I-103](#i-103), [I-107](#i-107)) and the last timing-dependent decision input — the residency-gated deactivation survey — was removed ([I-108](#i-108)). **Inert without pressure:** a batch that stays under 3/4 force-deloads nothing → reloads nothing → exchanges nothing (light batches run exactly as before). Relaxes [I-106](#i-106): mid-phase `blocksInUse` drives the pager's drain in all phases.

**Code.** `MemorySteward::maintainWorkingSet` / `pickBiggestDeloadable` / `evictOneForReload` / `claimAndLoadForWork` / `beginPhaseWindow` / `endPhaseWindow` ([`memory_infra/steward.hpp`](../GL_Quick_VS/GL_Quick/src/memory_infra/steward.hpp) / `.cpp`); the three phase windows + the worker handshake + the simplified barrier in `ExpressionAnalyzer::proveKernel` / `performElemPhase1` / `performElemPhase3` ([`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp)). Unit tests: `test_steward.cpp` `pick_biggest_deloadable_selects_and_skips`, `claim_and_load_for_work_handshake`.
...
<a id="i-94"></a>
## I-94 `MailLog` writes are single-threaded (register + commit barrier); the parallel phase-1 pull reads frozen logs and writes only its own cursor row
...
**Rule.** Every write to the batch logs (`mailBlobPool` / `mailRefs` / `mailHeads`, `mailEdges` ancestor lists) is single-threaded: registration at grid build (`buildGrid`), and the commit barrier at the `proveKernel` post-join seam — never during the parallel phase. The phase-1 pull runs on the per-LB worker thread but only READS the logs (frozen during the parallel phase) and WRITES the recipient's own `mailCursor` cells (via `setValueAtRelaxed`) and its own `mailIn`. Every cursor cell is pre-created at registration, so the pull only ever advances pre-existing cells — it never inserts or rehashes.

**Why.** This is what makes the parallel pull race-free. **Statified form (current):** (1) the mail pool is never deloaded or compacted, so a vid resolves to a stable physical address for the whole run — a committed blob and every `mailRefs` chain link stay readable at any later pull; (2) every read accessor (`mailEdges` `lookup`/`runLen`/`valueAt`, `mailHeads.find`, the `mailRefs` chain walk + the blob decode — `readBlobInto`'s `Codec<Mail>::deserializeInto` straight into the recipient's own `mailIn` on the production `pull<RoutingColdMail>` path, or `readBlob`'s decode into a stack-local `Mail` on the test `pull<Mail>` path) is pure with respect to SHARED state — strings are inlined in the blob, so a pull never mints into a shared interner, and the only mailbox mutated is the recipient's own per-LB `mailIn` (a worker owns its LB exclusively); (3) the one write per edge advances the recipient's own disjoint cursor slot via `setValueAtRelaxed`, which **skips the shared dirty-flag write** that would be a data race even on disjoint slots (`setAt` would not). A commit during the parallel phase, or a pull that inserted a missing key (rehash), or a cursor advance through `setValueAt` (shared dirty write), would each reintroduce a data race. The earlier statified trials' "cold-blob deserialize race" was actually `PagedHashIndex::reset` overrunning its single directory page (now fixed by the two-level directory + assert, [D-144](40_decisions.md#d-144)), not an inherent concurrency flaw. **Heap prototype (retired):** concurrent reads of an unmodified `unordered_map` are safe; disjoint per-recipient `cursor` writes touch different elements with no structural change.

**Spot.** A `mailLog.commit` call inside a phase-1/2/3 worker body; a cursor advance through `setValueAt` (shared dirty write) instead of `setValueAtRelaxed`; a `mailBlobPool`/`mailRefs`/`mailHeads` append or `mailEdges.appendToTail` outside the single-threaded seams; a pull that mints into a shared interner (a future string-interning variant); the mail containers added to `LbMemory::visitContainers` (would pull them into deload/compaction, breaking the never-moved-vid assumption); a single `TypedColdBlobMap` for the batches (its CSR shifts all later LBs' bytes on every non-tail commit — the reverted quadratic slowdown).
...
**Code.** `mail_log.hpp::MailLog` (`registerLb` / `commit` (+ the `const RoutingColdMail&` overload) / `pull` (the `if constexpr` `RoutingColdMail`/`Mail` dispatch) / `readBlob` / `readBlobInto` / `mergeBatchInto`; the five cold containers `mailBlobPool` / `mailRefs` / `mailHeads` / `mailEdges` / `mailCursor`); the register loop + the pull in `prover.cpp` (`buildGrid` / `performElemPhase1`); the commit barrier at `proveKernel`'s post-join seam (commits `lb->mailOut` directly — [D-143](40_decisions.md#d-143)). Unit test: `test_mail_log.cpp` (`no_ancestors_pulls_nothing`, `cursor_monotonic_no_redelivery`, `dormant_catch_up`, `forced_two_level_spill_no_corruption`).

---

<a id="i-101"></a>
## I-101 The per-LB routing mailboxes `mailIn` / `mailOut` ride the never-deloaded MAIL POOL as self-owned-arena `RoutingColdMail`, freed back to the pool after each read-out

**Scope.** `Memory::mailIn` / `Memory::mailOut` (direct members) and `RoutingColdMail` ([`memory_infra/routing_cold_mail.hpp`](../GL_Quick_VS/GL_Quick/src/memory_infra/routing_cold_mail.hpp); `addRoutingMailOrigin(RoutingColdMail&, ValueInterner&, …)`, `LbArena::bind` the cold lazy mail-pool bind; the id-form codecs `IntMailStatementKey` / `IntMailOrigin` in `mail_types.hpp`). Settled ([D-185](40_decisions.md)); originally hot ([D-179](40_decisions.md#d-179)); migrated string → id.

**Rule.** Each routing mailbox OWNS a per-LB cold `LbArena` drawn from the **never-deloaded mail pool** (`mailMemory`, the reservation the `MailLog` rides), bound LAZILY (`ensureArena` → `LbArena::bind(&mailMemory)` on first write — the root LB predates the pool init), with a write-only `dirty_` sink. The two live columns — `statements` (an `IntMailStatementKey`: the `(originalId, validityId, levels)` triple as a byte key) and `origins` (an `IntMailOrigin` id blob run) — ride that arena; `expandedImplications` is DELETED and `disintegrationSignals` dropped (both assert-guarded). NEVER deload-registered — no `ContainerTag`, absent from `LbMemory::visitContainers`. `clear` RETURNS the arena's blocks to the mail pool (`release` the columns + `arena_.releaseAll`), called the moment the content is read out — `mailOut` at the post-join commit barrier, `mailIn` after the phase-1 absorb — so nothing is retained; the next write re-acquires lazily. `mailOut` stores the SENDER LB's `NameMap` / `originInterner` ids; `mailIn` (and the committed blob) store GLOBAL `mailInterner` ids — the asymmetry is translated at the commit seam ([I-127](#i-127)). Observable order is re-imposed decoded-lex at the read boundary (`decodeMailOutStatements`/`decodeMailOutOrigins` via the sender's interners; `decodeMailInStatements`/`decodeMailInOrigins` via the global `mailInterner`; `routingMailOutToHeap`/`routingMailInToHeap` for the sacred dump + tests); the cross-LB commit serializes GLOBAL ids via `Codec<Mail>::serialize(const RoutingColdMail&, senderNm, senderOi)` (decode sender ids → re-mint global), the pull decodes straight into `mailIn` via `deserializeInto`.

**Why.** The commit barrier reads EVERY body's `mailOut` (incl. steward-evicted LBs, "not just the active set") to decide what each `MailLog` appends; that all-bodies read must never hit a deloaded arena. A deloadable mailbox breaks under deload pressure — `IncubatorPeano1` peaks at ~58% of the pool (past the ½ steward wake mark), so LBs deload, and `mailOut.statementsEmpty` (→ `count` → `PagedVector::size`) asserts residency on an evicted body (the crash the gate caught). So the never-deloaded mail pool is the home (where the `MailLog` it feeds lives), still "0 hot" (the mail pool is a sanctioned cold reservation, not a hot arena). Freeing the blocks after each read-out keeps the footprint transient. Single-threaded by construction.

**Spot.** `mailIn`/`mailOut` bound to a deloadable arena (`&LbMemory::manager`) or enrolled in `visitContainers`; a `clear` that retains blocks instead of `releaseAll`; a write to `mailOut.disintegrationSignals`; an `addOrigin(map&, …)` member on a routing mailbox (use the free `addRoutingMailOrigin(RoutingColdMail&, …)`); a `mailIn` decoded via the sender `NameMap` or a `mailOut` decoded via the global `mailInterner` (the id-space asymmetry); a `mailInterner` mint reached from a parallel worker (commit-seam only).

**Fix.** Keep `mailIn`/`mailOut` self-owned on the mail pool (`LbArena::bind(&mailMemory)`); never deload-register; `clear` returns blocks after the read-out; route mailOut origin writes through `addRoutingMailOrigin`; decode `mailOut` via the sender interners and `mailIn` via the global `mailInterner`.

**Code.** `routing_cold_mail.hpp::RoutingColdMail`; `lb_arena.hpp/.cpp::LbArena::bind`; `memory.hpp` (`Codec<Mail>::serialize(const RoutingColdMail&, NameMap&, ValueInterner&)` / `deserializeInto`, `routingMailOutToHeap` / `routingMailInToHeap`, the `decodeMailOut*` / `decodeMailIn*` helpers, the `mailIn`/`mailOut` members); the commit barrier in `prover.cpp` (`lb->ensureLoaded` + `lb->mailOut.clear` after `mailLog.commit`); `body.mailIn.clear` after the phase-1 absorb; the direct absorb in `standardProcessing`; `MailLog::commit(const Memory*, const RoutingColdMail&)` / `readBlobInto` / `pull` (`mail_log.hpp`). Unit tests: `test_mail_log.cpp` (`routing_cold_mail_*`).
...
**Why.** Like the routing `mailOut` ([I-101](#i-101)), internal mail MUST survive deload: `nextIterationInternalMail` carries one-step-delayed emissions across the deload seam, and `sameIterationInternalMail` carries the end-of-burst `sanitizeHashMemory` rewrites (written AFTER the post-burst clear, [I-21](#i-21)) to the next step — both are non-empty at the deload seam. So the cold deloadable path is their home (the routing `mailOut` rides the same arena for the same reason; not the never-deloaded mail pool, which is the cross-LB `MailLog`'s). The absorb reads the cold columns DIRECTLY — the sorted snapshots (`sortedStatements` / `sortedOrigins`) plus the `getDisintegrationSignal` read door, no transient heap `Mail` — and the sorted order reproduces the former `std::set` / `std::map` iteration, so behaviour is byte-identical ([D-142](40_decisions.md#d-142)). `makeHeapMail` survives only for the sacred dump + tests.

**Cross-LB residency.** The cross-LB deposit sites (`updateGlobalDirect` / `updateGlobal`) write a DIFFERENT LB's `nextIterationInternalMail`, which on the cold path may be deloaded. They `ensureLoaded(memoryBlockR)` before the cold-door write and SKIP a `dischargedForever` target (it never drains its `nextIter` again — dead deposit; and `ensureLoaded` asserts on a discharged LB). Single-threaded post-join ([I-28](#i-28)), so the reload is safe — the only spot in the change that is not a pure heap mirror, forced by cold storage.

**Spot.** A `ColdMail` channel switched to `RoutingColdMail` (`nextIter` would lose cross-burst + cross-deload content) or to a never-deloaded pool (its content must travel with the LB); a `.statements` / `.exprOriginMap` / `.disintegrationSignals` member access on a channel (cold now — use the doors / `makeHeapMail`); a cross-LB deposit without the `ensureLoaded` + `dischargedForever` guard (residency assert); a future internal-mail writer depositing from a `std::unordered_*` / pointer-ordered source (breaks the canonical deload bytes); `makeHeapMail` leaving `disintegrationSignals` empty (would drop the firing-time OR-disintegration signals — `or0` facts un-disintegrated).

**Fix.** Keep the channels as `ColdMail` aliases into `LbMemory`; write via the doors (`insertStatement` / `addInternalMailOrigin` / `setDisintegrationSignal`, all id-form on the LB's own `NameMap` / `originInterner`); read via the id-form decode helpers + `getDisintegrationSignal` (the absorb) or `makeHeapMail` (dump / tests); guard every cross-LB deposit with `!dischargedForever` + `ensureLoaded`; deposit only from sorted / positional sources.
...
**Rule.** Cross-LB routing mail travels as GLOBAL `mailInterner` ids so an id means the same at the sender and the receiver (a per-LB `NameMap` id could not — [I-101](#i-101)). The interner is FROZEN — decode / lookup only — throughout the parallel/multithreaded phase (phase-1 pull + absorb, phase-2 hashburst, phase-3). The ONLY sites that MINT (`intern`) are SINGLE-THREADED seams: (1) the `proveKernel` commit barrier AFTER `pool.join` — the mailOut→global translate, which first `ensureLoaded`s the sender so its `NameMap` / `originInterner` are resident; (2) the load-time `broadcastTheorems` self-inject + commit; (3) the post-join `updateGlobal*` / D-76 drains. `mailOut` holds the sender's per-LB ids, `mailIn` + the committed blob hold global ids, and the asymmetry is translated ONLY at that commit seam. EVERY mail-inbox writer `ensureArena`s before its first insert ([G-55](50_gotchas.md#g-55)).
...
**Code.** `arena_stack.hpp` (`ArenaStack`); `scratch_arena.hpp` / `.cpp` (`genScratchArenas` / `initGenScratchArenas`); `prover.cpp` (init beside `initScratchArenas`, `releaseAll` at `performElem2` exit); `memory.cpp` / `filter.cpp` (the generators); `memory.hpp` (`StaticRequestEmitter`). Unit test: `test_arena_stack.cpp`. See also [I-124](#i-124), [I-107](#i-107), [I-95](#i-95).
