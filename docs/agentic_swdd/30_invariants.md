...
<a id="i-214"></a>
## I-214 CUDA growth covers every candidate exactly once through fixed canonical windows and stable survivor compaction


**Scope.** Production CUDA normalized-key growth in [`gpu/phase2_cuda.cu`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_cuda.cu) `CudaPhase2GrowthBuffer::runRequestGrowth`, the `phase2GrowthShortCheapGateKernel` and `phase2GrowthCooperativeCheapGateKernel` cheap-gate producers, and `phase2GrowthProbeSurvivorsKernel`; fixed public capacity and record contracts in [`gpu/phase2_projection.hpp`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_projection.hpp) `Phase2GrowthCapacity` and `DeviceGrowthCandidateAttempt`.

**Rule.** At each frontier depth, stable radix passes order live nodes by original call, run, statement path, and candidate position. Their exact attempt spans are scanned into one canonical ordinal space and covered by consecutive windows of at most 16,777,216 records. Short and cooperative producers write every in-window ordinal exactly once, cheap-gate survivors are stably compacted by ordinal, and only those survivors perform normalized-key and owner probes. No capacity boundary drops, truncates, reorders, dynamically reallocates, falls back, or replays work on the processor. Atomic event and child arrival remains non-semantic; retained call, run, path, and candidate tokens reconstruct every observable order downstream.

**Why.** A complete depth can contain hundreds of millions of attempts, while the measured cheap gates reject more than half before map work. Fixed canonical windows bound materialized work without turning the window boundary or GPU scheduling order into proof state.

**Spot.** A depth allocates its full attempt count; a window begins before the preceding window ends; a candidate ordinal is skipped or written twice; survivor compaction uses arrival order; a window cap changes the result; an overflow truncates or invokes another backend; or physical event/frontier append order becomes an ordering key.

**Fix.** Restore the stable node sort, exact scanned spans, consecutive half-open windows, one flag write per ordinal, stable flagged selection, fixed asserted buffers, and token-based downstream ordering. Verify with a direct fixture whose four-record capacity cuts both a ten-attempt and nine-attempt depth, then run the complete verifier and 311-file identity gate.

**Code.** [`gpu/phase2_projection.hpp`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_projection.hpp) `Phase2GrowthCapacity` / `DeviceGrowthCandidateAttempt`; [`gpu/phase2_cuda.hpp`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_cuda.hpp) `CudaPhase2GrowthBuffer`; [`gpu/phase2_cuda.cu`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_cuda.cu) `CudaPhase2GrowthBuffer::runRequestGrowth` / `phase2GrowthShortCheapGateKernel` / `phase2GrowthCooperativeCheapGateKernel` / `phase2GrowthProbeSurvivorsKernel`. See [D-319](40_decisions.md#d-319), [I-213](#i-213), [I-211](#i-211).

<a id="i-213"></a>
## I-213 Each live CUDA growth node owns one immutable bounded pooled prefix; short and long node selection preserves ascending frontier index


**Scope.** CUDA normalized-key growth preparation, short-node expansion, cooperative expansion, and fixed ownership in [`gpu/phase2_cuda.cu`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_cuda.cu) `phase2GrowthPreparePrefixesKernel`, `phase2GrowthShortExpandKernel`, `phase2GrowthCooperativeKernel`, and `CudaPhase2GrowthBuffer`; public capacity and record contracts in [`gpu/phase2_projection.hpp`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_projection.hpp) `Phase2GrowthCapacity` and `DeviceGrowthPrefix`.

**Rule.** Every live frontier node receives exactly one 32-byte prefix header before candidate expansion. Its payload, distinct normalization-variable, and distinct secondary-variable slices lie wholly inside the three retained fixed pools; every offset and length is asserted, and no overflow can resize, truncate, fall back, or replay on the processor. The header's hypothesis, non-exempt scope, and mandatory-term summaries describe exactly that node's statement path. Both short and cooperative node lists are stable selections of the original frontier index sequence, so their physical traversal does not depend on block arrival order. Prefix pool allocation order may vary, but no semantic verdict, event identity, growth position, or canonical ordering key reads it. Candidate kernels extend the immutable pooled prefix with only their appended-expression suffix.

**Why.** Normalization and path summaries are node-level state reused by every later candidate. Storing them once removes duplicate work, while fixed pools retain process-owned memory. Stable index selection protects the existing atomic event/frontier seam from a newly introduced physical scheduling permutation.

**Spot.** A candidate kernel rebuilds the complete path prefix; a pool counter can exceed its capacity; a slice points outside its pool; allocation offsets enter ordering; a short or long list is appended by atomic block arrival; one node appears in both or neither list; or an overflow triggers allocation, truncation, or processor work.

**Fix.** Restore one preparation pass, exact asserted slices, complementary fixed flags, stable selection over ascending frontier indices, and suffix-only candidate expansion. Verify the direct short/cooperative twin, exact fixed byte total, full verifier, and complete artifact manifest.

**Code.** [`gpu/phase2_projection.hpp`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_projection.hpp) `Phase2GrowthCapacity` / `DeviceGrowthPrefix`; [`gpu/phase2_cuda.hpp`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_cuda.hpp) `CudaPhase2GrowthBuffer`; [`gpu/phase2_cuda.cu`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_cuda.cu) `phase2GrowthPreparePrefixesKernel` / `phase2GrowthShortExpandKernel` / `phase2GrowthCooperativeKernel` / `CudaPhase2GrowthBuffer::runRequestGrowth`. See [D-318](40_decisions.md#d-318), [I-211](#i-211).

<a id="i-212"></a>
## I-212 Exact CUDA filter classes share only an immutable compact span; original processor call ordinals remain the semantic ordering identity


**Scope.** Phase 2 host filter scheduling in [`gpu/phase2_projection.hpp`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_projection.hpp) `Phase2FilterScheduleArena`; device filtering and class-span expansion in [`gpu/phase2_cuda.cu`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_cuda.cu) `CudaPhase2FilterSortBuffer`; and growth event reconstruction in the same CUDA translation unit.

**Rule.** Two calls share a filter class if and only if all five fields are equal: logical-block projection index, memory kind, inclusive iteration ceiling, whole-key widening flag, and statement count. Equality is checked field by field after hashing; hash equality alone is never sufficient. Classes preserve first original occurrence order. Exactly one stable first-8,192 compact span is produced per class, sorted by decoded-name rank and then original statement index. Every original call maps to that class count and offset, but its own processor ordinal remains in growth events, task-local growth positions, duplicate-removal identity, and canonical stream ordering. All class, map, count, and offset storage is fixed, retained across clears, and asserts on overflow; no fallback, truncation, resize, or host semantic verdict exists.

**Why.** Exact duplicate calls examine the same frozen statement universe and registries, so their filter result is immutable and reusable. Their generator-call ordinals are nevertheless distinct positions in the processor search and may not be collapsed.

**Spot.** A class comparison omits one signature field; a collision merges unequal descriptors; duplicate calls receive copied rows instead of a shared offset; an event carries the class ordinal as its call ordinal; equal decoded names reorder by device arrival; or a capacity failure allocates, truncates, or invokes the processor.

**Fix.** Restore exact five-field interning, first-occurrence class order, one stable class span, the complete original-to-class expansion, and original call tokens below the shared span. Assert every fixed ceiling and use direct duplicate-class twins plus full artifact identity to verify the seam.

**Code.** [`gpu/phase2_projection.hpp`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_projection.hpp) `Phase2FilterScheduleArena`; [`gpu/phase2_projection.cpp`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_projection.cpp) `appendCall`; [`gpu/phase2_cuda.cu`](../../GL_Quick_VS/GL_Quick/src/gpu/phase2_cuda.cu) `CudaPhase2FilterSortBuffer::filterAndSort` / `phase2FilterExpandClassSpansKernel` / request-growth kernels. See [D-317](40_decisions.md#d-317), [I-211](#i-211).
...
<a id="i-202"></a>
## I-202 A grow position rewinds every field the appended premise touched, before it touches it

**Scope.** `memory.cpp::ExpressionAnalyzer::generateEncodedRequestsStatic` — the per-pop preparation of `keyState` / `gateState` and the per-position rewind — together with `ExpressionAnalyzer::NormKeyBuildState` and `ExpressionAnalyzer::RequestGateState` in `prover.hpp`.

**Rule.** The grow search prepares the fixed premise prefix once per stack pop and then folds ONE appended premise per position into the same two carried records. Before each fold it MUST restore every mutable field of both records to the value the prefix left: `nVars`, `nextNormId` and `pos` on the key state; `hypoFound`, `hypoValidityId`, `nonExemptScopes`, `nonExemptValidityId` and `secondaryCount` on the gate state. A field added to either record is a field that must be added to the rewind in the same commit. The prefix bytes in the shared key buffer must likewise stay untouched for the whole pop — every consumer between positions reads the key (`wholeKeyPresent` / `subkeyUSatisfied` copy it into their own probe array, `arenaKey` copies it onto the arena) and none writes it.

**Why.** The rewind is the whole correctness argument. The extended key must be BYTE-identical to a full rebuild, because it feeds both owner-set probes and, through `IntNormalizedKey`, the emitted request, and `addToHashMemory` installs a rule's whole weakly-name-sorted permutation family against that byte language. A stale `nVars` gives the next position a variable slot that a full rebuild would never have minted; a stale `secondaryCount` pushes an unrelated candidate over the distinct-secondary cap. Neither crashes. Both change which rules fire, silently, days later and somewhere else.

**Spot.** A new field on `NormKeyBuildState` or `RequestGateState` with no matching `prefix*` local and no matching restore line. Any write to `buf[0.. prefixKeyLen)` inside the position loop. A caller that folds two premises into one prefix state without rewinding between them. The symptom is a proof-artifact diff against the same input, never an assert.

**Fix.** Add the field to the rewind. Never "reset the whole record instead" — that is the rebuild this exists to avoid, and it would silently retire the optimisation while looking correct.

**Code.** `memory.cpp::generateEncodedRequestsStatic` (the `prefixVars` / `prefixNextNormId` / `prefixKeyLen` and `prefixHypoFound` / `prefixHypoValidityId` / `prefixNonExemptScopes` / `prefixNonExemptValidityId` / `prefixSecondaryCount` locals and their restore lines); `prover.hpp::NormKeyBuildState` / `RequestGateState`. Tests: `test_request_key_build.cpp::rewind_reuses_one_prefix_across_positions`, `test_request_gates.cpp::rewind_reuses_one_prefix_across_positions` — both fold many different last premises through one prefix, which is the only shape a forgotten field shows in.

**See also.** [D-296](40_decisions.md#d-296), [I-155](#i-155) (the one request generator), [I-77](#i-77) (deposits applied in canonical order, so request-generation order is not observable — but the KEYS are).

---

<a id="i-201"></a>
## I-201 A `buildStack` frame that fails into a candidate loop has every trace of itself rolled back

**Scope.** `visualizer.cpp::ExpressionAnalyzer::buildStack` — the candidate loop's rollback (`stack.resize(stackSnap)` plus the `g_coveredJournal` truncation) and the `rowsSurviveOnFailure` parameter that depends on it.

**Rule.** When the candidate-loop recursion returns false, the parent MUST restore `stack` and `covered` to exactly their state at the candidate's mark — all of it, not a prefix and not the rows only. That rollback is total, so a frame reached through that call site can never contribute anything to the chapter, and `buildStack` is licensed to skip its last-resort fallback there (`rowsSurviveOnFailure == false`). Exactly one call site may pass `false`; the two contradiction tail calls and the fallback's own dependency walk inherit the caller's value, and every other caller takes the `true` default.

**Why.** The skip is what took the FTA shortcut's chapter export from 4.6 M recursive calls to a small fraction of that. Its correctness rests entirely on the rollback being total: if a future edit made the parent keep any part of a failed candidate's output — a partial `stack` truncation, a `covered` entry retained as a memo, an origin-side side effect — then the skipped fallback's rows would have been part of the chapter, and skipping them silently changes chapter content. The failure mode is not a crash; it is a chapter quietly missing its degraded rows, which the verifier flags only where they happened to be load-bearing.

**Spot.** Any new state mutated inside the candidate loop that the rollback does not undo. In particular: a `covered` erase (the journal assumes insert-only), a memo keyed on a node that a failed candidate visited, or an origin-map write from the walker (forbidden anyway — the export is a reader, Rule 16). Also any second call site passing `rowsSurviveOnFailure = false`.

**Fix.** Either extend the rollback to cover the new state, or stop passing `false` at that call site. Never weaken the parameter's meaning to "probably discarded".

**Code.** `visualizer.cpp::ExpressionAnalyzer::buildStack` (candidate loop, rollback, the four recursive call sites, the skip block); `g_coveredJournal` in the same file. Test: `test_theorem_usage_graph.cpp::fallback_skipped_when_rows_cannot_survive`.

**See also.** [D-297](40_decisions.md#d-297), [D-51](40_decisions.md#d-51) (the fallback's origin), [I-67](#i-67) (`g_buildStackPath` insertion ownership — the same per-frame-ownership discipline on the path set).

---

<a id="i-204"></a>
## I-204 The read-only reload door rebuilds no derived index, and says so

**Scope.** `memory.cpp::Memory::ensureLoadedForRead` / `Memory::reloadFromImage`'s `rebuildDerivedIndexes` parameter, and `ReverseArgsIndex::markUnbuilt` / `reverseIndexRunOf`.

**Rule.** A reload through the READ-ONLY door (`ensureLoadedForRead`) restores container content but does NOT re-derive the four `ReverseArgsIndex` membership indexes the canonical stream omits ([I-117](#i-117) / [I-154](#i-154)). Each is marked unbuilt instead, and `reverseIndexRunOf` asserts on that mark. Every WORK-path reload (`ensureLoaded`, the steward's `claimAndLoadForWork`) keeps the rebuild — the parameter defaults to `true`, so a new caller opts out only deliberately. The RAW branch is unaffected: its whole-arena image restores the indexes verbatim, so there is nothing to rebuild or to skip.

**Why.** The reverse index exists solely for the request-generation candidate probe. Nothing downstream of the read-only door generates a request: its production callers are the chapter export's walk (`buildStack` / `contradictionLbHoldsRecord`, which read `exprOriginMap` + `originInterner` and nothing else) and `repairTierCitationOrigins` (`decodeOriginMapSorted` on the same two). The rebuild is proportional to hash-memory size, so it was pure cost on a path that never reads the result.

**Why the MARK, not just the skip.** An index left unbuilt is EMPTY, and empty is indistinguishable from "no NormKey was ever recorded" — a defined miss. A future reader on this path would therefore get a silent wrong answer (an empty candidate set) rather than a failure. The mark converts that into an assert at the probe (Rule 19).

**Spot.** A new `ensureLoadedForRead` caller that reaches request generation, or any consumer of `reverseIndexRunOf` on an LB revived through that door. The assert fires at the probe naming the promise that was broken.

**Fix.** Either give that caller the work-path door, or pass `rebuildDerivedIndexes = true`. Never relax the assert.

**Code.** `memory.cpp::Memory::ensureLoadedForRead` / `Memory::reloadFromImage`; `memory_infra/reverse_args_index.hpp` (`markUnbuilt`, the `unbuilt_` flag, the `reverseIndexRunOf` assert). Test: `test_lb_deload.cpp::read_only_reload_skips_the_reverse_index_rebuild`.

**See also.** [I-117](#i-117), [I-154](#i-154), [I-111](#i-111) (residency — a partial-BAND load would need it changed; this skip does not, the LB stays fully resident), [D-158](40_decisions.md#d-158).
...
The same `RT_MEASUREMENT` gate also controls the FTA-C-RT phase-and-attribution traps in `proveKernel` (`[RT phase measurement]`, `.rt/burst_phases.log`, the C15 `g_crt*` tallies). They are instrumentation on the same switch, not a second tracker home.
...
**The one documented exception — the cross-burst aggregate.** `resetRtAggregate` / `dumpRtAggregate` (`rt_tracker.cpp`, the `AggState` singleton) fold every tracker's sections at destruction into one process-wide table keyed by the full open-scope path (root → leaf labels joined with " > "), dumped once at end of batch to `.rt/_aggregate_<tag>.log`. It exists because the per-call files cannot answer "what share of the whole run does section X own" (trigger-gated, overwritten per burst). The exception is bounded: additive-only, mutex-guarded on the cold destructor path, dump-only, read by nothing in the prover (Rule 16 discipline), split-safe (parts sum). The per-call tracker contract above is unchanged — trackers still share no state with each other; they only deposit into the sink at death. Sections are additionally keyed by parent (`Section::parentIdx`), so one label under two enclosing scopes attributes separately (`PARENT > LABEL` rows).
...
**Rule.** `fillMailOut` is the sole writer to `memoryBlock.mailOut.statements`. `fillMailOut` is the sole writer to `memoryBlock.mailOut.exprOriginMap` **with documented exceptions** — history rows whose statement does not ride the per-step delta when `fillMailOut` walks it: (1) the multiplied-implication "multiplied from" row inside `memory.cpp::addToHashMemory`, restored as a direct `addOrigin(mb.mailOut.exprOriginMap,...)` because the multiplied implication installs as a *rule* in `overallHashMemory` and never reaches `intLocalEncodedStatementsDelta`; (2) the direct `Memory::addMailOutOrigin` paired-origin writes at emission sites (the integration-expansion / premise-element / or-branch-assumption rows of the integration prep, the two `equality1` rewrite sites), each pairing a history row with a deposit the delta loop does not carry at that point; (3) the [D-286](40_decisions.md#d-286) drop-site ship — `filterIterations` calls `copyOriginRowsToMailOut` for each canonicalization-dropped delta row before pruning it; (4) the `ordisMerge` convergence ship — step 2 calls `copyOriginRowsToMailOut` for each branch derivation before `removeExpressionFromMemoryBlock(state=0)` erases that branch's delta row ([D-311](40_decisions.md#d-311)). The rule for any future delta-dropping site: ship first, drop second. Every other writer to either field has been deleted: Added on: the canonical door's raw-line ship — `addExprToMemoryBlock` calls `copyOriginRowsToMailOut` for the RAW spelling of a rewritten local (status 0 / 1) deposit at the door, because the raw form never becomes a delta row while the registered canonical form's `equality1` bridge cites it ([I-217](#i-217)).
...
**Incubator topology ([D-332](40_decisions.md#d-332)).** The root-only writer set is unchanged; what changes in incubator mode is the reader set of the root's log — only the anchor LB pulls it (`collectMailAncestors`). `broadcastTheorems` is the one writer that may target another log: in incubator mode the externals batch is committed into the anchor LB's log and self-injected into the anchor LB's `mailIn` (the `store` parameter; root outside incubator mode).
...
<a id="i-65"></a>
## I-65 `applyEquiClasses` covers both (delta-class × all intEncodedStatements) AND (non-delta-class × new intEncodedStatements) through one statement-major batch per round, with equality / negated-equality statements filtered out

**Scope.** Prover. `prover.hpp::applyEquiClasses`, `buildEquiClassTable`, `collectEquiAffectedRows`, `stageEquiClosure`, `drainStagedEquiProducts`.

**Rule.** Inside the outer `while (intEncodedStatements.size grows)` fixpoint, each round runs the hooks class-major — Pass 1 over every live delta entry of `changedClassesThisStep` (keep-last dedup, dead-scope skip), Pass 2 over every non-delta class of `equivalenceClassesMap` — and then ONE statement batch. The class table (built once per apply) gives every class a due-from position: a delta entry's `startIdx` (0 on entry → covers ALL existing statements) or a non-delta class's persistent waterline `eqClassSttmntIndexMapMap[v][cls.variables]`. A row is visited when a comparable class is due for one of its argument ids; the visited row's closure under every comparable class is staged and drained. After the batch every due-from position is the registry size; at the end of the apply every table class's waterline is set to the final registry size. Both coverage clauses hold: a delta class reaches every statement it touches, a non-delta class every new statement it touches, and both reach the products.

Positive equalities (`nameId` of `=` without negation) never enter the batch — they are class inputs (`updateEquivalenceClasses`); negated equalities go to the D-226 expander once per due row. The class state is fixed inside an apply (no path in it admits a positive equality), which is what licenses building the table once.
...
**Fix.** Keep the `inputIndices.size == 1` check. When adding a new operator, either ensure it has the single-input shape (and rely on this gate), or ensure its admission map is populated from consumer-side registrations via `updateAdmissionMap`. Do not add a new operator to the fallback rule without careful RT measurement. A witness at an INPUT slot of a multi-input operator gets its admission path from the demand side instead: the input-slot demand route ([D-288](40_decisions.md#d-288), [I-196](#i-196)) — the unconditional widening of THIS gate to two-input operators was tried and reproduced the RT explosion.
...
**Rule.** When an integration-side rejection is revived (via equi-class rewrite or new-admission-key trigger), the `admissionMapIntegration` / `admissionSetIntegration` entry that enabled the revival is **not** erased afterwards. Since [D-299](40_decisions.md#d-299) this is no longer an asymmetry: the algebra counterpart `revisitRejected2` has no cleanup tail either, and no path on either side erases an admission key on use. The historical asymmetry — algebra's `cleanAdmissionMap` plus its D-62 canonicalization closure — is described below because the prohibition it motivated still stands.
...
- Any consumption-driven erase of an integration admission entry, in any form. The vestigial `admissionMapIntegration.erase(...)` calls that once sat inside `cleanAdmissionMap` went with that function; a new one that actually matched a u_-form key would violate this invariant.
...
**Code.** `prover.cpp::revisitRejectedIntegration2`; `prover.hpp::applyEquivalenceClassToAdmissionMapIntegration` (the post-insert revisit-fire path).
...
`mailOut.exprOriginMap` is the exception: it carries entries for ALL scopes per `trackExpansionHistory`'s convention. Receivers' merged `body.exprOriginMap` therefore covers the sender's full provenance graph — main, hypothetical, ordis-branch, and orint-branch alike — even though the rules themselves stay local to the sender at non-main scopes. This includes rows whose STATEMENT never mails at all: a delta row dropped by `cleanUpExpressions`' `filterIterations` canonicalization sweep (which runs before `fillMailOut` in the same phase-3 call) ships its origin history via `copyOriginRowsToMailOut` at the drop site ([D-286](40_decisions.md#d-286)) — otherwise a statement registered and dropped within one burst would be cited by mailed `equality1` rewrite rows while its own history could never follow, stranding `buildStack` at any receiver that cannot re-derive it (the C6 Step-10 export crash).
...
- **Site F** ([`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp), `addExprToMemoryBlock`) — duplicate suppression. Returns early if any ancestor scope already has the expression. The scan body is the shared predicate `gl::ancestorKnown` ([`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp)) — the SINGLE definition of the ancestor-known contract, also consumed (strict-ancestors mode) by the registration-door gates at `addStatement`, `applyEquivalenceClass`'s commit block, and the negated-equality expander's emit gate, and (self-inclusive mode) by the firing-record mirror in [`memory.cpp`](../GL_Quick_VS/GL_Quick/src/memory.cpp) — see [I-187](#i-187).
- **Site H** ([`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp), `addExprToMemoryBlock`) — blacklist scan over `memoryBlock.nameMap.ancestorsOf[valId]` against `intValidityNamesToFilter`. Filters out depositary scopes that have been blacklisted.

**Refusal side-signal.** A Site F (or door-gate) refusal still calls `stageOrReleaseForRefusedDeposit` ([`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp)) — the branch DERIVED the expression even though registering it is redundant, so the I-174 sequenced-release goal probe runs on the refused pair (non-minting; mirrors `ordisMerge`'s probe). Without it, a live `_ordis_` branch whose goal-matching derivation is ancestor-known would never resolve its cohort.
...
If any of the three holds, **skip** all three `addOrigin` calls (`mergedOriginMap`, `memoryBlock.exprOriginMap`, `memoryBlock.mailOut.exprOriginMap`) for that target. For a new target, cite each of the two bridge equalities at the scope where its body origin row lives: probe `validityName` first, then strict ancestors deepest-first, and assert on a complete miss. Cross-scope merge may combine a branch-local bridge half with an inherited ancestor half; their dependency scopes are therefore resolved independently.
...
> **Scope note (2026-08-30).** The gate covers the `equality1` history EMISSION only. The statement COMMIT of a product is gated on the statement registry alone (`intStatementLevelsMap` probe) and never on `exprOriginMap` — Rule 16 / [I-44](#i-44); see [D-321](40_decisions.md#d-321) and [G-70](50_gotchas.md#g-70).
...
**Rule.** Before pushing an `equality1` origin record for target `applied @ depositValidity`, check whether `memoryBlock.exprOriginMap[appliedWithValidity]` already has at least one entry. If it does, **skip** both `addOrigin` calls (`memoryBlock.exprOriginMap` and `memoryBlock.mailOut.exprOriginMap`) for that target. For a first emission, cite every justifying equality at the scope where its origin row lives: probe the class scope first, then strict ancestors deepest-first, and assert if none contains the row. A descendant class may contain equality content absorbed from an ancestor; citing that equality at the descendant scope would create an unresolvable dependency across an OR boundary.
...
<a id="i-37"></a>
## I-37 `rejectedMap` and `rejectedMapIntegration` are written by equi-class application only through the kernel's own writers, with the product's history closed at the park scope
...
**Rule.** `HashMemory::rejectedMap` and `HashMemory::rejectedMapIntegration` are mutated only by:

1. The disintegration path inserting fresh rejection records via `prover.hpp::updateRejectedMap` (algebra) or `prover.hpp::updateRejectedMapIntegration` (integration).
...
3. The two equi-class hooks `prover.hpp::applyEquivalenceClassToRejectedMap` and `prover.hpp::applyEquivalenceClassToRejectedMapIntegration` ([D-308](40_decisions.md#d-308)): they erase the hit key K and, for a park whose rewritten product is not demanded, re-insert the rewritten record under the canonical K' **through the same writers as channel 1** — never through a private insert — writing one complete `equality1` history row per rewritten product / sibling at the park scope, unless the rewritten compound is already local at that scope or known at a strict ancestor (or unchanged): such a park is dropped, the Site-F refusal the absorb applied to the re-sent compound (its products belong to the known compound's own disintegration); for a park whose rewritten product is demanded right now they erase K and mail the rewritten compound onto `sameIterationInternalMail` (the former D-63 / D-64 path), so the absorb's Pass B re-parks or admits it through channel 1.

A rewritten park record without its `equality1` history row — a re-inserted product whose only foundation is the pre-substitution product's `disintegration` row — is a violation: the row is what closes the chapter chain (`rewritten product ← equality1 ← pre-rewrite product ← disintegration ← compound`).

**Why.** Both maps hold real disintegration products whose `disintegration` origins were written at the production site — `prover.cpp::disintegrateExprCore2::trackExpansionHistory` (algebra) and the integration-side expansion path. Those rows are bound to the pre-substitution texts. A rewritten record needs a row of its own that the verifier can check: the hook writes `equality1` with source = the pre-substitution text at the same scope and justifiers = the `(=[member,canonical])` pairs that fired on that text — every link a tag the verifier already validates (`check_equality1` is syntactic: same core and arity, each differing argument covered by a cited equality at the row's scope or a strict ancestor; the source needs its own row, which the `disintegration` row is). The revival door's `equality1` self-source row (one dependency) is added on top at revival; the chapter walker skips it (a dependency on the node itself is a cycle) and takes the complete row.

**Why the demanded case keeps the mail path.** The absorb's Pass B is the only place that admits with the depth / secondary-variable caps, cascades co-witnesses of the same compound, runs `checkForEquivalence`, and stages the flag-5 relay. A rewritten product that an admission entry (a regular, non-ordis entry under K' at the park scope or a strict ancestor — `admissionRunHasRegularValue`, the D-243 rendezvous probe) or the single-input-operator route (`isAllowedAsOperatorInput`) would admit right now is therefore routed through the absorb as before; the probe is a side-effect-free superset of Pass B's test, so a refused product is re-parked under K' by channel 1. One demanded product sends its whole compound down that route.

**History.** Before (2026-05-13) the integration side performed a direct substituted insertion (D-43 / I-30) with no post-substitution provenance — the leak this rule was written against. D-63 / D-64 then routed every rewrite through mail and re-disintegration; (2026-08-26) measured that round trip at 44 s of a 45 s IncubatorGauss3 burst (a full statement-table scan per re-sent compound plus the re-disintegration of every park) and replaced it with the in-place re-key above.

**Spot.**

- A write to `HashMemory::rejectedMap` or `HashMemory::rejectedMapIntegration` that bypasses `updateRejectedMap` / `updateRejectedMapIntegration` (a raw `insertRejectedIdsBlob` / `assignRun` inside a hook).
- A hook re-insert with no `equality1` row for a changed product or sibling, or an `equality1` row with fewer than two dependencies at a re-key site.
- A chapter row tagged `equality1` or `disintegration` whose source expression cannot be located in any earlier chapter row at any ancestor scope.

**Fix.** Route the re-key through the kernel writers and write the complete `equality1` row per changed text; route a demanded product through the mail path. Never a private insert, never a self-source-only history.

**Code.** `prover.hpp::applyEquivalenceClassToAdmissionMap`, `prover.hpp::applyEquivalenceClassToRejectedMap`, `prover.hpp::applyEquivalenceClassToAdmissionMapIntegration`, `prover.hpp::applyEquivalenceClassToRejectedMapIntegration`, `prover.cpp::revisitRejected2`, `prover.cpp::revisitRejectedIntegration2`, `prover.cpp::disintegrateExprCore2` (the production-site origin emitter). See also D-57, D-63, D-64, D-106, D-308, I-22, I-36.
...
> **Retired on this branch by [D-299](40_decisions.md#d-299) (2026-08-22).** `cleanAdmissionMap` is deleted — no admission key is consumed, so there is no closure to run. Superseded by [I-200](#i-200). The rest of this entry describes the retired closure.
...
<a id="i-46"></a>
## I-46 RETIRED — hashMem rule registry canonicalised by end-of-burst `sanitizeHashMemory` via the `expandedImplications` index

**Status.** Retired ([D-312](40_decisions.md#d-312)): `sanitizeHashMemory` and `eradicateImplicationFromLB` are deleted. The rule registry stays canonical through the install-loop gate and the `applyEquiClasses` compact hook — [I-210](#i-210). `expandedImplications` survives as the hashburst dump's section source (Rule 14). The text below describes the retired mechanism.
...
- A theorem missing from `theorems.txt` whose head IS visible in intEncodedStatements at v=main (since the closure/registration decoupling the toBeProved row closes anyway, so a frozen toBeProved is no longer the symptom). Cross-check by reading the head's `intStatementLevelsMap` entry — if its size exceeds `memoryBlock.level + 1`, the verdict is silently refusing the registration.
...
**Scope.** Prover — BOTH `compilation`-origin emission sites: `prover.cpp::proveKernel`'s deferred-compaction drain (the per-tuple body inside the loop over `pendingCompactionQueue`) and `prover.cpp` `ExpressionAnalyzer::broadcastTheorems` (the LOAD-time external/theorem broadcast). Applies to every `compilation` origin emitted from either site.
...
**Why.** `compileImplicationToCompact` dedups alpha-equivalent inputs to the same `implication<N>` name; the binary stores only the first-seen body. A later input whose binder ordering differs from the binary registered form will be alpha-equivalent but not structurally identical. Moreover the registered `elements` carry a NORMALIZED premise order (`makeNormalizedEncodedKey` inside `excludeRepetitions`), so even a single-source input can differ from its own binary body — visible when several premises share one operator name (e.g. a corpus external with three `in3[·,·,·,*]` premises). The verifier's `check_compilation` reconstructs from the binary and compares with `rest[0]` modulo `_normalize_with_unchangeables`; the normalizer tries neither binder nor premise permutations, so non-canonical citations are rejected.
...
**Code.** `prover.cpp::proveKernel` — the deferred-compaction drain block's `addOrigin(... "compilation"...)` call — and the mirror block in `prover.cpp` `ExpressionAnalyzer::broadcastTheorems`. Reconstruction helpers `ce::extractExpressionUniversal` (in `compiler.hpp`) and `this->reconstructImplicationFullBind` (in `prover.hpp::ExpressionAnalyzer`).
...
<a id="i-49"></a>
## I-49 Every entry of the whole-key map, the subkey map, the remaining-args edge map and the originals chain map carries the full list of the rules that installed it; a removal deletes one owner, an owner-less entry is erased at the end of the apply; none of them is scope-wiped

**Scope.** The `HashMemory` owner-run containers — `normalizedEncodedKeys` (whole keys → `RuleOwnerRec` run), `normalizedEncodedSubkeys` (every growable prefix of every premise permutation → the `OwnerSet` record with its owner list), `remainingArgsOwners` (one owner run per `(arg-set, NormKey)` edge of `remainingArgsNormalizedEncodedMap`), `originals` (implication chain → owner run) and `copyOwners` (multiplication copy → the run of the RULES that produced it: the index owner is the copy text's id, and under `allow_multiplication` two rules can multiply into one copy, so a copy's entries and LMVs leave only with its last rule). Established by [D-71](40_decisions.md#d-71) with four; the `…MinusOne` / `…MinusTwo` pair is deleted with the obligatory stump ([D-295](40_decisions.md#d-295)); the owner lists reverse the count-free halves of [D-302](40_decisions.md#d-302) / [D-303](40_decisions.md#d-303) ([D-312](40_decisions.md#d-312)).

**Rule.**
1. **Owner identity.** An owner is the packed `(originalImplicationId, validityId)` pair (`RuleOwner`, `packRuleOwner`) — the same pair the LMV carries and the `encodedMap` drop predicate matches; one rule installed into one `HashMemory` instance at one scope is one owner.
2. **Whole-key map.** Every installing rule adds its owner to each of its whole keys (`addWholeKeyOwner` → `addOwnerToRun`: a new key gets the one-owner run, a present key gains the owner, a repeated owner — a symmetric rule reaching the key through two permutations — is a no-op). Runs are sorted ascending by the packed owner, duplicate-free. The request generator's record probe stays `wholeKeyPresent`, a bare lookup; it asserts that a present key has a non-empty run.
3. **Subkey map.** A subkey of fewer than `kSubkeyUCheckMinElements` (3) premises holds the signature-free record (`addShortSubkeyOwner`: loose byte set, zero signatures, the owner list with every owner loose); from three premises on it holds the u_-signature record (`mergeSubkeySignatures`: loose byte + the set of distinct owner signatures + the list of (owner, signature index) pairs — one rule may hold several pairs, since different premise prefixes of its permutations carry different u_ literals; -1 marks a loose contribution). The growth probe `subkeyUSatisfied` reads the loose byte and the signatures exactly as before — the owner section sits behind them and is never read on the hot path ([I-100](#i-100)).
4. **Remaining-args edges.** `insertRemainingArgsNormKeyBatch` records the owner on every distinct `(arg-set, NormKey)` edge of the batch (`remArgsEdgeKeyInto` builds the edge key: the arg-set id run followed by the NormKey blob's id run), whether the NormKey is new to the forward run or already present. The forward run stays a pure NormKey run and the derived `ReverseArgsIndex` is untouched ([I-154](#i-154)).
5. **Originals.** The chain `premises ++ head` gains the owner (the same chain text installed at two scopes is two owners of one chain).
6. **Removal.** A rule's removal re-enumerates its keys through the install's own enumeration and deletes its owner from every visited entry — asserting the owner is present (the tripwire that the enumeration is a pure function of the rule text and the config); an entry whose run is empty is erased once per apply per container (`eraseEmptyRuns`), never per key. The maps are never scope-wiped: a rule that leaves through `wipeSubtree` leaves its owners behind — a stale entry is a sound over-approximation (the request reaches the firing site, whose `encodedMap` misses), never a lost firing.

**Why.** A subkey, a whole key, an edge or a chain is legitimately shared by many rules — `makeNormalizedSubkeys` walks every permutation of an implication's key array, and a template premise shared by thousands of ground instances is the pathological case (the D1 flood: 16.7k owners under one one-premise subkey — the storage the maintainer accepted for exact removal). Only the full owner list makes removal exact: a reference count would be exact only if install and removal enumerate identically, and could not tell which rule a stale entry belongs to; the list asserts it. Dropping a live key would lose firings, so the maps shrink only through owner removal.

**Spot.** A whole key / subkey / edge / chain with an empty owner run outside the window between a removal and its end-of-apply `eraseEmptyRuns` (`wholeKeyPresent` asserts); an owner recorded on an entry the rule's enumeration never visits (the removal's presence assert fires on another rule); a subkey record whose owner section is read by the growth probe.

**Fix.** Whole-key installs go through `addWholeKeyOwner`, at `addToHashMemory` and `makeNormalizedKeysForAdmission` (`installAdmissionMarkerVariants`); subkey installs go through `addShortSubkeyOwner` / `mergeSubkeySignatures` by length at the same two sites; edge owners through `insertRemainingArgsNormKeyBatch`; chain owners through `addOwnerToRun` on `originals`; `wipeHashMem` wipes none of them.

**Code.**
- [`memory_infra/hash_memory.hpp::HashMemory`](../GL_Quick_VS/GL_Quick/src/memory_infra/hash_memory.hpp) — the four owner-run containers (`TypedColdBlobMap<NormKey, RuleOwnerRec>` / `TypedColdBlobMap<NormKey, OwnerSet>` / `TypedColdBlobMap<IdVecKey, RuleOwnerRec>` twice).
- [`memory.hpp`](../GL_Quick_VS/GL_Quick/src/memory.hpp) — `RuleOwner` / `packRuleOwner` / `RuleOwnerRec` / `Codec<RuleOwnerRec>`, `OwnerSet::owners` + `Codec<OwnerSet>`, `OwnerSetBlob`'s owner readers, `recordSubkeyOwner` (the heap oracle).
- [`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp) — `addOwnerToRun` / `addWholeKeyOwner` / `wholeKeyPresent` / `addShortSubkeyOwner` / `mergeSubkeySignatures` / `remArgsEdgeKeyInto` / `insertRemainingArgsNormKeyBatch` / `kSubkeyUCheckMinElements` / `subkeyUSatisfied`.
- [`memory.cpp::addToHashMemory`](../GL_Quick_VS/GL_Quick/src/memory.cpp), [`memory.cpp::makeNormalizedKeysForAdmission`](../GL_Quick_VS/GL_Quick/src/memory.cpp), [`memory.cpp::installAdmissionMarkerVariants`](../GL_Quick_VS/GL_Quick/src/memory.cpp) — the install sites.
- Unit tests: `test_memory.cpp` — `add_owner_to_run_sorted_unique`, `whole_key_present_reads_presence_only`, `short_subkey_owner_record`, `merge_subkey_signatures_raw_blob_matches_record_usignature`, `rem_args_edge_key_layout`, `insert_remaining_args_batch_matches_sequential`, `add_to_hash_memory_records_owners_everywhere`, `subkey_u_satisfied_reads_signature_only`.
...
**Incubator topology ([D-332](40_decisions.md#d-332), [I-220](#i-220)).** The routing law is unchanged, but in incubator mode an LB strictly below the anchor LB registers its mail ancestors only up to the anchor LB — the root sentinel is not on its edge list — so the root's log (the theorem-rule broadcasts) has one reader, the anchor LB. The externals seed of an incubator batch is stored in the anchor LB's log instead of the root's, which is still an ancestor log for every reader (`broadcastTheorems`'s `store` parameter).
...
<a id="i-70"></a>
## I-70 RETIRED (D-304) — the request-generation validity prune is deleted

**RETIRED (2026-08-25, [D-304](40_decisions.md#d-304)).** `ownerSetHasComparable` (both overloads) and its only caller `ownerKeyAccepts` are deleted; no probe reads an owner scope — the whole-key record probe is `wholeKeyPresent` (presence only, [D-305](40_decisions.md#d-305)), the subkey growth probe `subkeyUSatisfied` (u_ signature only, [D-302](40_decisions.md#d-302)) — and the whole-key record stores scope ids for the wipe alone ([I-49](#i-49)). The text below is the historical contract of the deleted prune.

**Rule.** At every whole-key owner-set match site a request is pruned only when its consensus validity is non-`main` AND `nm.comparable` to **no** owner of the matched key. The predicate must stay implied by "the request can fire": it is a runtime optimisation, never a soundness gate. The firing gate enforces the same comparability independently, so the prune is forbidden from changing results — `files/theorems/theorems.txt` must be byte-identical with the prune active or stripped.
...
**How to spot.** Each prune site reads `ownerSetHasComparable` after a lookup hit; the consensus vid is the `deeperOf` of the request's premises, folded one premise at a time as the search grows. Each owner's validity id rides the low half of its packed composite in `OwnerSet::partitionIds`, encoded once at insert and removed with its owner at wipe ([D-105](40_decisions.md#d-105), [I-80](#i-80)).
...
> **Retired on this branch by [D-299](40_decisions.md#d-299) (2026-08-22).** `consumedAdmissionKeys` is deleted along with the cleanup that was its only writer; the four insert-site gates and the `isAdmitted` assert are gone with it. Superseded by [I-200](#i-200). The rest of this entry describes the retired exclusion.
...
<a id="i-80"></a>
## I-80 RETIRED (D-304, then D-303) — the composite owner ids are gone; the whole-key index is a bare key set

**RETIRED (2026-08-25, [D-304](40_decisions.md#d-304)).** The composite `(expandedOriginal, scopeVid)` id, `makePartitionId`, `ownerSetHasComparable` and `partitionAccepts` are deleted. A whole-key record now holds `OwnerSet::ownerScopes` — the distinct scope validity ids of its installing rules — — itself deleted the same day by [D-303](40_decisions.md#d-303): the whole-key index is a bare key set with no record. The text below is historical.
...
**Scope.** Prover request generation. `ExpressionAnalyzer::ownerSetUSatisfied` (predicate) and its one production call site: the subkey growth probe `subkeyUSatisfied`, which applies THIS prune alone — no scope, no partition — and only from `kSubkeyUCheckMinElements` premises on ([D-302](40_decisions.md#d-302)). The whole-key record probes (the full-key half of `filterIntEncodedStatements`, the record probes of the grow-DFS inside `generateEncodedRequestsStatic`, the terminal probe of `produceExpressionStumps`) read key presence only since [D-305](40_decisions.md#d-305) (`wholeKeyPresent`), and the whole-key record carries no signature at all since [D-304](40_decisions.md#d-304). The insert-side producer is `recordUSignature`, called at every owner insert in `addToHashMemory` and `makeNormalizedKeysForAdmission`. The firing-side mirror is `checkLocalEncodedMemoryStatic`'s `encodedMap` (`ignoreU=true`) lookup.

**Rule.** At every whole-key owner-set match site a request is pruned only when, in addition to failing D-105/partition, **no owner's cached u_ signature is satisfiable** by the request's flattened `argFullId`. `ownerSetUSatisfied` keeps the request when `OwnerSet::hasLooseOwner` (some owner has no u_ constraint, or a literal was not interned at insert), when `OwnerSet::uSignatures` is empty, or when any signature's every `(slot, id)` pair equals the request's `argFullId[slot]`. The predicate must stay implied by "the request can fire" — a runtime optimisation, never a soundness gate. The firing gate enforces the exact u_ literal match independently (the `encodedMap` key pins each unchangeable arg's literal), so the prune is forbidden from changing results — `files/theorems/theorems.txt` must be byte-identical with the prune active or stripped.
...
**Scope.** Prover. The `performElem2` call tree (`generateEncodedRequestsStatic`, `checkLocalEncodedMemoryStatic`) run by the flat phase-2 executor pool.
...
<a id="i-76"></a>
## I-76 SUPERSEDED ([I-191](#i-191)) — the multi-part early-exit suppression is replaced by the deterministic doom line

**Scope.** Prover (historical). The former `g_isMultiPart` gates in `BurstSink::canAccept` / `::consume`.

**Rule.** SUPERSEDED. This entry required the burst early-exit be honored only for a single-part LB, with every multi-part LB running to completion, because siblings bailing on a shared boolean `stop` flag was a thread-timing race (a sibling not yet past its useful firing could be cut, so the proved-theorem set depended on scheduling — it surfaced as a heisenbug where the hashburst dump's mutex latency flipped a lost Peano `1·x=x`-mirror theorem back to present). The B10 RT investigation then measured the suppression's cost: ~15.8 s of a 16.6 s burst was post-goal grinding across 32 buckets that a single-part run skipped in 0.77 s — the "wasted-once-doomed work costs nothing" premise was false at scale.

[I-191](#i-191) resolves the race without the suppression: the trigger publishes its deterministic stream POSITION (not a bare boolean), every part stops at that position in its OWN stream, and the merge takes the winning part's chain alone — output is a pure function of proof state while wall-clock timing shapes only wall-clock. The per-burst thread-local `g_isMultiPart` is deleted with the gates; `g_splitCount` still gates only `partitionAccepts` ([I-159](#i-159), likewise superseded on the early-exit half).

**See also.** [I-191](#i-191), [D-282](40_decisions.md#d-282), [D-121](40_decisions.md#d-121), [D-201](40_decisions.md#d-201).
...
<a id="i-85"></a>
## I-85 — `intKnownStatements` row presence IS the membership: known ⟺ admitted ⟺ a row with non-empty levels

**Scope.** Prover / Memory. `Memory::intKnownStatements`, `StatementFlags` (`local` + `fullyDisintegrated` payload only), the write door `upsertStatementKey` (`memory.hpp`).

**Rule.** One membership, carried by ROW PRESENCE: a row exists iff the statement passed the one admission door (`addStatement` and the doors it dispatches to — `addEquality` / `addNegatedEquality`, the equivalence-class commit, the status-4 fact load, anchor handling, the compressor load) — and every row is paired with a NON-EMPTY `intStatementLevelsMap` row ([I-182](#i-182)). Every reader tests bare presence (`find!= nullptr`): the Site F ancestor scans (`addExprToMemoryBlock`, `checkLocalEncodedMemoryStatic`), the contradiction negation scans (`dischargeContradiction`, `burstDeactivates`), the burst dependency skip (`BurstSink::consume`), the ordisMerge / dead-branch probes, the equality-door run-once gates + mirror asserts, the integration-preparation gates, and the typing/head/anchor gates. `fullyDisintegrated` is OR-only through `upsertStatementKey` and never grants a row (the marker flags the row admission created, or no-ops). The only row-clearing writers are whole-entry erases (`wipeSubtree`, `eradicateImplicationFromLB`, `resetResentExpressionRegistries`, `resetParkedOrStatementRegistries`) and the CE teardown's full reset (`releaseCEBatchMemory`). A statement the door refused (iteration cap, secondary-variable cap, equivalence filter) leaves NO row.

**Why.** The former two-bit scheme (`registered` seen-at-a-door vs `known` in-the-level-registry) was an accident of the 2026-06 container fold, and its seams produced the B8 known-without-levels crash: the level row doubled as an admission verdict, writers deliberately withheld it, and the vacuous full-disintegration marker promoted refused statements to `known` without levels. One membership with mandatory levels makes that state unrepresentable; the retired `registered`-only rows were near-write-only (their one probe, head-already-derived, now proves such heads normally — sound).

**How to spot.** Any read of a retired `registered` / `known` field (compile error); a direct `insert({key, flags})` bypassing `upsertStatementKey`; a row created without its paired non-empty levels write (the door asserts).

**How to fix on violation.** Route the write through the admission door; if a clear seems needed, the operation is a teardown — erase the entry through the named teardown paths (which erase flags and levels together).

**Code.** `memory.hpp::upsertStatementKey` (write door), `memory_infra/int_encoded_expr.hpp::StatementFlags`; presence reads in `prover.cpp::addExprToMemoryBlock` (Site F), `memory.cpp::checkLocalEncodedMemoryStatic`, `prover.hpp::dischargeContradiction` / `burstDeactivates` / `BurstSink::consume` / `ordisMerge`.

**See also.** [D-264](40_decisions.md#d-264), [I-182](#i-182), [D-128](40_decisions.md#d-128), [I-58](#i-58), [I-84](#i-84).
...
**Scope.** Prover / admission + rejection — `TemplateInterner`, `mintTemplateKey` / `lookupTemplateKey` / `decodeTemplateKey`, the five maps (`admissionMap`, `admissionStatusMap`, `admissionMapIntegration`, `rejectedMap`, `rejectedMapIntegration`), `revisitInProgress`, the three `varsIn*Keys` caches, and every reader/writer listed in `D-132`.
...
**Siblings.** The same arena backs the persistent side tables that must stay readable while the main arena is cold — `pendingDisprovedGoals`, `pendingDeadOrBranches`, `orRetiredDisjuncts`, `pendingOrReleases`, `orStarterPick`, and the or-branch freeze pair `orLiveBranches` / `frozenOrBranches` ([I-206](#i-206)); every one of them resets in `dischargeStatementContent` before `persistentArena.releaseAll`, none is in any deload stream or dump section.
...
**UPDATED ([D-337](40_decisions.md#d-337), 2026-09-03).** The `WorkerOwned` early return of `claimAndLoadForWork` is phase-2-only. The handshake publishes the claimant in `Memory::claimHolder` right before its release-store of `WorkerOwned`; in phases 1 / 3 / 4 a `WorkerOwned` re-entry asserts that the caller IS that holder (a seam drain re-claiming an LB it already holds is the one legitimate case) — a foreign holder is two writers on one LB and aborts at the claim.
...
<a id="i-100"></a>
## I-100 The cold owner-set maps (`normalizedEncoded*`) are READ on the request-generation hot path via a zero-allocation byte peek, never a full `OwnerSet` decode — the prune's short-circuits stay free

**Scope.** The two `normalizedEncoded{Keys,Subkeys}` indexes in every `HashMemory` — the whole-key owner map `TypedColdBlobMap<NormKey, RuleOwnerRec>` and the subkey record map `TypedColdBlobMap<NormKey, OwnerSet>` (one blob per key, run-length-1, whole-value replace). The request-generation probes hit them 10^4–10^6× per burst — the subkey map through `subkeyUSatisfied` (lookup, then only the loose byte and the u_ signatures, from three premises on — the record's owner list sits BEHIND the signatures and is never read here, [I-49](#i-49)), the whole-key map through `wholeKeyPresent` (a bare lookup; the owner run is never read) — and the subkey common case (loose owner) reads only the `hasLooseOwner` byte before short-circuiting. So the hot read must NOT materialize the `OwnerSet` (two `std::set`s plus the owner vector, the cost the blob map's `recordsAt` would pay): `subkeyUSatisfied` does the `lookup` first (a miss returns `false` before any value read), then `TypedCold::peekRecordBytes` (zero-copy off the arena, a `thread_local` scratch only on a rare page straddle) wrapped in an `OwnerSetBlob` view whose readers the byte-overload `ownerSetUSatisfied` consumes in place. The prune stays a sound over-approximation ([I-79](#i-79)). Writes are install-time / removal-time (`mergeSubkeySignatures` / `addShortSubkeyOwner` RMW on the subkey map; `addWholeKeyOwner` on the whole-key map), single-threaded, not on this path.

**Code.** `ExpressionAnalyzer::subkeyUSatisfied` / `wholeKeyPresent` / `mergeSubkeySignatures` / `addShortSubkeyOwner` / `addWholeKeyOwner`; `OwnerSetBlob` (byte view, layout-coupled to `Codec<OwnerSet>`); `TypedCold::peekRecordBytes` → `HashMap::peekBlobContiguous` → `BlobCsrValueStore::peekBlob` (`PagedVector::contiguousRun`); the `OwnerSetBlob` overload of `ownerSetUSatisfied`. See [D-140](40_decisions.md#d-140), [I-49](#i-49).
...
3. **Erases batch through `eraseBlobIf` / `eraseIf`.** Single-key removals (the subtree wipe) collect the keys then run ONE compacting pass per container; `rejectedMap` is never erased ([I-37](#i-37)). The compaction marks `Restructured` so a mutated container is never tail-delta'd ([I-119](#i-119)).
...
**Status note.** The hash-memory twin `sanitizeHashMemory` is deleted ([D-312](40_decisions.md#d-312)); point 6 and its `PendingSanRec` / `SanPairRec` / `eradicateImplicationFromLB` obligations are retired with it, as are the unit tests `expimpl_snapshot_idx_sort_matches_ewv_sort` and `sanitize_levels_run_probe_matches_cold_int_set`. Points 1–5 (the goal twin `sanitizeToBeProved` and the shared core) stand.
...
**Scope.** `makeNormalizedKeysForAdmission` (`memory.cpp`) and the four write doors it wires: `encodeNormKeyInto` / `appendLmvIdsRecord` (`memory.hpp`), `ensureWholeKeyPresent` / `insertRemainingArgsNormKey` raw-run overload (`prover.hpp`).
...
**Code.** `prover.hpp · mergeSubkeySignatures`; `memory.hpp · buildUSignatureRunInto`, `OwnerSetBlob` (`uSigCount` / `firstSigOffset` / `sigPairCount` / `sigBytes` / `sigPairFirstAt` / `sigPairSecondAt`). Unit tests: `build_usignature_run_matches_record_usignature`, `merge_subkey_signatures_raw_blob_matches_record_usignature` (`test_memory.cpp`). See also [I-83](#i-83), [I-84](#i-84), `I-99`, `I-100`, `I-138`.
...
**Code.** `memory_infra/reverse_args_index.hpp · ReverseArgsIndex` (`appendEdge` / `removeEdge` / `reverseIndexRunOf` / `rebuildReverseIndex` / `clear`); wired at `memory.cpp · checkLocalEncodedMemoryStatic` (probe), `prover.hpp · insertRemainingArgsNormKey` (append), `memory.cpp · Memory::reloadFromImage` / `releaseStaticBlocks` (reload/release hooks). Unit tests: `rebuild_membership_matches_brute_force` / `append_edge_matches_brute_force` / `remove_edge_matches_brute_force` / `clear_then_rebuild_is_identical` (`test_reverse_args_index.cpp`). See also [I-117](#i-117), `I-107`, `I-95`, `I-103`, `I-111`, [D-199](40_decisions.md).

---

<a id="i-155"></a>
## I-155 — there is exactly ONE request generator with exactly ONE mandatory-ingredient mode: containment terms checked at a finished whole key, and the grow loop bumps the submatch tally exactly once per node


**Scope.** `ExpressionAnalyzer::generateEncodedRequestsStatic` (`memory.cpp`) and its four call sites in `performElem2` (`prover.cpp`) plus the counter-example call site. Supporting: `filterIntEncodedStatements` (`memory.cpp`), `requestGatesPass` (`prover.hpp`), and `MandatoryTerm` (`memory.hpp`).

**Rule.**
1. **One function, one mode.** No second request generator may be added, and no second way of saying what a request must contain. A new requirement is a new term shape, or it is a parameter — never a copy of the grow DFS. The retired `generateEncodedRequestsStaticPairs`, `generateEncodedRequestsStaticCE`, `growBaseCandidates`, `filterIntEncodedStatementsCE`, `makeMandatoryEncodedStatementLists1/2Static`, `preEvaluateFromEncoded`, `Stump`, `BaseCandidate` and the pairing merge itself are all gone; re-introducing any of them is the violation this invariant exists to catch.
2. **Everything is fixed, because there is nothing left to select.** Target index = the whole-key set `normalizedEncodedKeys`. Grow depth = `maxKeyLength`. `alsoAcceptFullKeys` = always true, because a candidate IS the whole request and a statement that is a whole key on its own is therefore emittable. There is no seed phase and no merge phase.
3. **The terms are a disjunction of conjunctions.** `MandatoryTerm` is one AND-group of statement views: a finished candidate is emitted only when it holds a statement from EVERY view of at least ONE term. Membership becomes one bit per filtered statement, computed once per call, and the search prunes a position as soon as no term can still be completed — neither within the remaining depth nor from the suffix of the sorted list still ahead of it. Equivalence with the retired merge rests on the subkey map being closed downward: every subset of a whole key is a growable subkey, so the merge's per-pair viability pre-filter and its recordable-base (minus-`k`) probe are both implied by the whole-key probe at the finished candidate. Emitting an already-satisfied candidate is what the retired seed phase did for a stump that was already a complete key.
4. **The submatch tally is bumped once per grow node, by the subkey probe alone.** The node runs `requestGatesPass`, builds one normalized key, and probes the subkey map through `subkeyUSatisfied` (u_ signature only, from three premises on, [D-302](40_decisions.md#d-302)) and the whole-key map through `wholeKeyPresent` (key presence only, [D-305](40_decisions.md#d-305)); the subkey verdict bumps the tally once (`if (subOk) ++g_growthMatchCount`), the whole-key probe never counts. Nothing else may bump it. The tally is the doom-line coordinate and drives the LB-split policy (D-109), so an extra bump is a proof change, not a metric wobble. A position the containment prune skips is not a grow node and contributes nothing — the tally counts enumeration actually performed, which is why the containment path reaches a far smaller total for the same proof than the merge did (`min_split_work` was re-derived from that scale).
5. **A candidate emits inside the search, and an EMPTY TERM LIST is a first-class input.** A recorded candidate IS the request and is handed to the consumer at the node that found it. Buffering candidates and emitting after the DFS would keep the answer and destroy the counter-example filter's contradiction early-exit (I-73). An empty term list does not mean "the counter-example filter": it means no ingredient is mandatory, so every whole key is a request, and two callers use it — the CE filter and the batch whose rule registry is itself the new thing (`localHashMemoryDelta`), which must meet every visible statement rather than only this burst's. The "one unsplit LB per conjecture" statement that licenses the CE path's unread tally lives at the CE CALL SITE in `performElem2`, the only caller for which it is true; the generator cannot tell a CE call from a termless prover batch, and must never infer "termless" ⇒ "unsplit".
6. **A candidate need not be growable to be recorded.** Its record gate is `wholeKeyPresent(normalizedEncodedKeys) && (no terms || termsSatisfied)`. The subkey probe decides only whether the search continues past this node. Re-adding a `subOk` conjunct to the record gate loses every request whose whole key is not also a growable subkey.
...
**How to spot violations.** A second function whose body contains `ArenaStack<StackItem>` over `filteredIdx`; any counting probe inside the grow DFS; a candidate buffered instead of emitted at the node that recorded it; a record gate that also demands the subkey probe; an emit-vs-skip decision keyed on whether terms are present rather than on what the terms say; a minus-`k` owner map reappearing.

**How to fix on violation.** Fold the second generator back in as a parameter of the existing one. If the grow DFS genuinely needs a counting probe, prove the tally is unobservable on that path — do not assume it.

**Code.** `memory.cpp · generateEncodedRequestsStatic` / `filterIntEncodedStatements`; `prover.hpp · requestGatesPass` / `BurstSink::canAccept` / `g_growthMatchCount`; `memory.hpp · MandatoryTerm` / `StaticRequestEmitter`; `prover.cpp · performElem2` (the five call sites). Unit tests: `test_memory.cpp · generate_encoded_requests_static_grow_reaches_the_whole_key` (the search reaches a whole key with an empty term list, with an always-satisfied term, and not at all when the key is un-installed), `generate_encoded_requests_static_mandatory_containment`, `mandatory_containment_survives_the_expression_split`, `filter_int_encoded_statements_also_accept_full_keys`, `request_gates_pass_hypo_scope_and_length`. See also `D-200` (superseded), `D-298`, `D-295`, [I-70](#i-70), [I-79](#i-79), [I-73](#i-73), [I-130](#i-130).
...
<a id="i-156"></a>
## I-156 the split stump NAMES A NODE of the same search; a run owns that node's subtree, so the stumps partition the unsplit enumeration
...
1. **The stump is part of the candidate, and the target map stays the whole-key map.** Attaching a split stump to an obligatory sequence would have made a candidate a key minus `|O| + |S|` elements, requiring a minus-`k` owner-set map for every stump length — an unbounded family of cold containers in the deload stream. The obligatory stump is now gone entirely and the minus-`k` maps with it, so the target map is unconditionally `normalizedEncodedKeys`; this clause survives as the reason never to reintroduce one.
2. **The stump IS the growing candidate — a run owns its node's SUBTREE.** The run seeds the depth-first stack with the stump itself (`init.count = |S|`, `init.allIdx = S`, `init.validityId` the fold of its elements' scopes) and its dispatch cursor at `init.start = pos(last element) + 1`, then grows it exactly as the unsplit search grows any node. The stump is never attached to or detached from a candidate, and the grow-depth test is the plain unsplit `top.count + 1 > targetLen`. Because the search extends only in ascending filtered-list position, every candidate has exactly ONE path from the root, so distinct stumps' subtrees are DISJOINT and their union is the whole unsplit enumeration: the split is an exact PARTITION generating the identical request set, once each. Termination is structural — children only ever push `start = i + 1`, so a run cannot climb out of its subtree and its stack empties precisely when the search would pop the stump's last element.
...
**Why.** (1) is what makes the dimension implementable at all. (2) is what makes the split nearly free: under the retired containment scheme the stump was attached to freely-grown candidates for the probes, so a candidate set `K` was re-derived once for every stump it contained, and the split inflated total work by the mean accepted-candidate size. Measured on the FTA shortcut's heaviest burst that was 2.69x, which is why splitting it bought 1.23x wall clock for 7.25x the CPU. Subtree ownership removes the redundancy by construction rather than by dedup. (3) is the difference between the sub-part emitting `{x}` and losing it silently.

Coverage of (2) rests on the producer enumerating in the SAME order the consumer grows in — `produceExpressionStumps` works in filtered-list positions and grows a child at `pos + 1`, so a stump always names a reachable node of the consumer's own search — and on every stump element being present in the consumer's filtered list. The latter follows from downward closure: the subkey map rejects every superset of a non-subkey, so a stump whose whole key the map accepts has every element accepted, and `filterIntEncodedStatements` keeps exactly those under the same reference memory and `maxIteration` gate. The position lookup asserts it (Rule 19) rather than tolerating a miss.

**The narrower-registry qualifier.** "The same reference memory" is load-bearing. The producer grows its stumps over the LB's OVERALL rule registry, but a batch reading a narrower one (`workingMemory`, `localHashMemory`, `localHashMemoryDelta`) may hold no rule that could ever use one of the stump's statements, so that element is legitimately absent from the batch's filtered list. Such a stump contributes NOTHING to that batch and the run skips it: by the same downward closure, a set containing a non-subkey is itself a non-subkey, so the stump fails both the growth and the record probe, and so does every candidate in its subtree — each one still contains that element. The stumped modes reach this verdict at the subkey gate, one probe after the seed; the containment mode must reach it BEFORE the gate, because it resolves the stump's view bits up front, so it tests the positions itself and skips on the first miss (`stumpIsUsableHere`). The skip is not a tolerated miss — it is the batch's correct answer, which is why the ascending-order assert in the seed frame stays a hard assert: nothing reaches it but a stump the subkey gate already accepted.

**How to spot.** Split and unsplit submatch totals for one burst differing — they must be EQUAL; compare the `[SPLITSTAT]` work column against an `lb_split: false` run. Also: a base candidate of size 1 vanishing under a stump split (the empty-candidate node was not probed); the unit tests `split_stump_search_partitions_the_unsplit_request_set` (whose `s0 + s1 + s2 == unsplit` assertion IS the partition property) or `split_stump_alone_is_recorded_as_a_base_candidate` failing; the ascending-position assert firing; a `MinusThree` map appearing.

**How to fix on violation.** Keep the stump as the seed frame with its cursor past its own last element — never re-attach it per probe. Keep the empty-candidate probe with its tally bump.

**Code.** `memory.cpp::generateEncodedRequestsStatic` (the `posInFiltered` lambda, the seed `StackItem`, and the pre-search probe), `memory.cpp::produceExpressionStumps`.
...
**How to spot.** The old `stump fan drops a recordable base candidate` assert; a recordable shallow request missing only when the producer grows beyond level 1; a minus-`k` map reappearing in the retention probe; `terminalOnly` records entering the depth-first stack; a producer calling the checker directly; a terminal record tested only against `overallHashMemory` and trusted without the per-batch probes.
...
<a id="i-158"></a>
## I-158 a stump sub-part owns a BUCKET of stumps; the filter is built once per bucket
...
**Rule.** A straggler's round-1 PRODUCER task ([D-201](40_decisions.md#d-201)) returns the regular frontier its whole-LB filter grows (up to `kStumpsPerBucketTarget × logicalCores`) plus any recordable terminal pre-stumps from replaced levels. `proveKernel` deals all records round-robin into `min(stumpCount, logicalCores)` buckets, permuted so each bucket is a contiguous run, and one bucket part runs per BUCKET. Inside the generator the statement filter and its name sort are built ONCE per bucket; the search then runs once per regular stump and performs only the shallow probe for a terminal stump. The bucket's stumps own DISJOINT subtrees ([I-156](#i-156)), so its yield is their sum with nothing to collapse.

**Why.** One part per stump would multiply the part count. Bucketing pays the fixed per-bucket setup (`filterIntEncodedStatements` and its name sort) once instead of once per stump. Growing more stumps than buckets (the `kStumpsPerBucketTarget` multiple) then round-robin dealing spreads each bucket's draw across the whole name-sorted list, which matters MORE under subtree ownership than it did under the retired containment scheme: subtree size falls steeply with list position, so a bucket must hold a mix of early and late stumps. The residual imbalance is real and visible — on the FTA shortcut's burst 25 the busiest bucket runs about 2x fair share and the split-ineffective report fires — which makes the deal order the next lever.
...
<a id="i-159"></a>
## I-159 RETIRED (D-304) — the rule dimension is deleted; the expression split is the only split

**RETIRED (2026-08-25, [D-304](40_decisions.md#d-304)).** `g_splitCount` / `g_splitProcessID`, `partitionAccepts` and the `processID` / `splitCount` parameters of `performElem2` / `ExecTask` / the CE-filter call are deleted; the expression (bucket) split is the only split dimension, and every bucket sees every rule by construction. The text below is historical.

**Scope (historical).** Prover. `prover.cpp::performElem2`, `prover.cpp::proveKernel`, `memory.hpp::partitionAccepts`.

**Rule.** `g_splitCount` (+ `g_splitProcessID`) is the RULE dimension, and ONLY `partitionAccepts` reads it. A whole-LB expression/bucket split keeps `g_splitCount == 1` so `partitionAccepts` accepts EVERY rule — the buckets partition the expression search, not the rules. Setting `g_splitCount > 1` for the expression split would rule-partition the buckets: a request whose rule falls in partition `j` but whose expression falls in bucket `k ≠ j` is then covered by NO part — incomplete, LOST theorems.

The entry's second half — the early-exit reading the per-burst `g_isMultiPart` — is SUPERSEDED with [I-76](#i-76): the doom line ([I-191](#i-191)) stops every part at a deterministic position in its own stream, so the early-exit no longer needs a multi-part signal at all and `g_isMultiPart` is deleted. The burst early-exit machinery reads the per-LB doom-line atomic, never `g_splitCount`.

**Why.** The expression split is sound only because every bucket sees every rule; `g_splitCount` expresses exactly that and nothing else. `performElem2` asserts `splitStump.count == 0 || partCount > 1` to catch a bucket dispatched as single-part.

**How to spot.** `partitionAccepts` reading anything but `g_splitCount`; a bucket part dispatched with `splitCount > 1` (would drop cross-terms); theorems present unsplit but absent when a straggler splits.

**How to fix on violation.** Dispatch bucket parts at `splitCount == 1`, `partCount = buckets`. Keep `partitionAccepts` on `g_splitCount`.

**Code.** `prover.cpp::performElem2` (sets `g_splitProcessID` / `g_splitCount`), `prover.cpp::proveKernel` (dispatches buckets at `splitCount == 1`, `partCount = buckets`).

**See also.** [D-201](40_decisions.md#d-201), [D-121](40_decisions.md#d-121), [I-76](#i-76), [I-191](#i-191), `I-160`.
...
- A DOOMED LB — one whose burst early-exited at the [I-191](#i-191) doom line this iteration — contributes NOTHING: excluded from the `T` sum and from classification, its `numberOfParts` set to the unsplit default (`lbDoomed` latch, [D-292](40_decisions.md#d-292)). An early-exited part's tally is cut at a timing-dependent overshoot past the published line (I-191 protects the merged content, not the tallies), so folding it in would make `T` — and any borderline straggler verdict — a race outcome. Whether a doom trigger fires at all IS deterministic (the winning line's position is always reached by its part), so the exclusion set is itself a pure function of proof state.
...
<a id="i-174"></a>
## I-174 — an `_ordis_` cohort releases its branches one at a time, in a ranked order that is a pure function of the pending disjunct set


**Scope.** `prover.cpp::disintegrateExprCore2` (the OR case's bootstrap: guard, ranking, single-branch mint, queue fill), `prover.hpp::pickTopOrDisjunct` / `orDisjunctTier` / `orDisjunctTokenCount` / `collectAnchorArgs` (the ranking), `prover.hpp::ordisMerge` (the goal probe), `prover.cpp::drainDeadOrBranches` (retirement staging), `prover.cpp::drainPendingOrReleases` (the release drain), `Memory::orPendingBranches` / `orPendingLevels` (deloadable queue, tags 805..810), `Memory::pendingOrReleases` (persistent staging inbox).

**Rule.** Cohort creation mints NO branch scope: the bootstrap registers the FULL structural leaf count in `orDisjunctCount` (so convergence fires only when every counted branch derived the expression), queues EVERY leaf in `orPendingBranches`, and stages the cohort on `pendingOrReleases` — the bootstrap runs inside `standardProcessing`'s internal-mail drain, whose step-3 clear would wipe any mid-drain seed insert, so ALL releases go through the one end-of-burst drain. Each release picks the top-ranked pending disjunct under the deterministic order: tier ascending (non-equalities, then bare equalities, then equalities carrying an anchor argument), token count descending within a tier, byte-lex ascending as the tie-breaker — re-derived over the REMAINING set at each release, never stored. ONE exception: a cohort opened by the route-(a) demand probe ([D-252](40_decisions.md)) records its ADMITTED disjunct in the one-shot `orStarterPick` row (persistent arena; ties among several admitted disjuncts fall to the standing ranking), and the drain releases that disjunct first, erasing the row on consumption — a recorded starter no longer pending falls back to the ranking, and every later release ranks as usual. After the first release, the next branch releases ONLY when the live branch resolves: it deposits an expression matching a `toBeProved` goal at a scope on its parent chain up to main (the `ordisMerge` goal probe), or it retires refuted (D-242). At most one branch releases per staged cohort per burst. An existing count row is the bootstrap guard — a re-deposit of the same or statement never re-queues, re-stages, or resets the sequence. An undecided live branch stalls its cohort — a defined degradation, no timeouts, no caps.

**Why.** Sequencing caps the live-branch population at one per cohort, and the ranking makes the expensive assumed-equality branches (the equivalence-class replication pathology) open only after every productive branch already resolved. (The eager admission that originally motivated the pure-ranking-only first pick — `allowOrDisintegration = !doNotDisintegrate` at the `addExprToMemoryBlock` entry, [D-250](40_decisions.md) — is superseded by the two-route opening of [D-252](40_decisions.md); on route (a) the demand names WHICH branch carries the relevance, so it opens first.)

**How to spot.** More than one `_boundary_ordis_` scope of one cohort minted in a burst where none resolved; an `orPendingBranches` run shrinking without a matching release seed on `sameIterationInternalMail`; a cohort whose count row is missing while pending rows persist (the retirement sites must erase them together); ranking consulting raw `u_`-prefixed leaf bytes (the anchor-argument tier test would silently never fire).

**How to fix on violation.** Route every release through `drainPendingOrReleases`; never mint a branch scope for a queued disjunct at any other site; keep the ranking inputs the CLEAN disjunct forms plus `collectAnchorArgs`' span set.

**Code.** `prover.cpp::disintegrateExprCore2` (OR case), `prover.cpp::drainPendingOrReleases`, `prover.hpp::ordisMerge`, `prover.hpp::pickTopOrDisjunct`. Tests: `tests/test_memory.cpp::pick_top_or_disjunct_ranking`, `::collect_anchor_args_from_chain`, `::ordis_goal_probe_stages_release`, `::dead_branch_retirement_stages_release`, `::drain_pending_or_releases_releases_top_ranked`, `::sequenced_ordis_bootstrap_mints_one_branch`.

**Freeze and goal gate.** A resolved live branch is FROZEN, never wiped: once every `toBeProved` goal on its chain is known at the branch or above, the end-of-burst `freezeResolvedOrBranches` (between `drainDeadOrBranches` and `drainPendingOrReleases`) moves it from `orLiveBranches` to `frozenOrBranches`, and `filterIntEncodedStatements` drops its subtree from the request universe — every row, bookkeeping entry, history line and the goal itself stay. An LB whose goal registry is empty opens no cohort, parks none and releases no pending branch (the `pendingOrReleases` staging drops). Full contract: [I-206](#i-206).

**See also.** [D-250](40_decisions.md), [I-166](#i-166), [I-167](#i-167), [I-172](#i-172), [I-84](#i-84), [I-206](#i-206).

<a id="i-175"></a>
## I-175 — or-compact elements carry each disjunct's true polarity; consumers negate with double-negation cancellation


**Scope.** `prover.cpp::constructOrTheorem` (registration), `prover.cpp::expandSignature` CASE 4 + `expandCompiledExpressions` or-branch (expansion), `prover.cpp::disintegrateExprCore2` ordis K-rules (exclusion premises), `verifier.py::_negate_expr` + `_build_or_from_elements` + `_build_or_subimpls_from_elements` + `_check_or_disintegration_implication` + `check_or_branch_assumption` (the Python mirrors), `generate_full_proof_graph.py::_build_gl_binary_map` or-branch (rendering).

**Rule.** An `or`-category `LogicalEntity` / GL-binary entry stores its disjuncts VERBATIM — a negated disjunct keeps its leading `!`. Every consumer that needs a disjunct's De-Morgan conjunct (or an exclusion premise) computes the negation with cancellation: a positive element gains `!`, a negated element contributes its bare positive core. No consumer may blindly prefix `!` to an element, and no producer may strip an element's sign.

**Why.** The OR's meaning lives in the elements; a positive-only store flips a negated disjunct's sign on every round-trip, silently turning `(a=b) ∨!(c=d)` into the FALSE `(a=b) ∨ (c=d)`. The one recorded incident (D-251) broadcast such a false theorem into the Gauss incubator, derived an equality between two distinct anchor numerals, and tripped the I-173 anchor-contradiction assert.

**How to spot.** A `!!(` substring in any expanded compound or chapter row; an or-expansion whose polarity differs from the constructing batch's `OR compiled:` log line; an incubator anchor contradiction whose false equality traces to an or-rule firing.

**How to fix on violation.** Restore the sign at the producer, never re-strip at a consumer; the negation helper is `negateScratch` (C++) / `_negate_expr` (verifier).

**Code.** Tests: `test_reconstruct_implication.cpp::expand_signature_or_negated_disjunct_cancels` (+ the mixed-polarity oracle case), `tests/test_verifier_or.py::test_negate_expr_cancels_double_negation` / `test_build_or_from_elements_negated_disjunct_cancels` / `test_build_or_subimpls_negated_disjunct_cancels`.

**See also.** [D-251](40_decisions.md#d-251), [I-23](#i-23), [I-173](#i-173).

<a id="i-176"></a>
## I-176 — every `_ordis_` branch seed rides `sameIterationInternalMail` through the full kernel pipeline


**Scope.** `prover.cpp::disintegrateExprCore2` (the bootstrap seed), `prover.cpp::drainPendingOrReleases` (the release seeds), the `standardProcessing` absorb (the consumer).

**Rule.** A branch's asserted-disjunct seed — the first branch at cohort mint time and every later released branch — is deposited as an `insertInternalStatement` on `sameIterationInternalMail` with the cohort's seed level run, paired with an `or disintegration` mail origin (`addInternalMailOrigin`, dependency = the or signature at the parent scope). No branch seed goes through an `addStatement`-only channel.

**Why.** The retired `orBranchStatements` → `out.statements` channel ran `addStatement` alone, which registers a statement but never disintegrates it — a COMPOUND disjunct (A15's `(existence3[…])` predecessor branch) would sit inert at its branch scope, its witnesses never minted. The internal-mail absorb runs each seed through `addExprToMemoryBlock`'s full pipeline (Site F dedup, disintegration, admission updates, `ordisMerge`), so compound seeds decompose exactly like any deposited statement; the mail origin makes the absorbed seed's history row identical to the former direct `exprOriginMap` write.

**How to spot.** A compound disjunct statement at an `_ordis_` branch scope with no witness facts beside it after its absorb burst; a branch seed appearing in `out.statements`; a seed statement whose `or disintegration` origin row is missing at the branch scope.

**How to fix on violation.** Seed through the two mail doors, never through the disintegration product channel; keep the origin write on the mail channel (the absorb's front-origin read expects it there).

**Code.** `prover.cpp::disintegrateExprCore2` (OR case, step 4a), `prover.cpp::drainPendingOrReleases`. Tests: `tests/test_memory.cpp::sequenced_ordis_bootstrap_mints_one_branch`, `::drain_pending_or_releases_releases_top_ranked`.

**See also.** [I-174](#i-174), [I-21](#i-21), [I-55](#i-55), [D-34](40_decisions.md#d-34).

<a id="i-177"></a>
## I-177 — `ordisOnly`-tagged admission values are cohort-opening demand evidence, invisible to general Pass-B admission


**Scope.** `memory.cpp::makeNormalizedKeysForAdmission` (the ordis candidate loop + `ExpressionAnalyzer::ordisRouteQualifies`), `memory.cpp::installAdmissionMarkerVariants` (the shared install interior), `LocalMemoryValue::ordisOnly` / `AdmissionMapValue::ordisOnly` / `StagedAdmissionValue::ordisOnly` (the tag's carriage rule → firing → drained value), `prover.hpp::isAdmitted` / `prover.cpp::updateAdmissionMapRecursion` / `prover.hpp::applyEquivalenceClassToAdmissionMap` / `memory.hpp::admissionRunHasRegularValue` (the reader discipline), `memory.cpp::drainAdmissionKeysAlgebra` step (5) (the revival split).

**Rule.** A rule failing every regular admission gate — (A) classic, (B) long-key, (C) local-u — still installs marker rules iff every non-anchor element AND the head are operator applications and their count (head included, anchors excluded) reaches `ExecutionParameters::kOrdisMinOperatorExpressions` (= 4); candidates are variables at config INPUT slots of a premise, gated by presence at a config input slot of the head, confinement (the candidate appears in no other key element; the head is allowed), and concreteness (every other argument of the marked premise is bound by the subkey). Keys fired by such rules carry `ordisOnly == true` in the admission VALUE, end to end (LMV bit → staged value → drained blob). Every GENERAL admission consumer skips tagged values: `isAdmitted`'s run walk, `updateAdmissionMapRecursion`'s propagation, the D-243 Pass-B rendezvous (value-level via `admissionRunHasRegularValue`), and the drain's `revisitRejected2` step; the equi-class admission hook CARRIES the tag through its re-key. A key with both a regular and an ordis producer is regular — the untagged record simply coexists in the canonical run ("untagged wins" is emergent, never a flag rewrite). The only tagged-value reader is the ordis cohort probe, which reads tagged AND untagged and never writes (no `updateAdmissionMap`, no status write — the maps' one ordis touchpoint). The regular Writer-A route stays byte-identical; the two qualification routes are mutually exclusive.

**Why.** The cohort probe needs demand evidence from rules the regular gates reject (A15's corpus row 34 produces NO admission keys under (A)/(B)/(C)), but general witness generation must see nothing new — an untagged widening would re-open the very grids the admission gates exist to contain. No admission key is consumed at all any more ([I-200](#i-200)), so ordis demand keys stay live indefinitely (a case split's relevance is not a once-satisfied computation).

**How to spot.** A tagged value admitted by `isAdmitted` (new witness mints on a rule that fails the regular gates); a tagged-only key waking `revisitRejected2`; a rule qualifying through BOTH routes; an ordis probe that consumes (a key vanishing from `admissionMap` with no general admission in the burst).

**How to fix on violation.** Route every new general reader through the tag skip (or `admissionRunHasRegularValue` for key-level presence probes); keep the ordis route's installs behind `ordisOnly == true`; never let the probe call the consuming helpers.

**Code.** `memory.cpp::makeNormalizedKeysForAdmission`, `memory.cpp::ordisRouteQualifies` (definition), `prover.hpp::isAdmitted`, `memory.hpp::admissionRunHasRegularValue`. Tests: `tests/test_memory.cpp::ordis_route_qualifies_predicate`, `::ordis_route_installs_tagged_marker`, `::is_admitted_blind_to_ordis_only_runs`, `::admission_run_has_regular_value_probe`, `::apply_equi_admission_drops_and_rekeys` (the tag-carry case).

**See also.** [D-252](40_decisions.md), [I-178](#i-178), [I-71](#i-71), [I-41](#i-41), [I-89](#i-89), [I-90](#i-90).

<a id="i-178"></a>
## I-178 — `rejectedMapOrdis` obeys the full rejected-map discipline; revival is mail-based and re-runs the whole or consumption


**Scope.** `HashMemory::rejectedMapOrdis` (+ facets base+58..61) and `ordisRevisitInProgress` (base+62), the `RejectedMapOrdisValue` codec family (`memory.hpp`), the park site in `prover.cpp::disintegrateExprCore2`'s or arm, `prover.cpp::revisitRejectedOrdis` + `resetParkedOrStatementRegistries`, `prover.hpp::applyEquivalenceClassToRejectedMapOrdis`, `memory.cpp::wipeSubtree` (the `coldScopeWipe` enrollment).

**Rule.** The parked-cohort map mirrors `rejectedMap` in everything but producer, value record, and revival product: packed `(templateId, validityId)` keys in the admission key space (I-89), canonical-run RMW under a decoded comparator (I-99 — the RMW also dedups re-parks), deload facets in the HashMemory tag band (discharge survival inherited), `coldScopeWipe` enrollment, a mirrored dump section in the extended block, and the I-37 equi-class discipline (drop the class-hit keys, re-mail the REWRITTEN or statement with a fired-substitutions-only `equality1` line, never a direct insert). An or head parks iff it is a real deposit (`allowOrProbe`), no route admits it, and the cohort is within the depth gate — one entry per operator-based disjunct product template, value = the clean or statement + the seed level run (the key carries the validity). Revival (`revisitRejectedOrdis`) hooks EVERY admission key-gain seam, fires for tagged and untagged gains, holds the DEDICATED `ordisRevisitInProgress` guard, snapshots-then-erases the whole cohort, and re-deposits the parked statement on `sameIterationInternalMail` so the absorb re-runs the FULL or consumption — fresh probe over all standing demands, standing starter tie rule, K rules deduped, the fresh-cohort guard against double-open; a revival may legitimately re-park. Because the parked statement is a KNOWN local statement, each re-deposit is preceded by `resetParkedOrStatementRegistries` — the maintainer-approved un-know that erases its rows from `intStatementLevelsMap`, `intKnownStatements`, `intEncodedStatements`, and both local vectors (the documented exception to I-85's OR-only flag discipline, precedented by the I-58 CE-teardown reset; `intLocalEncodedStatementsSet` membership stays — a monotone over-approximation re-registered on re-absorb). Ordis reads never consume admission keys; revival exact-validity only (widening deferred by design, like the integration-side probe and negated disjuncts).

**Why.** Parking is a DECISION deferred, not a fact rejected — so revival must re-run the decision (probe → starter → bootstrap), which only the mail channel does uniformly; and the mirror discipline keeps the new map inside every lifecycle contract (wipe, deload, dump, equi-class) the existing rejected maps already satisfy, instead of minting a parallel set of rules.

**How to spot.** A parked cohort surviving its parent scope's wipe; a revival that inserts into `rejectedMapOrdis` directly; a re-deposit dropped at Site F (the un-know missing — the cohort silently never reopens); `revisitRejected2` firing on an ordis-only key gain; a parked entry whose statement rewrote under an equi class while its key survived.

**How to fix on violation.** Route the park through `insertRejectedOrdisIdsBlob` only, the revival through the mail re-deposit only; keep the un-know immediately before each re-deposit; enroll any new lifecycle sweep exactly where `rejectedMap` is enrolled.

**Code.** `prover.cpp::revisitRejectedOrdis`, `prover.cpp::resetParkedOrStatementRegistries`, `prover.cpp::disintegrateExprCore2` (or arm park), `prover.hpp::applyEquivalenceClassToRejectedMapOrdis`, `memory.hpp::insertRejectedOrdisIdsBlob`. Tests: `tests/test_rejected_ordis.cpp` (codec twins, run order, snapshot, scope erase, equi drop-and-mail, park-and-revive, route-(a) starter), `tests/test_lb_deload.cpp::memory_deload_reload_preserves_all_four_hashmemories` (the band round-trip).

**See also.** [D-252](40_decisions.md), [I-177](#i-177), [I-174](#i-174), [I-37](#i-37), [I-85](#i-85), [I-99](#i-99).

<a id="i-180"></a>
## I-180 — Equality-shaped registration only through the `addEquality` / `addNegatedEquality` gateways (mirror pair invariant)


**Scope.** `prover.cpp::addEquality` / `prover.cpp::addNegatedEquality` (the two gateways and their mirror-absence asserts), `prover.hpp::addStatement` (the kernel dispatch door that routes both shapes to their gateways), `prover.cpp::disintegrateExprHypothetically` (the hypothesis-scope constituent deposit — internal mail, no direct registration), and every other row writer (`prover.cpp::addExprToMemoryBlock` status-4 branch).

**Rule.** A statement of equality shape `(=[a,b])` or negated-equality shape `!(=[a,b])` gains its `intKnownStatements` row only through its gateway, which co-registers the mirrored orientation at the same validity in the same call (guarded by `args[0]!= args[1]`, I-9): original and mirror get their rows together or not at all. A writer holding statements of unconstrained shape does not register directly — it deposits on `sameIterationInternalMail` so the absorb runs the full kernel pipeline including the `addStatement` shape dispatch (`disintegrateExprHypothetically` does since the A16 fix, the same channel as the I-176 or-branch seeds and the I-178 ordis revival; each deposit pairs a terminal internal-only `hypothesis` origin, [D-259](40_decisions.md#d-259)). The remaining direct writers cannot see these shapes (the status-4 branch returns before the door; the full-disintegration marker creates no rows).

**Why.** Consumers reach equality-shaped facts in either orientation, so the gateways materialize both directions as one atomic pair; each gateway asserts mirror absence before registering it. A one-sided row breaks that assert on the later gateway arrival of the opposite orientation — the A16 shortcut crash: the hypothesis loop direct-registered `!(=[10,rec])` (a `strictOrder` definition constituent — `strictOrder` disintegrates into `preorder` plus a bare negated equality), and `addNegatedEquality(!(=[rec,10]))` at the same hypothesis scope then hit the registered mirror.

**How to spot.** `Assertion failed: negMirrorRow == nullptr` (or the `addEquality` twin) — the primary is absent while the mirror has a row, so some path registered one orientation directly; a new `upsertStatementKey` call site that accepts an unconstrained statement span without shape dispatch.

**How to fix on violation.** Deposit the writer's statements on `sameIterationInternalMail` (the I-176 or-seed channel) so the absorb's `addStatement` door does the shape dispatch; never register the mirror inline at the writer and never weaken the gateway asserts.

**Code.** `prover.cpp::disintegrateExprHypothetically` (the mail-deposit loop), `prover.cpp::addNegatedEquality`, `prover.cpp::addEquality`, `prover.hpp::addStatement`. Tests: no dedicated unit test yet — the A16 shortcut run is the acceptance case (flagged, not silent).

**See also.** [I-9](#i-9), [I-12](#i-12), [I-85](#i-85).

<a id="i-179"></a>
## I-179 — the negated-AND De-Morgan door mints nothing and feeds true-polarity leaves to the shared or downstream


**Scope.** `prover.cpp::disintegrateExprCore2` (the no-instruction negated-AND door), `prover.cpp::consumeOrLeavesCohort` (the extracted or downstream shared with the compiled-or arm), `prover.hpp` declaration.

**Rule.** The door fires only for `!(op[args])` whose compiled entity has category `and`, discovered through the read-only `compiledEntity` fence — no `compiledExpressions` / `coreExpressionMap` / counter write exists anywhere on the path (parallel-phase safe). Disjuncts are the definition elements instantiated with the instance arguments and negated via `negateScratch` — TRUE polarity, double negation cancels, never a blind `!` prefix. The cohort signature is the negated compound itself; the machinery keys cohorts on interned signature strings and never requires an or-operator name. The flat negated statement continues to register unchanged; `doNotDisintegrate` suppresses the door with the rest of the disintegration. `consumeOrLeavesCohort` writes only LB-local state (I-28) and behaves byte-identically to the former in-arm code for compiled or facts.

**Why.** Negated AND-bodied applications are disjunctions in De-Morgan clothing; consuming them through the one existing case-analysis engine (K rules for the premise-decided case, ordis cohort otherwise) avoids a second flattening mechanism and honors the no-precompilation constraint — a mid-run registry write from a worker is exactly the race [D-258](40_decisions.md#d-258) rejects.

**How to spot.** A negated compound of an AND-bodied operator sitting flat at `main` with no K rules beside it (door not firing); an `orN` mint triggered from a disintegration path (constraint violated); a doubled `!!` in a K-rule premise or a disjunct (polarity bug).

**How to fix on violation.** Route shape recognition through `compiledEntity` only; build leaves with `negateScratch`; keep all writes LB-local; never mint an operator for the twin — if a consumer genuinely needs a compiled or entity, that is the load-time-mint alternative and a maintainer decision.

**Code.** `prover.cpp::disintegrateExprCore2`, `prover.cpp::consumeOrLeavesCohort`. Tests: `tests/test_negated_and_demorgan.cpp` (K shapes, flat registration, `doNotDisintegrate` suppression, non-AND negations stay flat).

**See also.** [D-258](40_decisions.md#d-258), [I-166](#i-166), [I-167](#i-167), [I-172](#i-172), [I-174](#i-174), [I-175](#i-175), [I-177](#i-177), [I-178](#i-178).

<a id="i-181"></a>
## I-181 — a Pass-1 dead-scope skip always stages the branch it skips for; the probe is non-minting


**Scope.** `prover.hpp::refutedOrBranchAtOrAbove`, its one consumer `prover.hpp::applyEquiClasses` (Pass 1), and the retirement pair `ordisMerge` / `drainDeadOrBranches`.

**Rule.** `applyEquiClasses` Pass 1 may skip a delta class ONLY on a nonzero `refutedOrBranchAtOrAbove` verdict, and that helper returns nonzero ONLY when (a) a chain scope is already staged in `pendingDeadOrBranches`, or (b) an `_ordis_` disintegration scope on the chain satisfies the `ordisMerge` staging predicate ([D-242](40_decisions.md#d-242): negate(asserted disjunct) `known` at the branch scope or an ancestor) — in which case the helper MUST mint that branch into `pendingDeadOrBranches` before returning. A skip without a staged branch would leave the doomed scope alive with its equi-class application suppressed — silently diverging state; the staging guarantees the SAME burst's `drainDeadOrBranches` retires the subtree, so skipped content is content the burst wipes anyway (ex falso). Every probe on the path is non-minting (`NameMap::lookup`, `intKnownStatements` reads, `parentOf` walks); the staging mint is the single write, and the drain's record step re-runs the predicate and asserts a hit, so a staged id always satisfies it.

**Why.** The skip is sound only as a *retirement accelerator*: the branch was provably dead before the apply began (the refutation absorbs pre-`equiApply` in the same `standardProcessing` call), and the drain wipes it at end of burst. Decoupling the skip from the staging would turn a work-avoidance gate into a semantics change.

**How to spot.** A `continue` in Pass 1 gated on anything but the helper; a helper return path that found the predicate true but did not mint; `pendingDeadOrBranches` entries the drain's record walk cannot re-prove (its assert fires).

**How to fix on violation.** Route every Pass-1 skip through the helper; keep the mint on the hit path; never widen the helper's verdict beyond the D-242 predicate without a new decision.

**Code.** `prover.hpp::refutedOrBranchAtOrAbove`, `prover.hpp::applyEquiClasses`. Tests: `tests/test_memory.cpp::refuted_or_branch_at_or_above_probes_and_stages`.

**See also.** [D-262](40_decisions.md#d-262), [I-172](#i-172), [I-174](#i-174), [I-65](#i-65).

<a id="i-186"></a>
## I-186 — the WorkInstruction entity table is a function signature → structure


**Scope.** `prover.hpp::prepareIntegrationCore` (the sole production entity writer), every table consumer that resolves a child by signature scan (`prover.cpp::flattenOrLeaves`, `prover.cpp::disintegrateExprCore2`'s `hit`/`innerHit`, `memory.hpp::cleanInstructionWork`, `prover.hpp::prepareIntegrationCore2`'s nested-OR marking).

**Rule.** `prepareIntegrationCore` commits at most ONE entity per distinct instantiated signature: the signature is built from the `u_`-argument pairs alone (template signatures carry only `u_` arguments; `replaceKeysScratch` is token-boundary, so the bound-variable pairs can never rewrite it) and probed against the already-committed entities BEFORE the fresh-`pi_lev_` mint; a hit returns without minting, committing, or recursing. Consequently the table is the unfolded definition GRAPH — one node per distinct sub-entity — and first-match signature resolution is well-defined. `flattenOrLeaves`' duplicate-identity asserts remain as the tripwire (I-19: never remove).

**Why.** A repeated compound element (first reachable when an equivalence-class collapse under an assumed equality produces a degenerate or instance like `(or6[…,X,X])` — [D-106](40_decisions.md#d-106) sanctions the repetition) used to re-unfold per occurrence, and an existence-category template mints a fresh `pi_lev_` placeholder per unfold — two entities with one signature and DIVERGENT element lists, an ambiguous table that aborted the B8 acceptance run on the flattenOrLeaves assert. The probe must sit before the `pi_` mint: probing after would burn `startIntPi` ids on skipped entities, and per-occurrence placeholder divergence is the defect itself.

**How to spot.** Two entities with equal signatures in any instruction table (the flattenOrLeaves asserts fire); `startIntPi` gaps that track duplicate element occurrences; a new entity writer that bypasses the probe.

**How to fix on violation.** Route every entity commit through the probe; never resolve the assert by weakening it or by tolerating divergent duplicates.

**Code.** `prover.hpp::prepareIntegrationCore`, `prover.cpp::flattenOrLeaves`. Tests: `tests/test_memory.cpp::prepare_integration_core_dedups_duplicate_signature`.

**See also.** [D-270](40_decisions.md#d-270), [I-166](#i-166), [D-106](40_decisions.md#d-106).

<a id="i-185"></a>
## I-185 — before any burst, every registered or's single-elimination reduced closure is registered


**Scope.** `prover.cpp::preMintReducedOrs` (the closure writer), its three call seams (`proveKernel` entry, `constructOrTheoremsInRun` tail, the post-compaction-drain barrier site), and every consumer that resolves a reduced (k−1)-ary or-operator by element list (`prover.hpp::compiledOrByElements`).

**Rule.** At every point where a burst can flat-consume an or statement, the compile registry already holds, for every or-category entry with k ≥ 3 flattened leaves, all k of its leaves-minus-one reduced operators (u_-renumbered canonical form, recursively to single-elimination closure). Reduced-or minting happens ONLY at single-threaded seams through `findOrMintOrOperator` ([I-137](#i-137) discipline; [I-23](#i-23) dedup) — never mid-burst. Consumers therefore treat a `compiledOrByElements` miss for a reduced leaf list as a hard assert, not a defined miss.

**Why.** The single-exclusion rule family (Part 1 of the B8 three-extension design) emits `!D_i -> or(D_rest)` with a genuine (k−1)-ary or-compact head from inside the parallel burst, where registry writes are forbidden. The closure guarantee moves the mint to the legal seams and makes the emission site allocation-free and assert-guarded.

**How to spot.** The emission-site assert on a reduced-or lookup miss; an or-category entry whose reductions are absent after a barrier (dump the registry and diff against the closure).

**How to fix on violation.** Add the missing seam call / fix the seam ordering — never mint at the consumer and never soften the assert.

**Code.** `prover.cpp::preMintReducedOrs`, `prover.hpp::compiledOrByElements`. Tests: `tests/test_or_theorem_construction.cpp::premint_three_ary_closure_and_idempotence` / `premint_four_ary_worklist_closure` / `premint_nested_or_flattens_and_reuses_child`.

**See also.** [D-268](40_decisions.md#d-268), [I-23](#i-23), [I-137](#i-137), [I-166](#i-166).

<a id="i-184"></a>
## I-184 — a head fired by a subset-exclusion rule (j = 1..k−2 negated disjuncts) never opens a cohort via route (b); it parks


**Scope.** `memory.cpp::isSubsetExclusionInstall` (the shape detector), `memory.cpp::addToHashMemory` (the head-install derivation, hoisted above the permutation loop — both shape detectors are permutation-invariant), `memory.cpp::checkLocalEncodedMemoryStatic` (the firing-site strip), `memory.hpp::LocalMemoryValue::subsetExclusion` + its codec / `appendLmvIdsRecord` / `LmvBlobView` layout family (byte @20, appended last), `prover.hpp::forEachCompiledOr` (the registry scan fence).

**Rule.** A rule with the subset-exclusion shape — j premises (1 ≤ j ≤ k−2) whose negations (double-negation cancelling, I-175) together with the or-category head's instance-substituted flattened leaves account for SOME registered or's flattened leaf list — carries `subsetExclusion = true`, detected BY SHAPE at every install (the D-241 re-detection discipline, so status-3 mail-recovered reinstalls re-mark). Detection is ORDER-FREE in the premises: the mail-compact round-trip stores only the first-seen body and dedups whole name-sorted permutation families (I-52), so a reinstalled rule's premises may arrive permuted — the detector runs ONE registry scan per install, matching each same-flat-size or entry by order-preserving subsequence embedding of the head leaves plus multiset-match of the premise negations under a consistent INJECTIVE signature-token-to-arg binding (injectivity keeps the degenerate repeated-arg instance untagged; flatten-based matching on both sides follows the I-166 flattened-cohort discipline, so nested parents tag too). At firing, `FiringRecord::allowOrDisintegration = productOfDisintegration && !subsetExclusion` — the head keeps flat consumption (`allowOrProbe` untouched, K rules still emit) but never carries the route-(b) open signal, so the fired reduced or PARKS in `rejectedMapOrdis` (I-178). Opening comes ONLY from the compound-demand admission map (Part 2 of the B8 design).

**Why.** The subset-exclusion rule is itself a disintegration product, so without the strip its fired or-head would ride route (b) and open a case split wherever negative premises exist — uncontrolled scope spawning, exactly what the D-32 demand discipline prevents (the maintainer's load-bearing park-first decision). Parts 1 and 2 are jointly the closing set; neither suffices alone. An order-sensitive detector would silently untag exactly the mail-recovered reinstalls the D-241 discipline exists for.

**How to spot.** `_ordis_` cohorts appearing without a matching demand entry whose signature is a reduced or; a `subsetExclusion` LMV whose fired record still carries `allowOrDisintegration`; the `kSubsetExclusionEmbedCap` tripwire assert (a contract break in the embed walk, never a tuning knob).

**How to fix on violation.** Restore the strip at the firing site; never widen route (b) for tagged rules and never move the tag off the shape-detection discipline (an emission-side-only tag misses mail-recovered reinstalls); never make the detector order-sensitive.

**Code.** `memory.cpp::isSubsetExclusionInstall` / `addToHashMemory` / `checkLocalEncodedMemoryStatic`; `prover.hpp::forEachCompiledOr`; `parameters.hpp::kSubsetExclusionEmbedCap`. Tests: `tests/test_disintegration_allowed.cpp::single_exclusion_shape_detected` / `single_exclusion_instance_args_canonicalize` / `subset_exclusion_two_premises_order_free`; `tests/test_or_theorem_construction.cpp::for_each_compiled_or_visits_and_stops`; `tests/test_memory.cpp::lmv_blob_view_matches_codec`.

**See also.** [D-268](40_decisions.md#d-268), [I-185](#i-185), [I-177](#i-177), [I-178](#i-178), [D-32](40_decisions.md#d-32).

<a id="i-182"></a>
## I-182 — a statement levels row is never empty; {-1} is the non-derived tier, transparent to level accounting


**Scope.** `intStatementLevelsMap` and every writer/reader of statement level runs: the `addStatement` door (`prover.hpp`), the equality doors (`addEquality` / `addNegatedEquality`), the status-4 fact load and the full-disintegration marker (`prover.cpp::addExprToMemoryBlock`), `prehandleAnchor`, the compressor loads, the union sites (`insertLevelSorted`, `coldIntRunNonNegAt`, the firing union in `memory.cpp::checkLocalEncodedMemoryStatic`, `ordisMerge`'s refutation union, `dischargeContradictionScopes`), the `allLevelsInvolved` discharge arithmetic (`dischargeToBeProved`), and `applyEquivalenceClass`'s depth-eligibility gate.

**Rule.** Four clauses. (1) A row in `intStatementLevelsMap` is NEVER empty: a statement arriving with no level provenance gets the singleton `{-1}` — the non-derived tier (loaded facts, anchors, broadcast theorem compacts, assumed premise elements, compressor premises) — substituted at the `addStatement` door; the direct non-derived writers pass `{-1}` explicitly. (2) A run is either exactly `{-1}` or all-values-≥0, never mixed (door asserts). (3) `-1` is transparent to level accounting: `insertLevelSorted` skips negatives, derivation unions read rows through `coldIntRunNonNegAt`, and the `allLevelsInvolved` arithmetic sees a `{-1}` row exactly as the former empty run — an all-non-derived union comes back empty and re-enters as `{-1}` at the door. (4) Depth comparisons treat a pure `{-1}` source as ground: rule installs (`addToHashMemory`) and class registrations (`updateEquivalenceClasses`) map `{-1}` to their empty-set conventions — the tier never enters hash memory or class pair-levels. (The `applyEquivalenceClass` commit's level-exact condition, which read the tier as ground via `effectiveMaxLevel`, is removed — D-320: the orbit commit's sole admission control is the not-yet-known probe.)

**Why.** The B8 known-without-levels crash: level-row presence doubled as an admission verdict, so writers deliberately withheld rows and the vacuous full-disintegration marker promoted row-less statements to `known`. Making every row present and non-empty (with `{-1}` distinguishing "deliberately no derivation depth" from "forgot to write") removes the empty/missing ambiguity that hid the defect class; transparency keeps the discharge arithmetic and every union verdict-equivalent to the former empty-run behavior.

**How to spot.** The door asserts (`addStatement` / `addEquality` / `addNegatedEquality` / status-4); the reader asserts upgraded to `runLen > 0` (ordisMerge, dischargeContradictionScopes, dischargeToBeProved, applyEquiClasses, fillMailOut); a mixed run anywhere is the singleton-clause assert.

**How to fix on violation.** Fix the producer that shipped an empty or mixed run — never widen a reader to tolerate it and never strip `-1` at a row write (only union reads filter).

**Code.** `memory.hpp::insertLevelSorted` / `coldIntRunNonNegAt`; `prover.hpp::addStatement` (the door). Tests: `tests/test_memory.cpp::insert_level_sorted_skips_non_derived_tier` / `cold_int_run_non_neg_at_filters_negative_tier` / `add_statement_empty_run_becomes_non_derived_tier` / `marker_never_creates_known_without_levels`.

**See also.** [D-264](40_decisions.md#d-264), [I-136](#i-136), [I-58](#i-58), [I-85](#i-85), [I-51](#i-51), [I-72](#i-72).

<a id="i-183"></a>
## I-183 — the ordis2 demand/park pair shares ONE key language; every parked cohort dual-files; only the ordis2 drain wakes the second index


**Scope.** `admissionMapOrdis2` / `rejectedMapOrdis2` (`memory_infra/hash_memory.hpp`), the park site (`prover.cpp::consumeOrLeavesCohort` step 4), the wake (`prover.cpp::revisitRejectedOrdis2`), the demand drain (`memory.cpp::drainAdmissionKeysOrdis2`), the route-(c) probe, the shared predicate (`memory.cpp::ordis2KeyEligible`), and both equi hooks (`prover.hpp::applyEquivalenceClassToAdmissionMapOrdis2` / `applyEquivalenceClassToRejectedMapOrdis2`).

**Rule.** Six clauses. (1) Both maps key on packed `(templateId, validityId)` whose template half is a clean GROUND compound text with polarity verbatim (I-175), admitted to the language only by `ordis2KeyEligible` (compiled non-atomic entity — the `compiledEntity` fence alone, which admits PREDICATES like `preorder`/`strictOrder`; the input-AND-output `operators` set is deliberately NOT consulted — plus arity ≥ `kOrdis2DemandMinArity`); the DEMAND side additionally requires ≥ `kOrdis2DemandMinPremises` non-anchor premises on the installing rule (slot included) and refuses to stage a ground text whose maximum iteration number exceeds `maxIterationNumberVariable` (the I-171 mirror) — the ONE predicate both the demand slot filter and the park filing consult. (2) Every cohort that parks in `rejectedMapOrdis` ALSO files in `rejectedMapOrdis2` under each eligible disjunct's clean text at the cohort parent's validity; the old filing is byte-untouched (I-178). (3) ONLY `drainAdmissionKeysOrdis2` wakes `rejectedMapOrdis2` (via `revisitRejectedOrdis2`, the I-178-discipline twin with its own `ordis2RevisitInProgress` guard); the algebra key-gain seams never cross-probe — their key language differs, and for operator-application disjuncts the old map's product template IS the disjunct text, so a cross-probe would double-wake. (4) Both arrival orders close: or-first parks then the drain's wake re-runs the consumption whose route (c) opens; demand-first opens at deposit time because the route probe precedes the park in `consumeOrLeavesCohort`'s chain. (5) Opening consumes the matched demand keys (`eraseBlobIf`); stale park entries under other keys are tolerated (bootstrap-guard idempotence + scope wipe), and re-parks come only through the fresh-failure path (RMW dedup collapses). (6) Route (c) reads and writes NO algebra admission entry, so a route-(c) branch's asserted disjunct is admitted as the COMPACT statement only — no interior admission entry demands its existence body, the witness never materializes, and the I-6 fan-out is structurally dodged (the recorded Part-2 branch-side semantics).

**Why.** The 3X wake failed on a key-language mismatch (demand text probed against product-template keys — structurally impossible to hit for existence-bodied operators). One shared language makes the rendezvous a plain key hit and makes the new park immune to the suspected or8/or10 consumption path (algebra key-gain wakes through shared witness-template keys touch only the old map).

**How to spot.** A demand that never opens a parked cohort with a byte-equal disjunct (dump: both sections populated, key texts equal, no wake) — check the validity halves; a park filing whose text fails `ordis2KeyEligible` (the filing loop's gate was bypassed); a wake from any seam other than the drain.

**How to fix on violation.** Fix the seam that diverged — never widen the eligibility predicate asymmetrically on one side, and never add a cross-probe from the algebra seams.

**Code.** As in Scope. Tests: `tests/test_rejected_ordis.cpp::ordis2.*` / `rejected_ordis2.*` / `prover.ordis2_park_files_disjunct_texts` / `prover.revisit_rejected_ordis2_wakes_erases_and_unknows`.

**See also.** [D-267](40_decisions.md#d-267), [I-178](#i-178), [I-177](#i-177), [I-174](#i-174), [I-99](#i-99), [I-89](#i-89).

<a id="i-189"></a>
## I-189 — Lean export remains ordinary-type, acyclic, row-total, and admission-free


**Scope.** `proof_export/certificate.py`, `proof_export/lean.py`,
the fixture certificates under `tests/fixtures/lean_certificates/`,
`lean_export/GLExport/ProofSupport.lean`, and the manifests each run writes
into its own `lean_export/` folder.

**Rule.** The Lean renderer consumes only typed schema-2 or schema-3 certificates
after the selected prior-theorem graph has been alpha-resolved, topologically
ordered, and asserted acyclic. Schema 3 pins every dependency certificate by
hash and asserts the combined graph acyclic. The encoded object theory has one
ordinary carrier `α`;
sets and operations are predicate relations over it, never term-indexed Lean
types. Relational induction is an explicit public theorem premise. Every selected
certificate row receives exactly one `status = emitted` manifest disposition.
Generated source contains no `sorry`, `admit`, `axiom`, `opaque`, or `unsafe`
shortcut. Full-corpus generation asserts 65 theorems and excludes exactly cyclic
sources 24 and 25; Gauss generation asserts all 29 sources 67 through 95 and
1,511 rows. Integration scopes close their internal parameters with ordinary
quantifiers. A reformulated defined output is witnessed by a concrete predicate
comprehension or relation lambda whose non-recursive defining predicate is
proved; it is never postulated by an existence axiom or represented by a
dependent GL type family.

**Why.** This separates two trust questions. Lean's kernel checks the generated
proof terms, while the neutral certificate retains the auditable mapping to GL
source rows. Ordinary predicates avoid smuggling GL typing judgments into Lean's
type formation, and the explicit induction premise keeps the one open Peano
principle visible instead of hiding it in an axiom declaration.

**How to spot.** A generated dependent family indexed by a GL value; an `axiom`
or admitted declaration; a manifest count below the certificate row count; a
forward or cyclic theorem reference; source 24 or 25 appearing as emitted.
For schema 3, also: a dependency hash mismatch; a free integration variable; a
reformulated existence with no validated totality witness; or any Gauss source
67 through 95 missing from the manifest.

**How to fix on violation.** Fix the certificate or renderer contract and add a
direct test, then rerun the full `lake build`. Never assume a cyclic theorem,
hide induction in the anchor, or weaken the trust-token scan.

**Code.** `proof_export/lean.py::render_definitions`,
`proof_export/lean.py::render_peano_theory`,
`proof_export/lean.py::render_gauss_theory`,
`proof_export/lean.py::write_lean`, and
`tests/test_proof_export_lean.py::LeanFullCertificateTests` /
`LeanGaussCertificateTests`.

**See also.**
[D-275](40_decisions.md#d-275) and
[`10_pipeline/10_external_proof_export.md`](10_pipeline/10_external_proof_export.md).

---

<a id="i-187"></a>
## I-187 — every statement-registration door refuses a deposit whose expression is known at a strict ancestor scope; refusals keep the I-174 resolution signal


**Scope.** The shared predicate `gl::ancestorKnown` ([`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp)) and its five consumers: Site F at [`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp) `addExprToMemoryBlock` entry (self-inclusive), the firing-record mirror in [`memory.cpp`](../GL_Quick_VS/GL_Quick/src/memory.cpp) `checkLocalEncodedMemoryStatic` (self-inclusive), and the three registration-door gates (strict-ancestors mode): `addStatement` (covers the `addEquality` / `addNegatedEquality` pair doors per I-180's sole-gateway rule), `applyEquivalenceClass`'s commit block, and the negated-equality expander's `emitNew` gate. Plus the refusal companion `stageOrReleaseForRefusedDeposit` ([`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp)), the end-of-burst sweep `sweepAncestorKnownRows` ([`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp), last step of `performElemPhase3`'s end-of-burst block), and the waterline repair `repairEqClassWaterlines` ([`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp)) inside `removeExpressionFromMemoryBlock`.

**Rule.** Six clauses. (1) An expression already known at the deposit scope's strict ancestor is never registered at the descendant scope — no statement rows, no levels row, no known row, no origin history line, no `newStatements` out-param row, no per-statement follow-up (admission update, `ordisMerge`). The ancestor row survives; when both exist (legacy state), the descendant copy is the one removed. (2) The scan is `ancestorKnown` — ONE definition, non-minting probes, `includeSelf` selecting Site F semantics (self + strict ancestors) versus door semantics (strict only; exact-scope handling stays with each door's pre-existing gate). (3) Every refusal calls `stageOrReleaseForRefusedDeposit`: if the refused scope is an `_ordis_` branch with a live pending cohort and the refused expression matches an open `toBeProved` goal on the branch's parent chain, `pendingOrReleases` is minted — the branch derived the expression, so the I-174 resolution signal survives the refusal. (4) All gates skip in `compressor_mode` (Site F parity). (5) Equality shapes refuse as a PAIR (the pair invariant holds at the ancestor too); no shape is exempt — the negated-equality scope-symmetry change (D-271) is the enabler that made ancestor rows fully substitutable for descendant copies. (6) The branch-first residue (ancestor row arriving after a legitimately registered descendant copy — undetectable at deposit time, `NameMap` has no child index) is closed by `sweepAncestorKnownRows` at the end-of-burst seam: statement-LIST rows only, decoded-lex removal order, registry rows kept as tombstones (I-58 / I-85), `mutatedThisBurst` raised on any drop (D-194), and every `intEncodedStatements` erasure repairs the persistent per-class waterlines (`repairEqClassWaterlines` inside the removal door — a stale waterline silently under-applies classes; the repair also covers `ordisMerge`'s pre-existing collapse removals).

**Why.** The B5/B7/B8 induction sub-LB blow-up (`docs/fta_ladder/B_RT_investigation.md`): 35 % of equi-class commits and 17 % of negated-equality registrations were ancestor-known duplicates, 62 % of a monster LB's statement list was `_ordis_` branch copies, and the grow search is super-linear in the statement universe. The contract existed (Site F) but only at one door; the trap run (2026-08-11) attributed the violations to the exact-key-gated doors this invariant now covers.

**How to spot.** A statement row at a scope whose strict ancestor holds the same `originalId` in `intKnownStatements` (the end-of-burst sweep's detection predicate); a new registration path that writes `intStatementLevelsMap` + `upsertStatementKey` without the `ancestorKnown` gate.

**How to fix on violation.** Route the writer through `addStatement` or add the gate + refusal companion; never exempt a shape — fix the consumer that made the descendant copy load-bearing instead (the D-271 precedent).

**Code.** As in Scope. Tests: `tests/test_equi_reshuffle.cpp::equi_reshuffle.ancestor_known_predicate_directions` / `add_statement_refuses_ancestor_known_descendant_copy` / `add_statement_refuses_ancestor_known_equality_pair` / `apply_equi_classes_refuses_ancestor_known_product` / `apply_equi_classes_descendant_rewrite_still_deposits`; `tests/test_rejected_ordis.cpp::prover.refused_ancestor_known_deposit_still_stages_or_release`.

**See also.** [I-27](#i-27), [I-174](#i-174), [I-180](#i-180), [I-85](#i-85), D-273 / D-271 in [40_decisions.md](40_decisions.md), [I-206](#i-206) (the freeze predicate probes `ancestorKnown` self-inclusively, so a goal deposit refused as ancestor-known still counts as reached).

<a id="i-188"></a>
## I-188 — every LB under the batch anchor mints its axed x-names; the containment door is active grid-wide, recursion subtrees included


**Scope.** `ExpressionAnalyzer::prehandleAnchor` ([`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp)) — the sole `intAxedVariables` writer — and the THREE axed-containment checks (each an argument-slot scan over `intAxedVariables`; the D-234 positive-anchor exception is the one config-gated pass at the two statement doors): the `addExprToMemoryBlock` prologue, the `addStatement` door (covers disintegration products and equalities that never re-enter the prologue), and the `applyEquivalenceClass` orbit-commit refusal (the door-bypassing registration path; anchors are never orbit products per I-53, so it carries no exemption arm). All three are set-keyed, so they are naturally inert in a mode-2 subtree's empty set.

**Rule.** Four clauses. (1) `prehandleAnchor`'s walk reaches every LB from the grid root; outside recursion subtrees every touched LB mints its axed names AND registers its x-substituted anchor form + `anchor handling` history line. (2) A recursion subtree takes one of two modes, decided at its root and threaded down (`axedMode` parameter — only subtree roots carry `isPartOfRecursion`): SET-ONLY (mode 1 — axed names minted, no statement) for block-#2 `_induction_` side-chains always and for any block-#1 root whose goals cite no `(1)`-typed anchor slot value; the ANCHOR-NUMERAL EXCEPTION (mode 2 — subtree entirely untouched: no mint, no registration) for a block-#1 root whose goal (the theorem head, queued verbatim at creation) cites a `(1)`-typed slot value as a whole token — there the ancestor's mailed axed-anchor statement registers at the inert door and the historical premise-completion machinery runs (anchor-numeral theorems need it: the unit-product row B3 un-proves without it, and its pre-fix chapters cite zero x-content — the machinery is search-dynamics scaffolding, so a statement-with-armed-door middle ground is provably near-inert). (3) The trace accumulates over the whole ancestor chain (each LB's own exprKey included), so a set-only descendant's axed set is a SUPERSET of its ancestors' — the mailed ancestor-form axed anchor is refused at its door instead of re-registering. (4) Set-only subtrees are entirely x-free — no x-citing statement, rule, or firing exists there; outside recursion subtrees, x-citing content beyond the directly-registered anchor forms exists nowhere.

**Why.** The x-copy leak (`docs/fta_ladder/B_RT_investigation.md`): the former recursion early return (above both the mint and the child recursion) left induction subtrees with empty axed sets; the parent's x-anchor rode statement mail in, registered at the inert door (trap run 2026-08-11: exactly ONE external status-3 arrival, all 704 further x6 registrations internal implication firings), and bred 553/7 700 statements + 282/1 884 rules citing `x6` in B5's rec2 sub-LB — ~7 %/15 % dead weight in the monster induction blocks, exported onward via `mailOut`.

**How to spot.** A statement or rule citing an x-name anywhere beyond the LB's own registered axed-anchor form(s); an `-- intAxedVariables (0):` hashburst-dump section on an LB whose ancestor chain cites a `(1)`-slot value; a new LB-creation path whose blocks the `prehandleAnchor` walk never reaches.

**How to fix on violation.** Find the registration path that bypassed the door or the walk gap that left the set empty; never exempt a shape from the door.

**Code.** As in Scope. Tests: `tests/test_axed_anchor_exception.cpp::axed_anchor_exception.prehandle_covers_recursion_lb_and_subtree` / `prehandle_registers_statement_outside_recursion` / `door_armed_on_recursion_lb_after_prehandle` (plus the pre-existing door/exception cases in the same file).

**See also.** [I-27](#i-27) (Site F, runs before the axed check), [I-36](#i-36) (the element-vs-anchor-slot collision family the x-copies exist for), D-234 / D-236 in [40_decisions.md](40_decisions.md), D-272 in [40_decisions.md](40_decisions.md).

<a id="i-190"></a>
## I-190 — the inter-chapter theorem-usage graph is acyclic


**Scope.** The chapter exporter (`ExpressionAnalyzer::buildStack` + `generateRawProofGraph` in [`visualizer.cpp`](../GL_Quick_VS/GL_Quick/src/visualizer.cpp)) on the producer side; the `theorem usage termination` meta-check (`collect_theorem_usage_citations` / `check_theorem_usage_termination` in [`verifier.py`](../verifier.py)) on the read side.

**Rule.** The directed graph whose nodes are the chapter-owning theorems of a proof graph and whose edge A → B means "the chapter of A cites B" (via a `theorem` row or a non-locally-originated citation cell) contains no cycle — including no self-edge. Per-chapter origin acyclicity (D-51 path filter, `origin chain termination`) plus this invariant give global derivation acyclicity, because chapters connect only through theorem-citation foundation leaves.

**Why.** A `theorem`-tagged origin carries an empty dependency vector, so within any single chapter a used theorem is ground truth; two chapters can therefore justify each other with every per-chapter check passing while neither has an actual grounding (the mirrored Peano pair, sources 24/25 — each chapter was one application of the other, and one of the two shipped in `theorems.txt`).

**How to spot.** `theorem usage termination` reports non-zero failure; D-275-style export tooling hits a cycle assert when ordering theorems.

**How to fix on violation.** Producer side: the export-time admissibility check must have been bypassed or a new citation channel escaped `TheoremUsageGraph` edge commitment — find it; never soften the verifier check. If a chapter has NO acyclic origin for its head (degraded last-resort emit), the fix is prover-side provenance for at least one cycle member, a Rule-8 decision.

**Code.** As in Scope. Tests: `tests/test_verifier_usage_graph.py` (resolver, edge extraction, cycle detection).

**See also.** [D-276](40_decisions.md#d-276), [I-67](#i-67), [I-16](#i-16), D-51 in [40_decisions.md](40_decisions.md).

<a id="i-192"></a>
## I-192 — goal lifecycle is level-free; only global registration is level-gated


**Scope.** `prover.hpp::dischargeToBeProved` (non-recursion MAIN branch), `prover.hpp::dischargeContradiction` (primed branch), `prover.cpp::updateGlobalDirect` / `drainUpdateGlobalDirect`, and `prover.cpp::updateGlobal` (the recursion route, which always had this shape).

**Rule.** A MAIN goal whose head is proved at the exact goal key CLOSES regardless of premise-level coverage: goal-row erase, goal-scope wipes, twin retirement, and the I-48 deactivation cascade never read the levels map. ONLY the global-list registration (`appendGlobalTheorem`, reached through `updateGlobalDirect`'s registration section or `updateGlobal`'s promotion) is gated by the `allLevelsInvolved` verdict, sealed at the discharge site into `UpdateGlobalDirectRec` — normal route: the head's own level row; twin route: the colliding pair's level-row union excluding the twin's own level (the assumed seed's tier). The D-240 settled-goal seed deposit runs for every non-vacuous primed record, refused or not.

**Why.** Two individually-correct disciplines composed badly (`docs/fta_ladder/B10_RT_investigation.md` §7): the level condition exists for artifact fidelity — an emitted implication must have consumed every premise level — but as a discharge gate it kept a level-poor proved goal open forever, holding its LB chain active to batch end (the B10 4-undead-block chain), while the level-blind twin route emitted the theorem anyway. Decoupled, the goal closes (the LB may die) and a level-poor closure emits NOTHING; an over-premised conjecture proves-but-does-not-register BY DESIGN — the narrower theorem is the conjecturer's to restate.

**How to spot.** A theorem head present in a goal LB's `intEncodedStatements` at main, the goal row closed, and the theorem missing from `theorems.txt` = a level-refused registration (designed when the conjecture is over-premised — shortcut pool row `(>[9](in[9,1])(>[10](=[9,10])!(strictOrder[1,4,9,10])))` under AnchorFTA is the standing reproduction case). An LB chain active at batch end held by an unclosed PROVED goal = a violation.

**How to fix on violation.** Find the lifecycle step that grew a levels-map read, or the registration surface that escaped the verdict; never re-couple closure to the verdict.

**Code.** As in Scope. Tests: `tests/test_memory.cpp::prover.level_poor_goal_closes_without_registering` / `prover.level_complete_goal_seals_registration_verdict` / `prover.contradiction_twin_seal_carries_level_verdict`.

**See also.** [D-278](40_decisions.md#d-278), D-279 in [40_decisions.md](40_decisions.md), [I-48](#i-48), [I-51](#i-51), [I-165](#i-165).

<a id="i-191"></a>
## I-191 — the burst early-exit stops every part at the doom line's deterministic position; the finalize merges the winning part's chain alone


**Scope.** `prover.hpp::BurstSink::canAccept` / `::consume`, the `packDoomLine` / `doomLinePosition` / `doomLineOrdinal` / `lowerDoomLine` helpers (`prover.hpp`), `prover.cpp::performElem2` (threads the line + part ordinal), `prover.cpp::proveKernel` (`doomLines[]`, external to the LBs), `prover.cpp::performElemPhase2` (the winner-only merge).

**Rule.** The burst early-exit runs for EVERY part, split or unsplit. When `burstDeactivates` fires on a part's own freshly-appended head, the part publishes `(its own submatch counter, its invocation ordinal)` — packed as `(position << kDoomLineOrdinalBits) | ordinal` — onto the LB's shared external doom line by atomic CAS-min, sets `selfStopped`, and ends its burst. Every other part stops once its OWN submatch counter lies STRICTLY past the line's position (a position tie may still publish, and the earlier ordinal wins the CAS-min). At the finalize, a set line makes `performElemPhase2` hand `applyFiringRecords` ONLY the winning part's chain — the part named by `doomLineOrdinal` — and the other parts' chains are freed unread; the sentinel `kNoDoomLine` merges every part as before. The line, the winner, and the doom lines vector live OUTSIDE the LB (the `stopFlags` precedent, [D-121](40_decisions.md#d-121)), so phase 2 stays read-only on the LB ([I-66](#i-66) / [I-83](#i-83)).

**Why.** This is the determinism-safe form of the split early-exit [I-76](#i-76) forbade. The winner — the lexicographic-minimum `(position, ordinal)` trigger — is a pure function of the parts' deterministic streams: a part can never be cut before its own trigger, because the line's position never drops below the winning position and the bail is strictly-past-position; near-simultaneous triggers both publish and CAS-min keeps the minimum regardless of wall-clock arrival. The winner's chain is its deterministic stream prefix up to its trigger. The losing parts' chains END wherever the line caught them — timing-dependent — which is exactly why they are discarded rather than merged; a doom trigger mirrors the phase-3 discharge exactly (it never over-exits, [D-222](40_decisions.md#d-222)), so the LB discharges regardless and every culled sibling record is wasted-once-doomed. Thread timing therefore shapes only wall-clock, never output. The B10 RT investigation's burst 26 (16.6 s across 32 buckets vs a 0.77 s single-part control) is the measured waste this recovers.

**How to spot.** Two same-config runs differing in `proved_theorems` / `global_theorem_list` / the proof graph; the merge reading a losing part's chain when the line is set; a part bailing at `counter == position` (must be strictly past — kills position-tie winners); the line reset between a part's run and its LB's finalize; a winner ordinal `>= partCount` (the `performElemPhase2` assert).

**How to fix on violation.** Keep the three legs together: (1) publish = CAS-min of the packed own-counter/own-ordinal pair, (2) bail = own counter strictly past the line's position, (3) merge = winner's chain alone when the line is set. Any leg alone reintroduces either the I-76 race or a completeness hole.

**Code.** As in Scope. Tests: `tests/test_lb_split.cpp::doom_line_packing_and_cas_min` / `burst_sink_stops_on_the_doom_line_for_every_part` / `phase2_finalize_merges_only_the_winner_on_a_doom_line`.

**See also.** [D-282](40_decisions.md#d-282), [I-76](#i-76) (superseded predecessor), [I-159](#i-159), [D-121](40_decisions.md#d-121), [D-201](40_decisions.md#d-201), [I-135](#i-135) (dropping a chain unread), [I-77](#i-77) (the merge sort is set-determined).

<a id="i-193"></a>
## I-193 — derivatives of anchored actual rules retain the anchor context


**Scope.** `ExpressionAnalyzer::makeNormalizedKeysForAdmission` and `installAdmissionMarkerVariants` in [`memory.cpp`](../GL_Quick_VS/GL_Quick/src/memory.cpp), covering the regular, `ordisOnly`, and `ordis2Demand` derivative routes.

**Rule.** Four clauses. (1) When an actual rule has an `Anchor*` premise, every installed derivative key retains that premise. (2) The anchor is never selected as the missing premise. (3) The regular route retains the anchor even when it contains the selected premise's output argument; regular acceptance counts, `ordisOnly` eligibility and binding qualifications, and `ordis2Demand` eligibility and binding qualifications remain based on their existing non-anchor rules. (4) The retained anchor is normalized by the unchanged whole-key normalizer: no anchor pinning, `u_` conversion, request-side special case, or layout change exists.

**Why.** Normalization preserves equality classes but cannot preserve a relationship to a premise that was removed. A lone `preorder[...,6,...]` has the same normalized shape as `preorder[...,2,...]`; retaining `AnchorFTA[1,2,3,4,5,6,7,8]` distinguishes a reference to anchor slot 6 from a reference to slot 2 while still accepting consistent alpha-renaming of the complete key.

**How to spot.** An LMV installed from an anchored actual rule whose `keyIds` contain no `Anchor*`; an anchor-shaped marker value; or a normalized request that changes an anchor-slot relationship and still hits the derivative key.

**How to fix on violation.** Fix derivative extraction. Do not change normalization, literal handling, `remainingArgs`, the firing-side request builder, `LocalMemoryValue`, persisted layouts, or the verifier.

**Code.** `memory.cpp::makeNormalizedKeysForAdmission`; tests `tests/test_memory.cpp::prover.admission_normalization_retained_anchor_preserves_bindings`, `prover.regular_admission_route_retains_anchor_context`, `prover.ordis_route_installs_tagged_marker`, and `tests/test_rejected_ordis.cpp::ordis2.install_route_installs_verbatim_tagged_variant`.

**See also.** [D-281](40_decisions.md#d-281), [I-183](#i-183), [I-177](#i-177), and [`20_core_concepts/06_anchors_and_scopes.md`](20_core_concepts/06_anchors_and_scopes.md#anchors-in-derivative-admission-keys).

<a id="i-195"></a>
## I-195 — flag-5 relay: carrier non-local, products local, staged only from local disintegrations


**Scope.** The flag-5 mail relay ([D-284](40_decisions.md#d-284)): `disintegrateExpr2`'s relay selection + `addExprToMemoryBlock`'s staging gate ([`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp)), `fillMailOut`'s flag/direct-deposit staging and the routing-drain status upgrade ([`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp)), the mail side column (`statementFlags_` on `DeloadableMailOut` / `RoutingColdMail`).

**Rule.** Five clauses. (1) A relay is staged only by a LOCAL (status-0/1) disintegration, main scope only — never by a mailed (status-3/5) one, so delivery-to-all-descendants is never duplicated and no relay cascade exists. (2) The relay target is the HIGHEST uncovered existence group: an uncovered group whose enclosing group is covered, or the top compound (`selectRelayWitnesses`). (3) The flag rides the mail statement's side column — never the statement set key — packed with the sender's witness-generation stamp (I-171 across the seam; flag 5 wins, stamps min-merge). (4) At the receiver, a flag-5 statement enters the door at status 5: the carrier registers NON-local (only local delta rows reach `fillMailOut`, so it is structurally never re-forwarded) and joins `intExternalStatements`; admitted products register LOCAL with their `int_lev` carriers memoized into `canBeSentIds`; undemanded products park under the unchanged Pass-B discipline. (5) Every mailed relay deposit ships the same per-key origin mirror as delta statements (`copyOriginRowsToMailOut`, D-45).

**Why.** The C6-class stall: a witness-bearing compact fired at the deriving ancestor cannot register its products there (admission demand is LB-local and lives only at a descendant), cannot mail as a bare row (a mailed positive existence never disintegrates at a receiver), and the descendant cannot fire the rule itself on an all-mailed premise set. The relay carries exactly the parked compacts to every descendant and lets the demand holder mint its own witnesses; the five clauses keep the mechanism bounded, deterministic, and dump-invisible.

**How to spot.** A relay staged from a status-3/5 disintegration (cascade); a flag inside `IntMailStatementKey` (set identity / dump breakage); a status-5 carrier in `intLocalEncodedStatementsDelta` (re-forward); status-5 products registered non-local (demand LB cannot fire on them).

**How to fix on violation.** Fix the staging gate / locality split; never widen the statement key and never weaken the Pass-B demand gating to compensate.

**Code.** `prover.cpp::disintegrateExpr2` (relay selection), `prover.cpp::addExprToMemoryBlock` (staging gate + locality split), `prover.hpp::fillMailOut`, the routing drain in `prover.hpp::standardProcessing`; tests `tests/test_rejected_ordis.cpp::prover.relay_selection_marks_highest_uncovered_group`, `prover.pending_relay_stage_merge_erase`, `prover.status5_relay_arrival_disintegrates_at_receiver`, `tests/test_mail_log.cpp::routing_cold_mail_codec.statement_flag_round_trip`.

**See also.** [D-284](40_decisions.md#d-284), [I-26](#i-26), [I-57](#i-57), [I-171](#i-171), [`20_core_concepts/03_mail_system.md`](20_core_concepts/03_mail_system.md#the-shape-gate-allowedformail).

<a id="i-194"></a>
## I-194

**`productsOfRecursionIds` is closed under MAIN-scope equivalence-class membership at the `standardProcessing` seam.**

- **Scope.** Prover / admission / equivalence classes.
- **Statement.** After every `standardProcessing` class-update fixpoint, every member of a MAIN-scope class that holds at least one `productsOfRecursionIds` member is itself in the set (`ExpressionAnalyzer::enrichProductsOfRecursionFromChangedClasses`, called between the `applyEquiClasses` fixpoint and the deferred expression-side cleanup). Non-main-scope classes never enrich the set. The closure is delta-driven (walks `changedClassesThisStep` only): a member that enters `productsOfRecursionIds` after its class's last change of the step is not retroactively closed over — both mint sites (`updateAdmissionMapRecursion`'s output substitution, `isAdmitted`'s cursor-witness admission) fire on the induction cursors before the relevant classes form, so the gap is not exercised in practice.
- **Why.** The marker-capture purity gate in `checkLocalEncodedMemoryStatic` stages admission demand only on requests whose iteration-bearing premise arguments are all products of recursion. A name proven equal to a product denotes the same value; without the closure, a mailed foreign witness-form (a flag-5 relay product minted at an ancestor) can become the sole surviving instance of induction-built content and starve the gate of every pure instance — the Gauss fold loss ([D-285](40_decisions.md#d-285)).
- **Spot.** `prover.hpp` `enrichProductsOfRecursionFromChangedClasses` (definition + the Step-4a2 call in `standardProcessing`).
- **Fix on violation.** The unit test `prover, classmate_por_enrichment` covers the positive, no-member, and non-main cases; a missed enrichment surfaces as a purity refusal of a demand whose culprit argument has a registered MAIN-scope equality to a product of recursion.
...
**Input-slot demand keys are UNTAGGED, install only through the fourth qualification pass's gated predicate, and are consumed through the ancestor-inclusive probe.**

- **Scope.** Prover / admission / disintegration.
- **Statement.** The algebra demand language carries marker keys at operator INPUT slots on exactly one route ([D-288](40_decisions.md#d-288)): the fourth, independent pass of `makeNormalizedKeysForAdmission`, gated per (premise, input slot) by `inputSlotDemandSlotQualifies` — candidate at a head input slot, confined to its premise, every other premise argument bound elsewhere, and at least `kInputSlotDemandMinPremises` non-anchor premises. These installs are UNTAGGED (readable by `isAdmitted` / wakeable by `revisitRejected2`), unlike the `ordisOnly` input-slot evidence ([I-177](30_invariants.md#i-177)) which stays Pass-B-blind. Consumer side: the Pass-B `it_` admission probes through `isAdmittedIncludingAncestors` — the exact deposit validity first, then its strict ancestors only (`NameMap::strictAncestorSpans`), never a sibling or descendant scope; the hit consumes at the validity that admitted. The I-6 standalone gate stays single-input; unconditional multi-input admission remains forbidden.
- **Why.** A witness at an input slot of a two-input operator (the `preorder` expansions' `in3` atoms) had no admission path at all — the regular route marks output args only, and ordis-tagged keys are invisible to Pass-B — so every such product parked forever (the C8 §7 stall). Demand-driven release closes the gap without the RT explosion the unconditional gate widening reproduced.
- **Spot.** `memory.cpp::inputSlotDemandSlotQualifies` + the fourth pass; `prover.hpp::isAdmittedIncludingAncestors`; `ExecutionParameters::kInputSlotDemandMinPremises`.
- **Fix on violation.** A tagged install on this route, a widened probe (descendant/sibling validities), or a bypassed premise-count gate re-opens either the Pass-B blindness (stall returns) or the fan-out (RT explosion). Tests: `input_slot_demand_slot_qualification_matrix`, `input_slot_demand_pass_installs_untagged_marker`, `is_admitted_including_ancestors_walks_parent_scopes`.
...
**The pre-split merge derives — never proves in an LB — and fires only under a proved or license aligned with literal polarity.**

- **Scope.** Prover / theorem construction / drain seam.
- **Statement.** `constructOrEliminationInRun` runs single-threaded on the phase-4 barrier, after `constructOrTheoremsInRun` and before the deferred-compaction drain, and is a defined no-op in compressor mode. A merge fires only when ALL hold: (1) two proved rows share everything byte-for-byte except one binder-free INNERMOST guard premise (the guard-index key: chain minus last link + head — sound only under the pre-split authoring convention); (2) a proved or theorem's head resolves to an or-category entity with exactly two registry leaves whose head-instantiated forms align with the two guards under ONE variable bijection — numeric tokens ≥ 9 bind bijectively, everything else byte-equal, so polarity stays literal ([I-175](#i-175)); (3) every or-theorem premise is covered: all its non-anchor variables lie in the bijection's domain and its image is byte-present in the common chain, or it is a typing atom over a common-chain variable (the pragmatic prover gate — the external verifier re-checks rigorously). The merged row is DERIVED: method `or elimination`, aux slots = the two variants, the or citation in `orElimCitedOrByMerged` (the chapter row's cell 7; `generateRawProofGraph` asserts on a ledger miss), broadcast via the standard compaction queue. No LB is read or written at the seam; unresolved pairs park on `orElimPendingPairs` and re-probe in insertion order; the pair ledger `orElimBuiltByPair` makes every pair a single attempt. **Family collapse (maintainer, 2026-08-19 — supersedes this entry's earlier "variants never subsumed"):** the guard variants are GRAPH SUPPORT, not pool content — the compressor probe receives the merged representative only (variants excluded from its input AND from redundancy-witness duty, protected from the survivor erase), and the variants leave `theorems.txt` in every save path while keeping their list rows and chapters.
- **Why.** The merge is classical or-elimination — the or theorem is the coverage certificate. Deriving (instead of closing a pool conjecture) keeps the seam free of cross-LB writes and costs zero runtime on the grid; byte-level detection keeps v1 free of alpha machinery, paid for by the authoring convention.
- **Spot.** A merge row without its ledger citation (assert), a merge from a retracted row (the seam runs after `revertVacuousGlobalTheorems` and rescans nothing), or any LB state change attributable to the seam.
- **Fix on violation.** Tests: the `or_elimination` suite in `src/tests/test_or_elimination.cpp` (license flow, pending retry, idempotence, dedup, literal polarity, compressor no-op).
...
**A `proved not broadcast` row is recorded evidence, never a circulating fact.**

- **Scope.** Prover / theorem registration / compressor / export.
- **Statement.** A level-refused closure registers with method `proved not broadcast` ([D-290](40_decisions.md#d-290)) — except in compressor mode, where the tier is a defined no-op (the re-derivation's theorem lists must stay byte-identical to the run it audits, the same contract as both or-construction seams) — and from that moment NOTHING may consume it as a fact or rule: no reformulation or back-reformulation emission, no compaction/broadcast (it never reaches any LB), no or-construction seeding (both the in-run scan and the post-prove head-switch walk skip the method), no compressor participation (excluded from the redundancy-probe input AND immune to the survivor erase), no `theorems.txt` row. Its ONLY sanctioned consumers are read-side: the chapter export (an ordinary direct-proof walk under chapter kind `proved_not_broadcast`), the verifier's citation resolution, and the pre-split merge, which may cite a tier row as a guard variant of an `or elimination` derivation. **The tier is subordinate at the registration sink** ([D-291](40_decisions.md#d-291)): a same-iteration level-complete record drains BEFORE the level-poor one (verdict-first order in `updateGlobalDirectLess`, then the producer-chain tiebreak — never the dispatch `coreId`, a race outcome), and a first-class `appendGlobalTheorem` arrival UPGRADES an existing tier row in place (method, aux columns, producer; the tier's `fullTheoremList` mirror leaves) while a tier arrival never lands on an existing row of any method — so a theorem sits in the tier only when NO circulating derivation of it ever registers.
- **Why.** The D-278 gate's point is that an unused-premise theorem must not compete with its stronger form; recording without circulation keeps that hygiene while making the (previously silent) refusal class auditable and merge-consumable.
- **Provenance completion at registration.** A tier closure characteristically kills its LB in the window BOTH its goals close (the pre-split twin pattern), so a cited sibling theorem's broadcast `theorem` history line may never land — the tier chapter's walk would hit `no origin found`. `repairTierCitationOrigins` (called by the tier branch) therefore walks the producer chain and deposits the missing `("theorem", [])` line for every registered theorem cited at `main` without one — Rule 16 completion of missing process documentation, never a proof input.
- **Spot.** A tier row appearing in `theorems.txt`, in the compressor's survivor accounting, as an or-theorem parent, or arriving at any LB's mail; a `no origin found` abort on a `proved_not_broadcast` chapter.
- **Fix on violation.** Tests: `level_refused_registration_lands_in_tier`, `merge_consumes_tier_variant`, and `tier_registration_repairs_dangling_citation` in `src/tests/test_or_elimination.cpp`.
...
<a id="i-199"></a>
## I-199 — an export row explicitly replays only the premises its certificate row cites


**Scope.** `proof_export/lean.py` — `_render_chapter_steps`,
`_scope_entry_and_specializations`, `_scoped_dependency`, and
`_render_reformulated_theorem`; the generated theory files under each run
export's `lean_export/GLExport/Generated/`.

**Rule.** Every certificate row and composition helper is an explicit Lean proof
term; `grind` is forbidden in the renderer and all generated proof modules.
Implication elimination applies the cited rule to cited premises in the rule's
order; equalities are eliminated by name; conjunctions follow an explicit
projection path. A scoped row enters its own scope and instantiates each cited
same-scope fact at **that fact's own** prefix. A scope-local existence witness is
introduced once, by the projecting row, and shared: consumers obtain it and
apply every guarded row to it. Induction typing, zero, successor, hypothesis,
and final composition all apply their named premises explicitly. FTA external
facts are complete universally quantified target-theorem propositions; rows
specialize them at the current anchor. FTA relational induction is an explicit
`AnchorFTA`-indexed schema. A zero-argument definition compaction introduces an
arbitrary FTA context and replays its cited internal theorem there with those
same global explicit premises.

**Why.** Without the restriction a passing build proves only that each row
follows from *something* already derived, not that GL's recorded justification
is correct — the exact reading under which the artifact is a re-proof rather
than an export. With it, a wrong or incomplete citation breaks the build.

**Spot.** `ExplicitRowReplayTests` rejects any `grind` in the generated modules
and checks every action's required explicit shape. A missing or wrong citation
surfaces as a renderer assertion or Lean application/type mismatch. Both the
default `lake build` and the separate FTA module compile must pass.

**Fix.** Correct the neutral action or its explicit renderer. Never add search or
an uncited premise to make a row pass; either result is a citation or encoding
defect and therefore a finding.

**Composition scaffolding.** Renderer-introduced induction membership and
defined-output totality do not correspond to certificate rows, but they obey the
same no-search rule and expose every premise application directly.
...
**RETIRED — superseded by [I-217](#i-217).** The drain-side canonicalizer (`canonicalizeMailArrival`, status-3 arrivals only, special tiers only, own scope plus ancestors) is gone: the deposit door `addExprToMemoryBlock` now canonicalizes EVERY non-goal, non-equality, non-anchor deposit under the classes at its own scope with the full `chooseCanonicalId` chooser, the absorb drain offers every arrival raw, and the status-3 `intExternalStatements` row is staged from the door's returned canonical id. The one-core rule survives unchanged: `forEachNonCanonicalMemberTokenCore` is the single member-token walk, `filterIterationsCore` and the door's `canonicalFormAtScope` its only two consumers.
...
**The external-mail staging is a strict view of the statement registry: an arrival is staged only if the registry accepted it, and every registry erase purges the matching staged row.**

- **Scope.** Prover / mail absorb / statement registry — `standardProcessing`'s `absorb` lambda status-3 drain, and the three per-expression registry-erase doors `resetResentExpressionRegistries` / `eradicateImplicationFromLB` ([`prover.hpp`](../../GL_Quick_VS/GL_Quick/src/prover.hpp)) and `resetParkedOrStatementRegistries` ([`prover.cpp`](../../GL_Quick_VS/GL_Quick/src/prover.cpp)). Consumer: `performElem2`'s request-generation batches.
- **Statement.** `Memory::intExternalStatements` never holds a row without a live `intKnownStatements` entry at the same `(originalId, validityId)`. Two halves. **Entry:** the drain stages an arrival only AFTER `addExprToMemoryBlock` has run on it and only when the registry then holds it — the door refuses whole shapes outright (an axed anchor-completion argument, [D-234](40_decisions.md#d-234) / [D-236](40_decisions.md#d-236); the iteration cap; the equivalence filter), and the staged id form is the row the door itself encoded and returned (`registeredOut`, the canonical form — [I-217](#i-217)); the drain no longer pre-encodes the arrival. **Exit:** every door that erases an `intKnownStatements` row erases the matching `intExternalStatements` row in the same breath, beside the `intEncodedStatements` scrub it already does. `Memory::wipeSubtree`'s bulk sweep needs no addition: it erases by closed scope and every external row is at `main` ([I-26](#i-26)), which never closes. `performElemPhase1` clears the whole staging each burst, so the invariant is re-established from empty every cycle.
- **Why.** The staging is request generation's mandatory-ingredient source. A row the registry does not hold cannot be a request premise — `BurstSink::consume`'s dependency skip refuses any request whose elements are not all in `intKnownStatements` — so a stale staged row can only produce requests that are built and then thrown away. Under the retired pairing merge that was measurable waste; under the mandatory-containment control (`D-298`) the grow universe is `intEncodedStatements`, so a stale row is simply invisible and the disagreement between the two containers becomes silent. The exit half is the one that is easy to miss: the absorb runs pre-fixpoint ([I-55](#i-55)), so the equivalence-class apply fixpoint later in the SAME `standardProcessing` call can merge a class discovered during this burst and erase the row the arrival was just given — which arrival-time canonicalization ([I-203](#i-203)) cannot anticipate, because the class does not exist yet when the arrival lands.
- **Spot.** A staged arrival with no registry row at request-generation time — the hard-stop assert at the head of the folded batch in `performElem2` names the LB and the expression. Measured before the repair: 10,274 of 570,443 staged arrivals (1.8%) had lost their row by the time the batch read them, all of them positive existence compacts; the shapes the entry half now refuses outright were `x`-prefixed anchor completions.
- **Fix on violation.** Find which door dropped the registry row and add the staged-row purge there; do NOT widen the grow universe to cover the stale row, and do NOT weaken the assert. If a door legitimately erases a registry row while the arrival must stay a premise, that is a contradiction with the dependency skip and belongs to the user (Rule 8), not to a local patch.
...
**An admission key is never consumed. Nothing erases an `admissionMap` entry because it was used.**

- **Scope.** Prover / admission — the four former consumption sites (`revisitRejected2`, `isAdmitted`, `isAdmittedIntegration`, and the integration admission-set branch of `disintegrateExprCore2`) and every `admissionMap` insert site ([`prover.hpp`](../../GL_Quick_VS/GL_Quick/src/prover.hpp), [`prover.cpp`](../../GL_Quick_VS/GL_Quick/src/prover.cpp), [`memory.cpp`](../../GL_Quick_VS/GL_Quick/src/memory.cpp)).
- **Statement.** A key registered in `overallHashMemory.admissionMap` stays live for the LB's lifetime. The only two removals are **canonicalization** — `applyEquivalenceClassToAdmissionMap` drops a changed key and re-keys it to the class canonical ([D-106](40_decisions.md#d-106)) — and **scope death** — `Memory::wipeSubtree`'s closed-scope sweep. Neither is driven by use. There is no consumed-key ledger and therefore no insert-side gate and no mutual-exclusion assert; both sides, algebra and integration, now behave alike ([I-22](#i-22)).
- **Why.** An admission key is a *demand*, and a demand is a template, not a token. Consuming it made the demand order-dependent: a supply arriving before its key existed parked, revived once, and killed the key, so an induction implication that eats through its premises via cursor keys stalled at that slot (C6 Gauss-fold, state doc Step 12). Removing consumption makes the rendezvous order-free in both arrival orders. Measured cost of the machinery before deletion: none — runtime flat across a full pipeline ([D-299](40_decisions.md#d-299)).
- **Spot.** Any new erase of an `admissionMap` / `admissionStatusMap` entry placed on a firing or revival path rather than on the canonicalization re-key or the wipe. A re-introduced consumed-key set is the same violation wearing a different name.
- **Fix on violation.** Delete the use-driven erase. If some future path genuinely needs a demand to expire, that is a semantics change and belongs to the maintainer (Rule 8), not to a local erase.

<a id="i-206"></a>
## I-206 — a resolved `_ordis_` branch is frozen, never wiped: its subtree leaves the request universe and nothing else changes; a goal-less LB has no or branches


**Scope.** [`memory.hpp`](../GL_Quick_VS/GL_Quick/src/memory.hpp) `Memory::orLiveBranches` / `Memory::frozenOrBranches` (persistent-arena pod sets), [`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp) `ordisMerge` (the live sighting), [`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp) `freezeResolvedOrBranches` (the end-of-burst sweep), [`memory.cpp`](../GL_Quick_VS/GL_Quick/src/memory.cpp) `filterIntEncodedStatements` (the single consumer), `prover.cpp` `consumeOrLeavesCohort` + `drainPendingOrReleases` (the goal gate).

**Rule.** Six clauses. (1) `ordisMerge` registers every branch scope it sees in `orLiveBranches` (idempotent; never a frozen, dead-staged or filtered scope). (2) `freezeResolvedOrBranches` runs once per burst in `performElemPhase3` — after `drainDeadOrBranches`, before `drainPendingOrReleases` — over every live branch: a branch is RESOLVED when every `intToBeProved` key whose scope is the cohort parent (`parentOf(vid)`) or an ancestor of it (`ancContains`) is `known` at the branch scope or a strict ancestor (`ancestorKnown`, self included); zero chain goals resolve vacuously. Resolved branches mint into `frozenOrBranches`; frozen and filtered vids leave `orLiveBranches`. (3) The frozen set is a one-way latch until discharge (`dischargeStatementContent` resets both sets with the persistent arena); a frozen branch is never re-evaluated. (4) The ONLY reader of `frozenOrBranches` is `filterIntEncodedStatements`: a statement whose `validityId` is a frozen branch or has one on its parent chain is dropped from the request universe (generator and stump producer alike, so the split partition of [I-156](#i-156) is preserved). No deposit door, registry, bookkeeping, history or wipe consults the frozen set; freezing erases nothing and, in particular, never erases a `toBeProved` goal. (5) Both sets are written only on single-threaded seams (`ordisMerge`'s callers, the end-of-burst sweep); phase 2 reads `frozenOrBranches` `const` ([I-83](#i-83)). (6) An LB whose `intToBeProved` is empty (any scope) opens no cohort, parks no cohort and releases no pending branch — evaluated at every open / release attempt; the flat K / subset-exclusion rules are unaffected.

**Why.** Under the sequenced release a branch that deposited its goal released its successor but kept working for the LB's whole life; its statements and every scope minted below it fed request generation in every later burst, although the cohort could use nothing more from it. The wipe ([D-242](40_decisions.md#d-242)) is not an option — `orBookkeeping`, the convergence history and the chapter export cite the branch's rows — so the branch is frozen: present, not used. The goal-registry gate closes the other leak: reductio LBs (status-0 seed, empty registry for life) opened cohorts whose branches could never resolve by goal.

**How to spot.** A request whose premise scope is a frozen branch or a descendant of one; a `frozenOrBranches` write outside `freezeResolvedOrBranches` / `dischargeStatementContent`; a goal erased by the freeze path; a `_boundary_ordis_` scope minted in an LB with `toBeProved count=0`; a frozen branch reappearing in `orLiveBranches`.

**How to fix on violation.** Route the exclusion through `filterIntEncodedStatements` only; keep the freeze read-only on the goal registry; keep the sweep on the end-of-burst seam; gate every cohort open / park / release on `!intToBeProved.empty`.

**Code.** As in Scope. Tests: `tests/test_memory.cpp::prover.ordis_merge_registers_live_branch`, `::prover.freeze_resolved_or_branches_all_chain_goals_known`, `::prover.freeze_resolved_or_branches_chain_scope_and_latch`, `::memory.filter_int_encoded_statements_excludes_frozen_subtree`, `::memory.generate_encoded_requests_static_skips_frozen_subtree`; `tests/test_rejected_ordis.cpp::prover.ordis_no_goals_neither_opens_nor_parks`, `::prover.drain_pending_or_releases_goalless_lb_releases_nothing`.

**See also.** [D-306](40_decisions.md#d-306), [I-174](#i-174), [I-172](#i-172), [I-187](#i-187), [I-108](#i-108), [I-83](#i-83), [I-156](#i-156).

<a id="i-207"></a>
## I-207 — inside one `applyEquiClasses` call the rejected maps gain keys only through the two rejected-map hooks' canonical re-keys; the hooks visit only keys at the class validity through a per-apply index that is never rebuilt, emit hits in template lex order, and skip a round that follows a round without hits


**Scope.** `prover.hpp` `applyEquiClasses` (round bookkeeping, one `RejectedValidityBuckets` per map per apply), `applyEquivalenceClassToRejectedMap` / `applyEquivalenceClassToRejectedMapIntegration` (bucket walk, hit count return, in-place re-key); `memory.hpp` `RejectedValidityBuckets`.

**Rule.** (1) Between the entry and the exit of `applyEquiClasses` the only inserts into `rejectedMap` / `rejectedMapIntegration` are the two hooks' re-keys under the canonical K' ([D-308](40_decisions.md#d-308)); hook drops, admission-hook revivals and the hooks' demanded-case mail erase; every other insert lands at an absorb outside the apply. (2) The two hooks read, from the per-apply bucket index built once at the first hook use, only the keys at the class validity whose template names a non-canonical class member (the argument postings; sorted + deduplicated candidates, each re-checked by the substitution test); a key at another validity or naming no member is never decoded, a key dropped earlier in the same apply is a `lookup` miss, a K' inserted during the apply is not in the index and is visited at the next apply. (3) Hits are processed in template lex order (drop set and re-key records alike). (4) A hook runs for a class in round r > 0 only if that map's hook hit something in round r-1 or the class never ran the hook (a delta entry pushed mid-round).

**Why.** Rules 1, 2 and 4 together make the skip exact: the index cannot grow, so a round without hits proves the next round hits nothing; K' never names a non-canonical member of the class that minted it, so the class misses no hit by not seeing K'; a K' that equals a key dropped earlier in the same apply is reached again through that key's stale posting. A key naming members of two classes reaches its fully canonical form within two applies (phase 1 and phase 3 of consecutive elementary steps). Rule 2 removes the O(classes × M log M) whole-map walk that was 94 % of `applyEquiClasses` at the measured burst. Rule 3 keeps the emission sequence the former whole-map sort produced.

**How to spot.** A hook decoding keys outside `runBegin..runEnd`; a rebuild of `RejectedValidityBuckets` inside an apply; a Pass-2 hook call without the `skipRm` / `skipRmi` gate; a Pass-1 gate without the `k < deltaCountPrevRound` clause; a hook insert that is not a `updateRejectedMap` / `updateRejectedMapIntegration` call.

**How to fix on violation.** A new insert path inside the apply goes through the kernel writers and is invisible to the current apply's index (visited next apply), never a reason to rebuild the index mid-apply; a new consumer that needs a map-wide view gets its own bucket run, not a whole-map walk.

**Code.** As in Scope. Tests: `tests/test_memory.cpp::memory.rejected_validity_buckets_group_keys_by_validity`, `::prover.apply_equi_rejected_walks_only_class_validity_and_returns_drops`, `::prover.apply_equi_rejected_rekeys_in_place_when_undemanded`, `::prover.apply_equi_rejected_rekey_unions_pair_levels`, `::prover.apply_equi_rejected_demanded_compound_still_mails`, `::prover.apply_equi_rejected_integration_rekeys_in_place`.

**See also.** D-307, D-308, I-37, I-117, I-124, I-130, I-99.

<a id="i-208"></a>
## I-208 — every or-category entity carries its complete or-implication list before any burst, and a registry-or cohort's K / subset-exclusion rules enter hash memory only as expansions of those compacts


**Scope.** `memory.hpp` `LogicalEntity::implications`; `prover.cpp` `compileOrKRules` / `compileOrSubsetExclusions` / `expectedOrImplicationCount` / `preMintReducedOrs` (completion pass) / `consumeOrLeavesCohort` (compact route); `visualizer.cpp` `loadGlBinary` / `exportCompiledExpressionsJSON`; `verifier.py` `_expand_implication_compact`.

**Rule.** (1) An or minted anywhere gets its K-rule compacts at the mint; `preMintReducedOrs` — the seam before every burst — appends the subset-exclusion compacts once the reduced closure exists and asserts every or's list holds `expectedOrImplicationCount(k)` entries (k for k < 3, 2^k − 2 otherwise); an or loaded from a binary without the field is the only or allowed to reach that seam with an empty list (`orsAwaitingImplications`). (2) The list is the projection: entry n is the compact `(implication<N>[u_…])` whose expansion over the or's own tokens is the n-th rule of the disintegrator's emission order (K-rules in leaf order, then size-j exclusion subsets, j ascending, lexicographic). (3) `consumeOrLeavesCohort` never reconstructs a registry or's rules: it instantiates the compacts positionally and disintegrates them as product statements; the De-Morgan door (no entity) is the sole on-the-spot construction. (4) Provenance: the compact carries the `disintegration` origin citing the or's expanded form; the rule carries an `expansion` origin citing the compact.

**Why.** The compacts are the handle a later equivalence-class rewrite needs (statements are rewritten, on-the-spot text is not); completing the list at the seam keeps `or<N>` numbering unchanged; the completeness assert is the tripwire for a missed mint hook.

**How to spot.** A `reconstructImplicationFullBindScratch` call in `consumeOrLeavesCohort` outside the De-Morgan else-branch; a `compiledExpressions[...] = LogicalEntity("or", …)` write without `compileOrKRules`; a binary or entry whose `implications` length is not `expectedOrImplicationCount`; a `disintegration` row for a registry or whose expression is a `(>` rule (registry ors emit compacts).

**How to fix on violation.** Route the new mint through `findOrMintOrOperator` or call `compileOrKRules` after the insert; never pad the list — regenerate it at the seam.

**Code.** As in Scope. Tests: `tests/test_or_implications.cpp` (all), `tests/test_rejected_ordis.cpp::prover.single_exclusion_rules_emitted_for_registry_or`.

**See also.** D-309, I-52, I-184, I-185, I-137, D-269.

<a id="i-209"></a>
## I-209 — every two-element existence entity carries its complete existence-implication list before any burst, and a negated existence's two rules enter hash memory only as expansions of those compacts


**Scope.** `memory.hpp` `LogicalEntity::implications`; `prover.hpp` `compileCoreExpressionMapCore` (the `!(>` mint hooks); `prover.cpp` `compileExistenceImplications` / `expectedExistenceImplicationCount` / `preMintReducedOrs` (completion pass) / `disintegrateExprCore2` (negated-existence block); `visualizer.cpp` `loadGlBinary`; `verifier.py` `check_expansion`.

**Rule.** (1) An existence minted anywhere gets its two compacts at the mint (`[0]` = `left → !right`, `[1]` = `right → !left`); `preMintReducedOrs` — the seam before every burst — asserts every two-element existence's list holds `expectedExistenceImplicationCount` entries; an existence loaded from a binary without the field is the only one allowed to reach that seam with an empty list (`existencesAwaitingImplications`). (2) The negated-existence block recognizes its input through the registry (`compiledEntity` on the inner core — the prepared instruction is empty for a negated compound, `prepareIntegrationCore`'s polarity guard) and never builds rule text: it instantiates the compacts positionally, registers each as a product statement of the negated existence with an `expansion` origin citing it, and lets the implication branch expand it into the rule (rule ← `expansion` ← compact). (3) The compile renames the bound variable before `compileImplicationToCompact` — the `u_` strip would otherwise merge it with token `u_1`.

**Why.** The compacts are the handle a later equivalence-class rewrite needs (statements are rewritten, on-the-spot text is not); the completeness assert is the tripwire for a missed mint hook; the rename keeps the compact spanning the existence's full arity.

**How to spot.** A probe of the prepared instruction for the positive existence (the retired `innerHit`) in the negated-existence block; a `reconstructImplicationFullBindScratch` or `insertImpl` call there; a `startInt++` there; an existence registered without `compileExistenceImplications`; a binary existence entry with two elements whose `implications` length is not 2; an `expansion` row citing a negated existence whose expression is a `(>` rule (the rule row cites the compact).

**How to fix on violation.** Route the mint through `compileCoreExpressionMapCore`'s `!(>` branch or call `compileExistenceImplications` after the insert; never pad the list — regenerate it at the seam.

**Code.** As in Scope. Tests: `tests/test_existence_implications.cpp` (all); `tests/test_verifier_structural.py::test_expansion_negated_existence_compact_row_accepted`.

**See also.** D-310, D-309, I-208, I-52, I-175, I-137, D-274.

<a id="i-210"></a>
## I-210 — only the canonical form of a compact implication has rules in hash memory; the install gate and the `applyEquiClasses` compact hook keep it so; a rule's removal is exact through the owner lists and the install's own key enumeration


**Scope.** The rule side of compact implications (`(implication<N>[…])`, D-309 / D-310 and every mailed compact): `installExpandedImplication` (the install door's per-implication body), `canonicalizeCompact`, `expandCanonicalCompact`, `applyEquivalenceClassToCompactImplications` (the `applyEquiClasses` hook), the carrier index `Memory::compactExpansions`, `removeCompactExpansion` / `removeRuleFromHashMemory` / `finishCompactRemovals`, and the owner-run maps of [I-49](#i-49). Established by [D-312](40_decisions.md#d-312).

**Rule.**
1. **Statements are multiplied, rules are canonical.** `applyEquiClasses` multiplies compact statements like any statement (hash fodder, [I-65](#i-65)); hash memory holds rules only for a compact that is canonical at its scope — every argument the canonical representative (`chooseCanonicalId`) of the deepest class visible from the scope that holds it.
2. **The install gate.** `installExpandedImplication` canonicalizes a compact carrier before installing its rule: a non-canonical K installs nothing; its canonical form K* is expanded once (`expandCanonicalCompact` — skipped when K* is already a carrier here or known at a strict ancestor), K* registered with the `equality1` row `K* ← K, (=[from,to])…` and the level run = the deposit's levels ∪ the fired pairs' levels (D-280). The gate is the one place both compact paths share (a cohort's compacts reach the door only through the implication channel's source column).
3. **The hook.** For every class visit of `applyEquiClasses` (both passes, the D-307 round skip), the compacts at the class scope or a strict descendant naming a non-canonical member are reconciled: the rules of a now non-canonical compact leave and its canonical form is expanded; a registered canonical compact without rules is expanded as is. Removals are staged and finished ONCE per apply (`finishCompactRemovals`: the carrier keys erased in one pass, one `encodedMap` compaction + owner-less erasure per instance), before the next hashburst.
4. **Exact removal.** A rule leaves through the install's own key enumeration under `RuleIndexOp::Remove` (whole keys, subkeys, remaining-args edges, the chain — each losing exactly this rule's owner, [I-49](#i-49)); the LMVs go through the batched compaction; the transient instances are probed (`onlyIfPresent`), the persistent ones asserted. The drop set is staged by the enumeration copy by copy (`RuleIndexOp::dropSet`): the install's own multiplication (`multiplyImplication`) decides the copies, each under its own `ruleInterner` id, so a multiplied rule stages every text's LMVs and a rule whose multiplication emitted no copy (the trivial-head skip on the original itself, every other partition equating two `u_` variables — [G-69](50_gotchas.md#g-69)) installed nothing and stages nothing; the transient probe reads the FIRST copy's chain owner for the same reason. The visited set that turns a symmetric-premise repeat into a no-op (`RuleIndexOp::firstVisit`) is keyed by (container, owner, key): two copies of one rule reaching one entry are two owners, and each leaves. The owner an index entry carries is the COPY (`(copy text id, scope)`), and two different rules can multiply into one copy text; `copyOwners` lists the rules per copy, the removal delists the rule first and touches a copy's entries and LMVs only when it was the copy's last rule (the transient probe asks the same list: is this rule listed on its first copy here).
5. **The carrier index and the dump index stay in step; a shared rule leaves with its last carrier.** `compactExpansions` (carrier → expansions + install kind) gains an entry at every install and loses it at every removal; `expansionCarrierCount` counts the carriers per `(text, scope)` — two compacts can expand into one rule text — and the rule (and its `expandedImplications` pair, the dump's section source) leaves hash memory only when the count reaches zero.

**Why.** A stale-named rule fires only on stale-named statement variants and yields stale-named conclusions the multiplication must fold back; with the statements multiplied, the canonical rule on the canonical premise combination derives everything the variants would (the maintainer's "one canonical form is enough as expressions are multiplied"). The end-of-burst `sanitizeHashMemory` (I-46) rewrote only `it_/int_` names, keyed on expanded texts and scanned every map — inert in practice.

**Spot.** A hash-memory rule whose carrier compact is non-canonical at its scope while its class exists; a compact carrier in `compactExpansions` whose rules are not in `encodedMap` (or the reverse) after an apply; `ownerlessPending` still raised after an apply; a removal assert on owner presence (the enumeration diverged from the install — a derivative-key gate reading mutable state).

**Fix.** Route every rule install through `installExpandedImplication`; keep the hook registered in both `applyEquiClasses` passes and `finishCompactRemovals` after the fixpoint; never install a rule outside the gate.

**Code.** [`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp): `isCompactImplicationSpan`, `canonicalizeCompact`, `compactExpandedHere`, `expandCanonicalCompact`, `installExpandedImplication`, `applyEquivalenceClassToCompactImplications`, `removeCompactExpansion`, `finishCompactRemovals`, the `applyEquiClasses` wiring; [`memory.hpp`](../GL_Quick_VS/GL_Quick/src/memory.hpp): `RuleIndexOp`, `CompactExpansionRec`, `DisintPairChannel::forEachSortedWithSource`, `RejectedValidityBuckets::ensureBuiltFrom`. Unit tests (`test_memory.cpp`): `canonicalize_compact_rewrites_to_the_deepest_class_canonical`, `canonical_gate_expands_only_the_canonical_compact`, `compact_hook_removes_stale_rules_and_expands_the_canonical_form`, `carrier_index_records_and_removes_a_compacts_rules`, `remove_rule_from_hash_memory_deletes_exactly_its_owners`. See [I-49](#i-49), [I-65](#i-65), [I-207](#i-207), [I-208](#i-208), [I-209](#i-209).

<a id="i-211"></a>
## I-211 — a selected CUDA Phase 2 performs every semantic request stage on-device after stump discovery, emits complete provenance-bearing canonical records, and never falls back

when merged to `main`.

**Scope.** `main.py` / `run_modes.py` Phase 2 backend selection; [`prover.cpp`](../../GL_Quick_VS/GL_Quick/src/prover.cpp) `ExpressionAnalyzer::proveKernel`; the CUDA projection, task, filtering, growth, ordering, evaluation, and doom buffers; `Phase2SealingArena`; [`memory.cpp`](../../GL_Quick_VS/GL_Quick/src/memory.cpp) `applyFiringRecords`.

**Rule.** Four clauses. (1) Processor is the default of every run; CUDA is entered only by explicit selection (`main.py --GPU` for the whole run, `--phase2-backend cuda`, or a batch config's `use_gpu: true` — [D-329](40_decisions.md#d-329), [D-338](40_decisions.md#d-338)), and an unavailable device, capacity overflow, projection error, launch error, unsupported live frontier depth, or device contract failure asserts instead of invoking processor semantic work. (2) A split logical block may use the processor stump producer only to discover disjoint expression branches. After that boundary the selected CUDA route performs statement filtering, normalized-key growth, whole/subkey satisfaction, request order and deduplication, dependency and validity gates, reverse-owner expansion, local-value selection, substitution, firing verdicts, complete head provenance and levels, marker payloads, canonical content ordering, doom-prefix selection, and per-task submatch counting on-device. (3) Host sealing decodes projected identifiers and converts already-decided bounded payloads into `FiringRecord` ownership only; it never re-evaluates a rule or reconstructs missing provenance. `applyFiringRecords` may skip its sort only for the device-canonical chain and shares every deposit operation with the processor route. (4) Processor and CUDA shortcut artifacts, including provenance row content and order, must be byte-identical and pass the same verifier with zero failures before performance is evaluated.

**Why.** A partial port can look fast by omitting the branch-heavy work or provenance that defines Phase 2. Automatic fallback can also hide a device-capacity or semantic defect behind a successful processor result. The explicit boundary makes acceleration measurable without weakening the proof contract.

**How to spot.** A CUDA run with no device-pass telemetry; a CUDA error followed by processor completion; host code calling the request evaluator to fill a device record; a generated head without its complete source-first provenance; `firingRecordsCanonical=true` on a processor-built chain; different artifact bytes or verifier totals between backends.

**How to fix on violation.** Stop at the first exact failing seam and fix the device representation or kernel forward. Do not add a fallback, theorem-only path, host semantic replay, or a relaxed identity comparison.

**Code.** `main.py` `main`; `run_modes.py` `shortcut_run` / `run_gl_quick`; `main.cpp` `main`; `run_modes.cpp` `fullRun`; `prover.cpp` `ExpressionAnalyzer::proveKernel` / `ExpressionAnalyzer::performElemPhase2`; `phase2_cuda.cu`; `phase2_sealing.cpp` `Phase2SealingArena::downloadAndSeal`; `memory.cpp` `applyFiringRecords`. Direct CUDA twins live in `tests/test_phase2_projection.cpp`. See [D-313](40_decisions.md#d-313), [I-44](#i-44), [I-66](#i-66), [I-77](#i-77), and [I-191](#i-191).
...
**Only the class-canonical form of a deposit is admitted: the deposit door rewrites every non-goal, non-equality, non-anchor deposit under the equivalence classes at its OWN scope and ends the deposit when that form is already known.**

- **Scope.** Prover / the deposit door / equivalence classes / mail absorb — `addExprToMemoryBlock` ([`prover.cpp`](../../GL_Quick_VS/GL_Quick/src/prover.cpp)); `canonicalFormAtScope`, `forEachNonCanonicalMemberTokenCore`, `findEqualityCiteScope`, `originRowExists`, and `standardProcessing`'s `absorb` lambda ([`prover.hpp`](../../GL_Quick_VS/GL_Quick/src/prover.hpp)).
- **Statement.** Five clauses. (1) After Site F (raw text) and Site H, every deposit with `status!= 2` that is neither a positive nor a negated equality nor an anchor predicate passes `canonicalFormAtScope` exactly once: each token of the statement that is a member of a class in the deposit scope's OWN bucket (`equivalenceClassesMap` at that validity — an ancestor's bucket never binds; a fresh scope's bucket is empty, so its seeds keep the spelling their scope name / `recursionHypothesisId` were minted from) and is not that class's `chooseCanonicalId` representative (normal > `int_lev_*` > `it_*_lev_*`, weak members at the scope skipped, lex-min within a tier) is rewritten to it — the special tiers from the token scan, the normal names from `collectExprTokens`. The walk is the ONE core `forEachNonCanonicalMemberTokenCore`; the registry filter `filterIterationsCore` is its `firstSpecialMemberId` / empty-normal-source instance (byte-identical probe order, [I-134](#i-134)); no parallel twin of the walk may be written. `skip_eq_classes` disables the door. (2) A changed deposit whose canonical form is known at the scope or a strict ancestor (`ancestorKnown`, self-inclusive — the same predicate Site F runs on the raw text) ends there: nothing registers, no history line is written for the new raw spelling, and BOTH the raw and the canonical ids run `stageOrReleaseForRefusedDeposit` ([I-174](#i-174)). (3) Otherwise the door proceeds with the canonical text, and the rewrite FLIPS the deposit to a local derivation iff at least one applied equality is a LOCAL statement of this LB (`appliedEqualityIsLocal`: the pair's row or its mirror in `intKnownStatements` at the class scope or a strict ancestor; a transitively joined pair with no direct row is local iff any equality between two members of the class is a local row) — a rewrite under this LB's own equality is its class product, the `equality1` rewrite `applyEquivalenceClass` would have produced, while a rewrite licensed only by mailed equalities keeps the arrival's locality — so in the local case the canonical form registers LOCAL (the local registry and the per-step delta: the main-goal discharge's only source, and `fillMailOut`'s). Under `trackHistory` the raw text keeps its producer line under the raw key (a rewritten deposit without an origin line asserts — a bridge citing a form no chapter can resolve is the `buildStack: no origin found` abort) and, whenever the registered row will be local, that row ships to `mailOut` at once (`copyOriginRowsToMailOut`, the [D-286](40_decisions.md#d-286) ship: the raw form is never a delta row, so `fillMailOut` would never carry it and a receiver's chapter walk would dangle on the bridge); the canonical form's origin is ONE `equality1` bridge — deps[0] = the raw form at the deposit scope, then each applied `(=[member,canonical])` cited at `findEqualityCiteScope` (class scope, else the deepest strict ancestor holding the row, [D-227](40_decisions.md#d-227)) — written only when the canonical form has no origin row yet ([I-34](#i-34)); the level run is the deposit's run ∪ the applied pairs' class levels (`{-1}` transparent, [I-182](#i-182)); the optional `registeredOut` receives the encoded canonical row. (4) The absorb drain offers every arrival RAW and stages the status-3 `intExternalStatements` row from the door's returned id only when the registered row stayed non-local (a rewrite under a local equality is local, not external) — the door is the single statement canonicalizer (the drain-side `canonicalizeMailArrival` of the retired [I-203](#i-203) is deleted). (5) `applyEquiClasses` keeps multiplying the registered canonical form into its full orbit (products commit inline in `applyEquivalenceClass`, never through the door), so a literal non-equality goal still closes from the product in the same `standardProcessing` call (Step 4 before Step 5); a positive equality still merges classes once at arrival and a negated one still gets only the one-sided [I-12](#i-12) expansion — the raw door leaves both paths untouched.
- **Why.** The locality flip is what the first landing lacked, and it cost eight shortcut theorems: at the induction zero block of `in[9] ∧ in[10] ∧!(9 ≤ 10) → 10 ≤ 9` the class {0, 10} has canonical "10", so the mailed non-local fact `0 ≤ 9` proceeded as `(preorder[1,4,10,9])` — the goal's literal text — registered non-local; `dischargeToBeProved` reads only local delta rows for a main goal, and the later local derivation of the same canonical text was Site-F-refused as known, so the goal never closed (on `main` the mailed row keeps the `0` spelling and the LOCAL class product discharges it). The IncubatorGauss3 heavy LB absorbed ~106k internal-mail rows per burst, ~69k of them class variants of known facts that paid the full door (`checkNecessityForEquality` sweep + `disintegrateExpr2` attempt) before `addStatement`'s `filterIterations` refused them silently with no canonical substitute (36.5k code-28 refusals measured). One canonical form per fact per scope at the one entry every deposit passes turns those variants into an entry-level refusal, and a variant whose canonical form was never registered now registers it instead of being lost. Same-scope-only is what the earlier door attempt lacked: its ancestor-scope rewrite renamed `_orint_` seeds under a branch scope name embedding the raw disjunct and rewrote the recursion hypothesis away from `recursionHypothesisId`; equalities raw is what keeps equality-head goals closable (goal closure is literal and neither equality shape is ever multiplied).
- **Spot.** A registered statement at scope S carrying a member of a class in S's own bucket that is not that class's representative, when the class existed at the statement's arrival; a raw spelling registered next to its canonical form at one scope; an `intExternalStatements` row whose text differs from the door's canonical form; a bridge `equality1` row whose raw dep has no origin line at the receiver.
- **Fix on violation.** Tests (`src/tests/test_memory.cpp`): `for_each_non_canonical_member_token_core`, `for_each_non_canonical_member_token_core_normal_source`, `canonical_form_at_scope_substitutes_and_records`, `canonical_form_at_scope_own_scope_only`, `canonical_form_at_scope_non_derived_tier`, `canonical_form_at_scope_shape_gates`, `origin_row_exists_and_find_equality_cite_scope`, `door_registers_canonical_form_only`, `door_skips_known_canonical_form`, `door_exempts_goals_and_equalities_not_or_compacts`; the twin pins `filter_iterations_core_view_matches_heap_scan` and `tokens_view_of_facets_match_typed_assign_oracle` hold the registry filter byte-identical.

<a id="i-216"></a>
## I-216 — At the origin cap no existing row is displaced; the class→body merge keeps every body row

**Scope.** Every origin writer (`addOriginId` heap + cold, `addMailOriginRecord`, `MergeClassAccum::addOriginId`, the string `ExpressionAnalyzer::addOrigin`) and the `updateEquivalenceClasses` origin sync `exprOriginRunReplace`.

**Why.** A per-key run is well-founded only in insertion order: a row's dependencies were recorded before it. Any tag-based displacement can remove the one non-circular row (the class merge's `equality2` line for a pair equality) in favour of rows that are circular for the chapter walk, leaving a goal with no acyclic derivation — [D-325](40_decisions.md#d-325).

**Spot.** A class-pair equality whose rows are all same-tag firings with no `equality2` line; a chapter goal with no acyclic derivation (a least-fixpoint pass over `exprOriginMap` — root not derivable); `buildStack` at its 5M-call tripwire.

**Fix.** Never reintroduce a displacement rule; if a key needs more rows, raise the cap (and the run-buffer constants in `parameters.hpp`).

<a id="i-218"></a>
## I-218 — The equivalence classes at one scope are pairwise disjoint

**Scope.** `Memory::equivalenceClassesMap` per validity id; the sole writer [`prover.hpp::updateEquivalenceClasses`](../GL_Quick_VS/GL_Quick/src/prover.hpp) (the other caller wipes a scope).

**Rule.** After every write-back no two classes at the written scope share a member. `updateEquivalenceClasses` guarantees it by its same-scope fixpoint pass after the ancestor pass ([D-322](40_decisions.md#d-322)); `mergeTwoEquivalenceClasses` accepts a same-scope merge whose only overlap is a shared member (member bridge).

**Why.** Every class consumer (canonical chooser, orbit multiplication, negated-equality expansion, the compact-implication hook) assumes one class per member; two overlapping classes give two canonical forms for one member and make the I-33 precondition `!eqArgsSubsetB` fire on the next equality that touches them.

**Spot.** `Assertion failed: !eqArgsSubsetB` in `mergeTwoEquivalenceClasses`; a Rule-14 dump's `equivalenceClassesMap` section listing two classes at one scope with a common name.

**Fix.** Never trim the fixpoint pass; a new class writer must re-establish disjointness before it returns.

<a id="i-215"></a>
## I-215 — `changedClassesThisStep` is cleared only at `standardProcessing`'s tail; out-of-step deltas survive to the next call

**Scope.** `Memory::changedClassesThisStep` — writer `updateEquivalenceClasses` ([`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp)), consumers `applyEquiClasses` (Pass 1), the deferred `cleanUpExpressions` sweep, and the recursion-products closure ([I-194](#i-194)), all inside `standardProcessing`; the sole clear is `standardProcessing` Step 7.

**Rule.** No code path clears the tracker except `standardProcessing`'s Step 7, which runs after all three consumers. A delta minted OUTSIDE any `standardProcessing` call — LB seeding via `addTheoremToMemory` (variable-copy axioms, equality premises), post-join barrier deposits — therefore survives to the LB's next `standardProcessing` call, whose `applyEquiClasses` Pass 1 back-applies the class to every pre-existing statement. The retired `performElemPhase1` entry clear discarded exactly those deltas unconsumed; because a class's Pass-2 waterline is seeded to the then-current statement-registry size at creation, Pass 1 was the only path that could ever multiply the class into the seeding-time statements ([D-323](40_decisions.md#d-323)).

**The one sanctioned in-call loss.** A class minted after Step 4 inside the same `standardProcessing` call — only `dischargeContradiction`'s head deposit can do this (an equality-headed vacuous-truth / incubator close) — is wiped by that call's Step 7 unconsumed. Harmless by construction: every reacting `dischargeContradiction` role deactivates the LB in the same act, and a deactivated LB never applies classes again ([I-112](#i-112)).

**Spot.** A statement multiplied by a seeding-time class only inside hypothesis scopes while missing at `main` (the two-variable `interval ⟹ in` Gauss root); a `[WL-TRAP]`-style probe showing a class created with waterline N whose Pass 1 never ran and whose Pass 2 always starts ≥ N.

**Fix.** Never add a second clear site (entry clears included); a consumer added after Step 7 must move the clear below itself.

<a id="i-219"></a>
## I-219 — only the FIRST equi representative of an or<N> gets full processing at a scope; later variants stay passive statements

**Scope.** The deposit door's or-uniqueness gate: [`prover.cpp::addExprToMemoryBlock`](../GL_Quick_VS/GL_Quick/src/prover.cpp) (the `orEquiDuplicate` term of the `willDisintegrate` seam), the probe/insert/clear/re-key quartet `orEquiRepresentativeRecorded` / `recordProcessedOr` / `clearProcessedOr` / `recanonicalizeProcessedOrLedger` with the shared `canonicalOrIdAtScope` ([`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp)), the per-scope `Memory::processedOrLedger` ([`memory.hpp`](../GL_Quick_VS/GL_Quick/src/memory.hpp), persistent arena), and the three clear/GC seams — `resetParkedOrStatementRegistries`, both ordis park hooks' re-mail loops ([`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp)), and the disproof-cleanup wiped-scope sweep.

**Rule.** An `(or<digits>[…])` deposit (shape predicate `isOrCompactSpan`; the De-Morgan `!(op[…])` door and or-shaped goals never match) whose equi class already holds a fully-processed representative at the deposit's OWN scope gets exactly two things: its (door-canonical) text registers as a plain statement, and `applyEquiClasses` multiplies it like any statement. Everything else is suppressed on every entry path, local and flag-5 relay alike — no `disintegrateExpr2`, so no K/subset-exclusion compact instantiation, no cohort bootstrap, no `rejectedMapOrdis`/`rejectedMapOrdis2` filing, no `admissionMapOrdis2` consumption, and no `fullyDisintegrated` stamp. The equi test iterates the scope's ledger run and matches a recorded or with the same operator whose spelling canonicalizes (own-scope class bucket, `canonicalFormAtScope` — exactly the door's walk, so the gate is door-consistent; `checkForEquivalence`'s whole-arg Cartesian probe stays active behind it for the ors the gate does not suppress) to the deposit's canonical spelling; the deposit-time canonicalization keeps the test current when the first or's canonical form later stops being canonical. Every path that intentionally re-runs an or consumption clears the ledger row first — the parked-or un-know and the two ordis park hooks' re-mails — and the disproof cleanup sweeps rows at wiped scopes; `wipeSubtree`d rows stay inert like the cohort rows (a closed scope never re-deposits). The ledger is per-scope (I-167: equal signatures under different parents stay independent case splits), rides the persistent arena (no deload stream, no dump section), and is reclaimed at discharge.

**Why.** Or proliferation was self-feeding: every equi spelling of one or reaching the door got the full per-statement consumption (K compacts recursively disintegrated, cohorts, parks), which minted more statements and equalities, which minted more spellings. The canonical deposit door (I-217) only binds at deposit time — forms registered before their arguments became class-equal each kept driving their own duplicated cohorts.

**Mechanism ([D-335](40_decisions.md#d-335)).** `recordProcessedOr` stores `packInt32Pair(canonicalOrIdAtScope(origId), origId)`; the canonical half is the text's class-canonical id under the scope's OWN bucket (the door's walk, I-217) and is re-derived for every row of the scope by `recanonicalizeProcessedOrLedger` at the class-change seam, so `orEquiRepresentativeRecorded` is one pass over the run comparing ids — the canonical half catches the equi-variant case, the original half the byte-equal case, exactly the two matches the former probe-time canonicalization produced, at the cost of the run length instead of a class walk per record per deposit. The D-331 drain groups by the same canonical half.

**Spot.** Two `orDisjunctCount` rows at one parent whose signatures are equi variants of each other; a Rule-14 dump showing K compacts instantiated for two spellings of one or at one scope; a parked or whose revival re-deposits and consumes nothing (a missing ledger clear).

**Fix.** Never widen the two passive allowances (registration + multiplication) for a suppressed variant; a new or-consumption re-run path must clear the ledger row first; the ledger rows are packed `(canonical id, original id)` pairs and the probe is an exact id membership test on either half — every writer of a scope's class bucket must run `recanonicalizeProcessedOrLedger` for that scope (today the one seam is `updateEquivalenceClasses`, right after the `changedClassesThisStep` push); a bypassing writer leaves stale rows and a duplicate or gets full processing again.

**See also.** [D-331](40_decisions.md#d-331) — the merge-seam companion collapsing cohorts that opened before their signatures became class-equal (`retireDuplicateOrCohorts`).

<a id="i-220"></a>
## I-220 — in incubator mode the root sentinel's mail log has exactly one reader, the anchor LB; a grid-wide seed batch is stored in the anchor LB's log

**Scope.** The mail-edge registration at grid build (`collectMailAncestors` → `MailLog::registerLb`), the load-time seed `broadcastTheorems`, and every reader of the root's log, in batches with `incubator_mode`. Established 2026-09-02 ([D-332](40_decisions.md#d-332)).

**Rule.** In incubator mode an LB strictly below the anchor LB (`isAnchorLb`: `exprKey` starts with `"(" + anchorInfo.name`) registers its mail ancestors up to and including the anchor LB and never the root sentinel; the anchor LB keeps its root edge; the root has none. Consequently every batch the root's log carries — the deferred-compaction compacts of in-run proofs, the `updateGlobalDirect` / `updateGlobal` theorem-origin rows, the load-time batch of theorems proved by earlier incubator batches — is read by the anchor LB alone — for the theorems `anchorOnlyRule` accepts (single-premise, or two-premise with an operator second premise and an equality head connecting the operator's output to an anchor slot value; the first premise is this batch's anchor or, for a loaded theorem, an earlier incubator batch's). A nested incubator-derived theorem of any other shape is stored in the anchor LB's log as well (load: `nestedStore`; in-run: the drain's second `mailOut` deposit), so the premise LBs below the anchor keep its rule. A seed batch that must reach every LB (the externally provided theorems) is stored in the anchor LB's log and self-injected into the anchor LB, so the anchor LB and every LB below it read it. The anchor LB relays the root's HISTORY: at its status-3 absorb every origin row of the root's batches is copied into its own `mailOut.exprOriginMap` (`relaysAbsorbedHistory`), so an LB below the anchor receives one cycle later exactly the origin rows the root's log used to deliver — the compacts' `compilation` rows, the `theorem` rows — and never the statements. Outside incubator mode nothing changes: the whole chain is registered and the root's log reaches every LB.

**Why.** The incubator's own theorem rules are `anchor → head`; the anchor LB fires them and mails the heads down, so installing them below the anchor is pure cost (the dominant incubator runtime). The externals are multi-premise lemmas that fire in the premise LBs below the anchor — the ES2 / ES3 LBs depend on them — so they may never be narrowed.

**Spot.** A `buildStack: no origin found` in an incubator chapter below the anchor LB naming a compact (`(implication<N>[])`) — the history relay is missing or gated off; an incubator batch whose ES2 / ES3 LBs stop proving (externals routed through the root's log — check the `store` of the externals `broadcastTheorems` call); a `collectMailAncestors` assert (an incubator LB without the anchor LB on its chain — a conjecture that does not start with the anchor premise); an `incubatorAnchorLb` assert (the root sentinel with zero or several anchor children); a non-incubator batch whose theorem list changes (the pruning leaked outside `incubator_mode`).

**Fix.** Keep the two batch kinds on their logs: incubator-derived theorems in the root's log (one reader), externals in the anchor LB's log (every reader below the anchor). Never add a second reader to the root's log in incubator mode by re-registering the root edge; never route externals through the root's log in incubator mode.

**Code.** `prover.hpp` / `prover.cpp` — `ExpressionAnalyzer::isAnchorLb`, `collectMailAncestors`, `incubatorAnchorLb`, `anchorOnlyRule`, `relaysAbsorbedHistory` (the routing-mail origin merge in `standardProcessing`), `broadcastTheorems(provedTheorems, originTag, store)`; the `buildGrid` registration loop in `analyzeExpressions`; the two `broadcastTheorems` call sites (prior theorems → `body`, externals → the anchor LB in incubator mode). Tests: `src/tests/test_incubator_mail_routing.cpp`.

<a id="i-221"></a>
## I-221 — the seven rule-index maps gain install records only through the per-instance staging and its flush; a reader never sees an unflushed staging

**Scope.** Every install write into `encodedMap`, `normalizedEncodedKeys`, `normalizedEncodedSubkeys`, `remainingArgsNormalizedEncodedMap`, `remainingArgsOwners`, `originals` and `copyOwners` of a `HashMemory` instance, and every reader of those maps (the phase-2 producer and finalize, the request-generation probes, the firing check, the hashburst dump, the owner-less sweep, the deload dump, the CE clone).

**Rule.** An install (`addToHashMemory`, `makeNormalizedKeysForAdmission`, `installAdmissionMarkerVariants`) writes only through the staging twins into the worker slot's staging bound to the target instance (`ruleStagings.acquire`); the cold maps change only in `ExpressionAnalyzer::flushRuleIndexStaging`, which rebuilds each touched map once in key-id order and mints the keys the map lacked AFTER the rebuild in staging first-touch order. The flush runs at the close of every staging window (`Memory::ruleStagingArmDepth`: the two mail absorbs of `standardProcessing`, `applyEquiClasses`, the exit of the `addToHashMemory` that closes its LB's outermost window, a standalone `makeNormalizedKeysForAdmission`'s exit); a window close (`closeRuleIndexStaging`) flushes ALL FOUR of the LB's instances (a window's nested rounds stage into any of them) and releases their bindings; a remover's entry (`removeRuleFromHashMemory`, `finishRuleRemovals`) flushes its instance mid-window and keeps the binding; a removal acquires no staging and uses the direct doors; `acquire` takes only unbound stagings. The slot's stagings are asserted empty at every seam a reader reaches (`RuleStagingRegistry::slotIsEmpty` at the phase-2 producer and the gen-arena releases; `boundTo` at the owner-less sweep); the staging is never enrolled in `LbMemory::visitContainers`, never deloaded, and empty at every task end. The bytes of every map after a flush equal the bytes the sequential direct doors produce for the same records (the doors survive as the twin oracles).

**Why.** The direct doors splice every record into a dense CSR pool — O(pool tail) per write, 2.79 TB memmoved per full run at 74 % of the run's worker-seconds in the install. One rebuild per map per window turns k writes per key into one, with no tail move (D-333). The reader asserts are the tripwire: a missed flush seam would silently drop rules from the request universe (a missed theorem, no crash), so a reader on unflushed staging must stop the world.

**Spot.** `performElem2`, the gen-arena release sites or `eraseOwnerlessEntries` asserting "staged rule-index writes... a flush seam was missed"; `RuleStagingRegistry::acquire` asserting that every staging of the slot is bound elsewhere; a map whose bytes differ from the direct-door result in `test_rule_index_staging.cpp`.

**Fix.** Add the flush at the seam the assert names (never weaken the assert, never make a reader flush); keep the window closers and the remover entries as the only flush sites; extend the differential tests when a section's merge semantics change.

**Code.** `memory_infra/rule_index_staging.hpp` (`RuleIndexStagingSection`, `RuleIndexStaging`), `memory_infra/rule_index_staging.hpp` (`RuleStagingRegistry`, `ruleStagings`), `memory_infra/cold_hash_map.hpp` (`BlobCsrValueStore::rebuildRunsGenerated`, `HashMap::assignAllRunsGenerated`), `memory.hpp` (`Memory::ruleStagingArmDepth`, `stageLmvIdsRecord`, `serializeLmvIdsBlob`), `prover.hpp` (the `stage*` twins, `planStagedFlush`, `mintStagedKeys`, `flush*Section`, `flushRuleIndexStaging`, the window in `standardProcessing`, the `applyEquiClasses` wrapper), `memory.cpp` (the install sites, the unarmed exit flush), `prover.cpp` (the reader asserts), `memory_infra/lb_deload.cpp` (the dump assert). Tests: `src/tests/test_rule_index_staging.cpp`.

<a id="i-222"></a>
## I-222 — an arena allocation's bytes live at the offset `alloc` RETURNS, never at a cursor mark captured before the call


**Scope.** Every byte-bump allocation on an `LbArena` / `ScratchArena` (`alloc`, `allocBytes`, the `ArenaStack` / `ScratchString` doors) and every caller that later reads the allocated bytes back.

**Rule.** A caller that needs the address of what it allocated keeps the offset (or pointer) the allocation returned. It never reconstructs the address from a `cursor` mark captured before the call: `LbArena::alloc` aligns the cursor up to the requested alignment and pads to the next block boundary when the request would straddle the current block, so the returned offset equals the pre-call mark only when neither happened. A bounded key or record that is built for one lookup / one stage call lives in a caller-owned stack buffer (Rule 28's "stack array for bounded scratch"), not on the arena at all.

**Why.** This is the G-72 root cause ([G-72](50_gotchas.md#g-72)): the remaining-args edge key was built by an arena allocation and read back at the pre-allocation cursor mark at four sites (the install batch, the direct install door, both removal doors). Whenever that allocation wrapped to a new block, the mark pointed at the poisoned tail of the previous block, and the install staged — or the removal looked up — `0xCDCDCDCD…` instead of the key. The wrap depends on where the worker slot's gen-arena cursor sits inside its block, which depends on which LBs ran on that slot before, i.e. on scheduling: a deterministic proof state aborting intermittently, more often under the CUDA route and under RT instrumentation.

**Spot.** A `cursor` mark captured immediately before an `alloc` and then passed to `resolve`; a key or record builder that returns only a length; an owner-list assert (`removeOwnerFromRun: the key is absent`) whose key-family dump shows the arena poison pattern.

**Fix.** Return the allocation's offset alongside the length, or build into a caller-owned bounded buffer (`kMaxRemArgsEdgeKeyBytes` for the edge key); never read back at the mark.

**Code.** `prover.hpp` (`remArgsEdgeKeyInto`, `stageRemainingArgsNormKeyBatch`, `insertRemainingArgsNormKeyBatch`, `removeRuleKeyEntries`, `removeRemainingArgsOwner`), `memory_infra/lb_arena.cpp` (`LbArena::alloc` — the align-and-pad contract), `tests/test_memory.cpp` (`rem_args_edge_key_independent_of_block_wrap`).
