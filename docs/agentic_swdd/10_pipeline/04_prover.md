...
**Phase split + barriers (LB-split refactor, [D-113](../40_decisions.md#d-113) / [D-114](../40_decisions.md#d-114)).** The elementary step is three phase helpers — `performElemPhase1` (pre-hashburst mail absorb), `performElemPhase2` (the hashburst), `performElemPhase3` (post-burst absorb + sanitize). `proveKernel` runs them as **barriered sweeps with a join between each**: phase 1, then phase 2, then phase 3. Phases 1 and 3 are plain across-LB work-stealing sweeps (a local `runPhase` generic-lambda). Phase 2 is the **flat executor pool** ([D-115](../40_decisions.md#d-115)): one work-stealing pool over the flat list of `(LB, part)` tasks across all active LBs (an LB with `numberOfParts == N` contributes N tasks, never nested), each task running `performElem2` — the per-part request-generation + fixpoint unit — appending its firings to its own per-task `SealedPageSet` record chain, with the per-slot scratch arenas as its only other write surface ([I-83](../30_invariants.md#i-83)). After that pool joins, a per-LB finalize sweep (`performElemPhase2`) hands each LB's sealed part sets in part order to `applyFiringRecords` — a pointer-index sort over the chains' stable payload addresses ([I-77](../30_invariants.md#i-77)) — and applies them. The whole pool + finalize is wrapped in an adaptive re-split loop (≤ 2 passes — see *Adaptive split policy* below). The flat pool is what runs one heavy LB's hashburst across N cores. The single-LB orchestrator `performElementaryLogicalStep` (entry `isActive` early-return, `RT_TRACKER_DECL`, all three phases inline) became dead once `proveKernel` called the phases directly and has been **removed** — the phase model is the only path. The hashburst dump now has two traps bracketing the live path: ENTRY at the top of phase 1, EXIT at the bottom of phase 3 (the EARLY-EXIT trap was removed once deactivation deferral made it dead — Rule 14, user-approved). The whole restructure is byte-identical at `splitCount == 1`; the byte-identity rests on [I-28](../30_invariants.md#i-28) (no cross-LB intra-cycle observation; sorted post-join drains).

**Adaptive split policy ([D-111](../40_decisions.md#d-111)).** Each LB carries `Memory::numberOfParts` (N, default 1) and is managed two-state (bang-bang): unsplit (N = 1) or fully split (N = `fixed_number_splits`). Phase 2 is a bounded loop over a `toRun` set. Pass 1 runs every active LB at its current N; each part handles its `id % N == processID` rule slice (via the inert-at-N=1 `partitionAccepts` filter). In the per-LB finalize, the pure helper `adaptiveSplitDecision(N, maxSub, cap, fixedParts, fallbackRatio)` decides:
- an UNSPLIT LB whose single part hit the submatch cap (`maxSub ≥ maxNumberHashRequests`) **escalates** to `fixed_number_splits` and is flagged for redo — its truncated burst is **discarded** (the finalize skips `performElemPhase2` entirely; the read-only phase 2 left the LB pristine, [I-74](../30_invariants.md#i-74)) and re-runs from scratch at the full split in pass 2 of the SAME iteration;
- a SPLIT LB whose busiest part ran below `split_fallback_ratio × cap` (default 10%) **coarsens** back to N = 1 for the next iteration (this burst is kept and applied);
- otherwise it holds.

The loop runs ≤ 2 passes — escalation lifts N above 1, which can never re-trigger the redo gate ([I-75](../30_invariants.md#i-75)). `numberOfParts` persists across iterations (never reset). The per-step delta clears live once per LB in `performElemPhase2`, before the merge, so each part's request generation still reads the deltas. The **incubator** and `disable_lb_split` are forced N = 1 and never escalate; the CE filter never enters `proveKernel`.

The run is **deterministic** (two adaptive runs byte-identical: `theorems.txt` + verifier check total) and **preserves the theorem set** (`theorems.txt` byte-identical to the fixed-100 reference): a deactivating LB proves its goal whether it early-exited at N=1 or ran complete at N=100, and the discard-and-redo guarantees a cap-cut unsplit burst is never applied truncated. The sorted firing-record merge makes a kept burst's deposits partition- and thread-order-independent, so the proof graph does not depend on how requests were split or scheduled — the source of the run-to-run determinism. It is **NOT** byte-identical to fixed-100 at the proof-graph level: at N=1 the burst early-exit ([I-76](../30_invariants.md#i-76)) reshapes a deactivating LB's post-goal auxiliary derivations, so the verifier total sits a handful of checks off fixed-100 (both airtight) — the expected unsplit-vs-full-split difference, matching the original single-LB prover. Determinism and theorem-set preservation are independent of the escalate / fall-back thresholds; they change only how much wasted work happens (a `split_fallback_ratio` above `1/fixed_number_splits` lets a medium LB oscillate 1 ↔ `fixed_number_splits`, wasting one capped unsplit pass per iteration).

**Firing-record capture (LB-split groundwork).** `checkLocalEncodedMemoryStatic` no longer mutates its deposit containers inline. Each firing appends a `FiringRecord` (`memory.hpp`); after the `FIXPOINT_LOOP`, `applyFiringRecords` (`memory.cpp`) sorts the records by a total content key and applies them — head deposits to `sameIterationInternalMail.statements` / `.exprOriginMap` / `.disintegrationSignals` + `canBeSentIds`, marker deposits to `deferredIntegrationPreps` / `canBeSentMarkerIds` / `admissionKeysAlgebra`. This makes the order-sensitive deposits (cap-bounded `addOrigin`, per-head `disintegrationSignals` last-write, the staged-vector drain order) a function of the firing SET, not the request-evaluation ORDER — the determinism the LB split's rule-partition merge depends on. The phase-2 burst performs no deactivation or discharge at all — those run only in phase 3's post-burst `standardProcessing` ([I-66](../30_invariants.md#i-66)). The fixpoint also mints **nothing** into the LB's `NameMap`: the `encode` sites in `checkLocalEncodedMemoryStatic` became the non-minting `NameMap::lookup` ([D-116](../40_decisions.md#d-116)), so the whole hashburst is read-only on the shared LB — the precondition for parallel executor threads. See [D-117](../40_decisions.md#d-117) / [I-77](../30_invariants.md#i-77).
