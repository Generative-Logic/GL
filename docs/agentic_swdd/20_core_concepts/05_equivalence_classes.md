...
- `mb.intStatementLevelsMap` — erase the packed keys that were dropped from the runtime vectors.
...
The newStatements-pair filter (the third operation pre-fix `cleanUpExpressions` performed — dropping non-canonical pairs from the per-equality return vector) is retired: with the L5 id-form flip `cleanUpExpressions` no longer takes a `newStatements` parameter at all (its Step-4b caller passed an empty throwaway and ignored the return, so the branch was already dead). Its live work is the `intEncodedStatements` / level-registry canonicalization.
