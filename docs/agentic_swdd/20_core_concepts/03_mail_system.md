...
- **`mailBlobPool`** — `PagedVector<char>`: every retained committed batch's `Codec<Mail>` blob (statements + exprOriginMap only, global interner ids), append-only until retirement.
