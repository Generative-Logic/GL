...
> **Id-form / mint-at-commit status (current).** All retained mail carries integer ids, not `std::string`. `mailOut` uses one dedicated per-LB int32 `ColdStringTable` for its statements, origin keys, and dependencies; that table deloads with the outbox. `mailIn` and committed blobs use the global `mailInterner` on the never-deloaded mail pool. Internal mail (`ColdMail`) carries per-LB `NameMap` / `originInterner` ids. The commit seam translates the producer's private `mailOutInterner` ids into global ids. The global table is frozen during parallel phases and resets only at the safe rolling-window seam. See [I-91](../30_invariants.md#i-91), [I-101](../30_invariants.md#i-101), [I-102](../30_invariants.md#i-102), `I-127`, `I-162`, and `I-163` ([30_invariants.md](../30_invariants.md)).

> **Statification and retention status (current).** `MailLog` is statified onto a dedicated, never-deloaded **mail pool** ([D-141](../40_decisions.md#d-141), [I-95](../30_invariants.md#i-95)). `mailIn` is `RoutingColdMail` on a self-owned mail-pool arena: phase 1 claim/reloads the LB first, then pulls, absorbs, and clears the inbox under that claim. `mailOut` plus its private interner is `DeloadableMailOut` inside `LbMemory` on the main pool ([D-205](../40_decisions.md#d-205)); the serial commit barrier reads the always-resident `mailOutPending` bit and claim/reloads only producers with output. Internal-mail channels remain deloadable `ColdMail`. Retention has the same two fixed per-batch modes: zero initially dormant LBs rolls delivered history/global ids; any initially dormant LB keeps both for catch-up.

> **Pull-model status.** The push/broadcast routing this chapter historically described — `mailOut → sendMail → per-core boxes → smashMail → every descendant's mailIn` — has been **replaced by a pull model** ([D-137](../40_decisions.md#d-137)) and the push machinery deleted. Each LB stores each emitted batch once in `MailLog`; a receiver pulls un-ingested batches from its `parentMemory`-chain ancestors in `performElemPhase1`, tracking a per-(recipient, ancestor) cursor. A retained ref chain is append-only within its current rolling window, or for the whole execution batch when dormant catch-up is required. `mailIn`/`mailOut` survive as per-LB staging buffers. Delivery, scopes, gates, and latencies are byte-preserved.
...
Every `Memory` has two routing endpoints with different residency. `mailIn` is a mail-pool `RoutingColdMail`; `mailOut` is a main-pool `DeloadableMailOut` containing its own string table. Both carry id-form statements and origins and neither carries disintegration signals or expanded implications. Every `Memory::insertMailOutStatement` / `addMailOutOrigin` write sets `mailOutPending` and refreshes an always-resident logical-byte snapshot. After phase 3 the steward may deload the whole LB, outbox included. The serial commit sweep skips `mailOutPending == false`; otherwise it claim/reloads the LB, serializes the outbox, calls `clearMailOut`, and releases the claim. No persistent heap `Mail` is introduced.
...
- **`nextIterationInternalMail`** — the cross-iteration deferral channel. Filled by the **post-burst** `standardProcessing` call's `dischargeToBeProved` (via `internalMailOut`), and by `addExprToMemoryBlock`'s vacuous-truth path (the registration drains' settled/reformulated-head deposits are removed — the compact broadcast re-derives the head at the chain LB; [D-324](../40_decisions.md#d-324)). Drained at the start of the next step's **pre-burst** `standardProcessing` call, alongside `mailIn`. Lifecycle: cross-iteration; carries one-step-delayed emissions.

**Storage: both internal channels are cold and deloadable.** `sameIterationInternalMail` and `nextIterationInternalMail` are `ColdMail` aliases into `LbMemory`, deload-registered at bases 455 / 505 and `survivesDischarge` ([I-102](../30_invariants.md#i-102)). They carry per-LB id-form statements, origins, and packed `disintegrationSignals`; the always-empty `expandedImplications` column is absent. Writers use the cold doors and the absorb reads the columns directly. A transient heap `Mail` exists only for dump/test materialisation, not persistent execution storage.

**The disintegration-signal record carries three fields** ([D-233](../40_decisions.md)): the two firing-time bools (bits 0-1 of the packed int32) plus the firing's witness-generation stamp — max iteration across the firing's premises, plus one — in bits 2+ as `iteration + 1`, so the all-zero value keeps meaning "nothing carried" and decodes to -1. On a duplicate write for the same (expression, scope) the bools keep last-write (the canonically sorted record apply fixes the order) while the iteration min-merges. The absorb passes the carried iteration into `addExprToMemoryBlock`, where `disintegrateExprCore2` names the statement's witnesses `it_<iteration>_lev_...`; a -1 (non-firing writers, routing mail — which has no signal column) maps to generation 0 at the mint. See [I-171](../30_invariants.md#i-171).
...
1. **Cycle start (phase-1 pull, then pre-burst absorb).** `mailIn` starts empty; the **phase-1 pull** (`MailLog::pull`, see [The pull model](#the-pull-model)) fills it first — walking this LB's `parentMemory` chain and merging each ancestor's un-ingested batches (statements + exprOriginMap) into `mailIn`, then advancing the per-(recipient, ancestor) cursor. `nextIterationInternalMail` holds the previous step's cross-iter deferral emissions (vacuous-truth, post-burst discharge). `mailOut` is empty. `sameIterationInternalMail` is empty (was drained by the prior step's post-burst pass). The **pre-burst `standardProcessing`** call then runs:
 - Drains `mailIn` (status=3) into the statement registries as NON-local rows (`isLocal` is `status == 0 || status == 1` at the `addStatement` door, so mail-arrived rows never enter `intLocalEncodedStatementsDelta` and are never re-forwarded by `fillMailOut`). Status=3 disintegration is shape-gated to the two rule carriers: compact `(implication<N>[…])` forms recover their `(chain, head, …)` and install into `overallHashMemory` + `workingMemory`; negated compact existences `!(existence<N>[…])` (universals in De-Morgan clothing) likewise disintegrate and install their universal rule — witness-free, so the name-mint concern behind the gate does not apply ([D-225](../40_decisions.md#d-225)). Every other entry is absorbed as a bare statement; regular facts land in `overallHashMemory` + `externalStatements`/`intExternalStatements`. **Each row is offered RAW; the deposit door canonicalizes it** ([I-217](../30_invariants.md#i-217)): `addExprToMemoryBlock` rewrites every non-goal, non-equality, non-anchor deposit under the classes at its own scope (`canonicalFormAtScope`, shared walk core with `filterIterationsCore`), ends a deposit whose canonical form is already known, and otherwise registers the canonical form — LOCAL iff an applied equality is a local statement here (then the row rides the local delta and the raw form's mailed origin line ships to `mailOut`), the canonical form gains an `equality1` bridge, the applied pairs' class levels union into the level run — and reports the registered row, from which the status-3 `intExternalStatements` staging is taken only when the row stayed non-local (the retired drain-side `canonicalizeMailArrival` of I-203 is deleted). EXCEPTION: a statement whose mail side column carries flag 5 enters the door at STATUS 5 — full disintegration with witness minting at the carried generation stamp, carrier non-local, admitted products local ([I-195](../30_invariants.md#i-195); the receiver half of the flag-5 relay).
...
4. **Cycle end (retire + interner reset + commit barrier).** After phase 3 joins, rolling mode asserts every `mailIn` is empty, retires the delivered `MailLog` window, then resets the global interner. `mailOut` does not block that reset because it uses its own per-LB id table. The single-threaded commit barrier reads only shell `mailOutPending` bits while LBs may be cold; it claim/reloads each pending producer, appends one batch with global ids, clears the outbox and private interner together, then releases the claim. Post-barrier root deposits explicitly claim/reload the root and retain the existing next-iteration latency.
...
The cross-LB carrier is `MailLog` on `ExpressionAnalyzer` ([`mail_log.hpp`](../../GL_Quick_VS/GL_Quick/src/mail_log.hpp)). Logically it stores each producer's retained batches plus `cursor[recipient, ancestor]`, the cumulative number of batches that edge has ingested. Physically those roles are five cold containers on the dedicated mail pool (the `ExpressionAnalyzer`-owned `mailArena{ &mailMemory }`). All five are pure runtime containers — never in `LbMemory::visitContainers`, so never deloaded, dirty-tracked, or reshuffled. Keys are the `uintptr_t` of a `const Memory*` and are never dereferenced.

The retained batches use three containers forming **independent per-LB chains**. Each chain is append-only inside its retained-history window, so a commit is O(1) and never touches another LB's bytes:

- **`mailBlobPool`** — `PagedVector<char>`: every retained committed batch's `Codec<Mail>` blob (statements + exprOriginMap + the per-statement flag side column, global interner ids), append-only until retirement.
- **`mailRefs`** — `PagedVector<BlobRef>` (`{uint32 start; uint32 len; int32 prev}`): one ref per retained batch; each LB's refs form a newest-first back-linked chain via `prev`.
...
> **Why not one `TypedColdBlobMap`.** A single CSR would concatenate every LB's run, so a commit to any non-tail LB shifts later bytes. Independent per-LB ref chains preserve O(1) commit within either retention mode ([D-141](../40_decisions.md#d-141)).
...
**Lifecycle.**
- **Register and select mode** (single-threaded, at `buildGrid`): every LB pre-creates its ancestor edges and zero cursors. After all LBs exist and anchor prehandling is complete, a one-time scan selects rolling mode iff every `permanentBodies` entry is active. No LB is born after this point.
- **Stage** (`fillMailOut`, unchanged): writes `body.mailOut` in phase 1 and phase 3.
- **Retire** (post-phase-3, pre-commit): rolling mode asserts every `mailIn` is empty, clears delivered blob/ref pages, resets each retained-chain head to `-1`, and resets the global `mailInterner`, while preserving routing, cursors, and cumulative producer counts. Full-history mode skips the complete step. The interner arena reuses its physical blocks, so its peak becomes the largest one-window dictionary rather than cumulative distinct strings.
- **Commit** (the cycle-end barrier): uses `mailOutPending` to claim/reload only producing LBs, appends `DeloadableMailOut` through `MailLog::commit`, then clears the columns and private interner together.
- **Pull** (`MailLog::pull` in `performElemPhase1`): for each registered ancestor, decode the `count - cursor` retained refs straight into `body.mailIn`, then advance the cumulative cursor to the ancestor's cumulative count.
...
**Dormant LBs and the fixed retention gate.** A parked induction-zero LB never runs while dormant, so its cursors stay behind. On wake (`activateZeroCondition`), its first phase-1 pull must catch up its ancestors' whole logs. Therefore any initially dormant LB fixes full-history mode for the whole execution batch. If every LB starts active, every relevant reader pulls a retained window before the post-phase-3 retirement seam; an LB that later deactivates cannot reactivate ([I-112](../30_invariants.md#i-112)). Rolling is therefore safe only for the all-active-at-build case and never switches dynamically ([I-161](../30_invariants.md#i-161)).

**`mailPeek` — the fold-free pending-mail poll (quiescent-burst skip).** `MailLog::mailPeek(recipient)` is the pull's ancestor walk WITHOUT the fold: it returns `true` the instant any registered ancestor's committed batch count exceeds the recipient's cursor for that edge (`mailHeads[A].count > mailCursor[(X, A)]`) — i.e. `pull` would deliver at least one batch — and decodes/merges nothing and advances no cursor. It is the case-B wake of the quiescent-burst skip ([D-194](../40_decisions.md#d-194)): a converged LB with pending mail must still be swept so its phase-1 pull runs. Because it reads only the never-deloaded mail pool (`mailHeads` / `mailEdges` / `mailCursor`, [I-101](../30_invariants.md#i-101)), it never forces a reload of a cold recipient and never reads residency ([I-106](../30_invariants.md#i-106)). It is evaluated single-threaded at `proveKernel`'s active-build — the same program point the pull runs at the start of phase 1 — so a cross-LB commit at iteration N wakes the recipient at the N+1 active-build, the identical iteration the reference would pull: no mail latency is added ([I-55](../30_invariants.md#i-55) preserved).

### Three behaviour-preservation rules

1. **Load-time broadcast → store once.** `broadcastTheorems` (previous-batch externals + proved-theorem rules) and the `buildGrid` startup dispatch reach *all* LBs, not just one producer's descendants. They store the seed batch once in the **root's** log (every descendant pulls it on the normal walk) and **self-inject the root's `mailIn`** directly (the root has no ancestor to pull from). Single-threaded at load, before any pull, so the batch is in place for burst 1. **Incubator batches** ([D-332](../40_decisions.md#d-332), [I-220](../30_invariants.md#i-220)): an LB strictly below the anchor LB registers no root edge, so the root's log — the in-run compacts, the theorem-origin rows, the load-time batch of theorems proved by earlier incubator batches — is read by the anchor LB alone (nested incubator-derived theorems that `anchorOnlyRule` rejects are additionally stored in the anchor LB's log, at load and in-run); the externals batch is stored in the anchor LB's log (`broadcastTheorems`'s `store` parameter) and self-injected there, so it still reaches the anchor LB and every LB below it. The anchor LB relays the root batches' origin rows (history only, never the statements) into its own `mailOut.exprOriginMap` at its status-3 absorb (`relaysAbsorbedHistory`), so an LB below the anchor keeps receiving the compacts' `compilation` rows and the `theorem` rows its chapters cite, one cycle later.
...
3. **Field coverage.** `statements` + `exprOriginMap` + the per-statement flag side column (`statementFlags_`, [D-284](../40_decisions.md#d-284)) reach `mailIn`; the heap `mergeBatchInto` twin carries only the first two (heap `Mail` has no flag column — the heap `Codec<Mail>` paths emit an empty flag section and drop a decoded one, exactly like `disintegrationSignals` stays internal-channel-only). Copying the whole `Mail` would deliver `expandedImplications` cross-LB for the first time — a forbidden semantics change (Rule 8).
...
**Statified race-safety (current).** The pull is race-free by three properties: (1) retained mail-pool pages are frozen throughout each parallel phase; retirement and commit occur only after the join; (2) every read accessor and `Codec<Mail>::deserializeInto` is pure during the pull; the global mail interner is frozen and never mints in parallel; (3) the cursor advance uses `setValueAtRelaxed`, writing only the recipient's disjoint value slot and skipping the shared dirty flag. The next retirement cannot run until all workers have finished reading the current window ([I-94](../30_invariants.md#i-94), [I-161](../30_invariants.md#i-161)).
...
### The shape gate (`allowedForMail`)

Besides the validity gate, every statement deposit passes the shape gate `allowedForMail` (`prover.cpp`). Decision order:

1. **No `int_lev` token** (`scanSingleDistinctIntLev` verdict 0) → mailable.
2. **Parent-level pass** ([D-284](../40_decisions.md#d-284)): every `int_lev` token's embedded level is STRICTLY BELOW the LB's own `level` (`allIntLevLevelsBelow`, `str_ops.hpp`), and the text is not `(>`-shaped → mailable. Such names can only have arrived by ancestor mail ([I-57](../30_invariants.md#i-57)), and every ancestor deposit is pulled by all of this LB's descendants directly, so the names are known below by construction. Before this pass existed, a mail-arrived premise flunked the gate at the receiving LB, which broke the `allGood` transitive-mailability chain for fired heads over mailed premises — the C6 stall's Gap B.
3. **Whole-expression memo** — non-minting `canBeSentIds` probe (note: this precedes the count bail, so a multi-token expression minted via the `allGood` firing route passes).
4. **Count bail** — two or more distinct `int_lev` tokens not caught above → refused (own-level witnesses whose carriers never qualified are genuinely unknown below).
5. **`(>`-prefix refusal** (rules travel only as compacts, [I-54](../30_invariants.md#i-54)), then the **marker-form memo** (`canBeSentMarkerIds`, non-atomic category only).

### Flag-5 relay staging (sender side)

When a local (status-0/1) disintegration leaves an existence group without any admitted witness, `disintegrateExpr2` selects the HIGHEST such groups — an uncovered group whose enclosing group is covered, or the top compound (`selectRelayWitnesses` over the per-witness records `WitnessMetaStore` collects at the mint sites; `prover.hpp`) — and returns their instantiated compacts on `DisintProducts::relayCompacts`. `addExprToMemoryBlock` stages them (main scope only, never from a mailed status-3/5 deposit) into the per-LB `pendingRelayIter` / `pendingRelayLevels` side tables (deload tags 811..815), keyed by the compact's NameMap id with the disintegration's witness-generation stamp (min-merged) and the staging deposit's level run.

`fillMailOut` consumes the staging: a shipped delta statement found in pending-relay gets flag 5 + the generation stamp on its deposit (`Memory::setMailOutStatementFlag`); staged compacts that did not ship through the delta (unregistered nested compacts, or ones whose gate qualified later) are direct-deposited in decoded-lex order, gated by `allowedForMail`, with the staged level run, the flag, and the same per-key origin mirror the delta loop ships (`copyOriginRowsToMailOut` — the D-45 paired-origin requirement at receivers). Shipped entries erase; refused candidates stay staged and re-try next pass. See [D-284](../40_decisions.md#d-284); the receiver half is the status-5 absorb.
...
**Canonicalization-dropped rows still ship their history** ([D-286](../40_decisions.md#d-286)). `cleanUpExpressions`' `filterIterations` sweep prunes statements carrying a non-canonical equivalence-class member from the local lists AND the per-step delta — and it runs earlier than `fillMailOut` in the same phase-3 `standardProcessing` call, so a statement registered and dropped within one burst would otherwise never be walked by any mail pass. Since `equality1` rewrite rows citing such a statement DO mail, the drop site calls `copyOriginRowsToMailOut` for each dropped delta row before removing it: the statement stays unmailed (the canonicalization policy is unchanged) but its origin history reaches every descendant, keeping the receiver-side citation chains closed. Without this, a receiver that cannot re-derive the cited statement — an induction-zero block that only catch-up-absorbs mail — holds a permanently dangling citation and `buildStack` asserts (the C6 Step-10 export crash).

**Or-converged branch derivations ship before the per-branch removal** ([D-311](../40_decisions.md#d-311)). `ordisMerge`'s convergence step 2 removes the converged expression from every branch scope with `removeExpressionFromMemoryBlock(state=0)`, which erases the branch rows from the per-step delta — and the cohort's last-released branch ([I-174](../30_invariants.md#i-174)) converges in the very step it derives, so that branch's `equality1` row would never be walked by any mail pass while the `or convergence` row citing it ships. Same shape as D-286, same cure: `copyOriginRowsToMailOut` for each branch derivation before the removal. Without it a descendant walking the mailed convergence row — the IncubatorGauss3 contradiction LB under `(EnumerationSet2[2,6,10])` — holds a permanently dangling citation for the last branch and `buildStack` asserts.
...
> **Historical — retired 2026-05-25 (see [D-92](../40_decisions.md#d-92)).** The transitive walk described below was the first attempt and is no longer in the code: `shipExprHistoryTransitively` is an empty stub whose former body is kept under `#if 0` as the record. The current shipment contract is the delta loop plus the sanctioned direct writers listed in [I-64](../30_invariants.md#i-64) — ship-before-drop at every delta-dropping site ([D-286](../40_decisions.md#d-286), [D-311](../40_decisions.md#d-311)).

**The shipment contract — transitive walk in `fillMailOut` (historical).** For each delta entry that fillMailOut ships, the function (a) writes the entry's own origin lines to `mailOut.exprOriginMap`, and (b) walks every dep referenced by those origins via `shipExprHistoryTransitively`. The walk is recursive:
...
**Consumers.** The hashburst entry absorbs `sameIterationInternalMail` at the top, immediately before the legacy `mailIn` absorb, with `status=1` (full disintegration pipeline — unlike legacy `mailIn` whose `status=3` disintegration is shape-gated to the two rule carriers, compact implications and negated compact existences; see [D-19](../40_decisions.md#d-19) rationale and [D-225](../40_decisions.md#d-225)).
...
- **Statification-phase concurrency (RESOLVED).** `MailLog` is race-free without a lock: retained pages are frozen during parallel pulls; retirement and commit are single-threaded after the join; the global mail interner is frozen during pulls; and `setValueAtRelaxed` advances disjoint cursor cells without a shared dirty write.
- **The externalStatements leak (FIX PENDING).** Mail arrivals are never equivalence-class-canonicalized at the door: the absorb drain's `status == 3` external-mail staging pushes each mailed statement verbatim into `intExternalStatements` ("raw mailIn expressions 1:1"), which `performElem2`'s request-generation batches 3/4 pair directly — so a non-canonical class member mailed from an ancestor participates in the hashburst for its arrival burst, bypassing the registry's one-representative collapse (the parallel `addStatement` registry offer IS class-filtered, including the C6-era swallow-without-substitute eqfilter gap — two faces of the same missing door-side canonicalization). Full write-up, evidence, and the agreed fix direction: `docs/fta_ladder/C_RT_investigation.md`, section "The externalStatements leak".
...
- **Over-pull to all descendants.** A proven rule is available to every descendant (each pulls the root's log), not just those that need it — memory traded for simplicity, as before. Resolved for incubator batches by [D-332](../40_decisions.md#d-332) (the root's log reaches only the anchor LB); still the case for the main batches.
...
- [`mail_log.hpp`](../../GL_Quick_VS/GL_Quick/src/mail_log.hpp) — the `MailLog` containers and `registerLb` / `commit` / `pull` / `retireDeliveredBatches` lifecycle.
