...
| `varsInRejectedMapIntegrationKeys` | `TypedColdSet<NameId>` (cold cache) | Monotonically-growing set of non-marker args appearing in any `rejectedMapIntegration` key. Used by the equi-class-rewrite helper for fast class-overlap short-circuit; cold-stored, never wiped. `NameId` is the current 32-bit identifier type. |
| `admissionMapOrdis2` | `TypedColdBlobMap<int64_t, AdmissionMapOrdis2Value>` (cold) | The admission half of the ordis2 demand/park PAIR ([D-267](../40_decisions.md#d-267), pure ordis — no machinery shared with the algebra or integration maps): key = packed `(templateId, validityId)` whose template half is a fully-GROUND compound premise text (polarity verbatim, I-175) — the one missing premise of a rule that matched everything else. Populated at the post-fixpoint drain when an `ordis2Demand`-tagged marker variant fires; its drain wakes `rejectedMapOrdis2` under the SAME key language, and `consumeOrLeavesCohort`'s route (c) probe consumes the demand on cohort open. Value = `{sourceImplId, levels}` audit context; facets base+63..66; scope-wiped like the other rejection buffers; equi hook `applyEquivalenceClassToAdmissionMapOrdis2` (drop-and-rekey) at both `applyEquiClasses` seams. |
| `rejectedMapOrdis2` | `TypedColdBlobMap<int64_t, RejectedMapOrdisValue>` (cold) | The rejected half of the ordis2 pair: the SAME or heads that park in `rejectedMapOrdis` additionally file here under each ELIGIBLE disjunct's clean GROUND text (compiled non-atomic entity — predicates included, `operators` not consulted; arity ≥ `kOrdis2DemandMinArity`; polarity verbatim) at the cohort parent's validity — the demand map's key language, so `drainAdmissionKeysOrdis2`'s wake (`revisitRejectedOrdis2`: snapshot → erase key → un-know → re-mail) is a plain key rendezvous. Value family REUSED from `RejectedMapOrdisValue`; facets base+67..70 + the dedicated `ordis2RevisitInProgress` guard at base+71; scope-wiped; equi hook `applyEquivalenceClassToRejectedMapOrdis2` (I-37 drop-and-mail) at both seams. The algebra key-gain seams never touch it (different key language — a cross-probe would double-wake operator-application disjuncts). |
| `normalizedEncodedKeys` | cold owner-run blob map | `TypedColdBlobMap<NormKey, RuleOwnerRec>`: every whole key → the sorted-unique run of the rules that installed it (`RuleOwner` = packed `(originalImplicationId, validityId)`, added by `addWholeKeyOwner` → `addOwnerToRun` at the install sites). The request generator's emission gate: `wholeKeyPresent` is a bare lookup (the owner run is never read on the hot path). A rule's removal deletes its owner; an owner-less key is erased at the end of the apply (`eraseEmptyRuns`); never scope-wiped — a key whose rules left through a subtree wipe keeps their owners, its request reaches the firing site, whose wiped `encodedMap` misses it ([I-49](../30_invariants.md#i-49)). Five facets: key Lengths + Bytes (5/6), owner-run RunStarts + BlobStarts + BlobPool (72..74). See [D-71](../40_decisions.md#d-71), [I-100](../30_invariants.md#i-100), [D-312](../40_decisions.md#d-312). |
| `normalizedEncodedSubkeys` | cold record blob map | The cold `TypedColdBlobMap<NormKey, OwnerSet>` record map — the fast-rejection index over every growable subkey, which decides whether a candidate may be extended. The record is the growth probe's data followed by the owner list: below `kSubkeyUCheckMinElements` (3) premises the signature-free record (`addShortSubkeyOwner`: loose byte, zero signatures, every owner loose); from three premises on the loose byte plus the set of distinct u_ signatures (`mergeSubkeySignatures`) plus the (owner, signature index) pairs (one rule may hold several: its permutations' prefixes carry different u_ literals; -1 loose). The growth probe `subkeyUSatisfied` is lookup + the D-120 signature check from three premises on and never reaches the owner section; scope comparability and partition are not read. A rule's removal deletes its owner (and its signature when no surviving owner shares it); an owner-less subkey is erased at the end of the apply; never scope-wiped — a stale subkey is a sound over-approximation the whole-key probe corrects. The `…MinusOne` / `…MinusTwo` siblings, which recorded a key minus one and minus two elements for the obligatory-stump batches, are deleted ([D-295](../40_decisions.md#d-295)). See [I-49](../30_invariants.md#i-49), [D-120](../40_decisions.md#d-120). |
| `remainingArgsOwners` | cold owner-run blob map | `TypedColdBlobMap<IdVecKey, RuleOwnerRec>`: one owner run per `(arg-set, NormKey)` EDGE of `remainingArgsNormalizedEncodedMap`, keyed by the edge's bytes (`remArgsEdgeKeyInto`: the arg-set id run followed by the NormKey blob's id run). Written by `insertRemainingArgsNormKeyBatch` for every distinct edge of a batch, new or already present in the forward run; the forward run stays a pure NormKey run and the derived `ReverseArgsIndex` is untouched ([I-154](../30_invariants.md#i-154)). The last owner leaving an edge removes the NormKey from the forward run and the reverse-index edge. Five facets at 78..82. |
| `copyOwners` | cold owner-run blob map | `TypedColdBlobMap<IdVecKey, RuleOwnerRec>`: one run per multiplication COPY (key = the packed copy owner `(copy text's ruleInterner id, scope)` as eight bytes) naming the RULES `(recorded text's ruleInterner id, scope)` whose install produced that copy. `addToHashMemory` lists the rule on every copy it installs; under `RuleIndexOp::Remove` it delists the rule first and touches the copy's index entries and LMVs only when no other rule owns the copy — under `allow_multiplication` two different rules can multiply into one copy text (heads differing only in the order of the equated variables), and the copy's index owner is the copy, not the rule ([I-49](../30_invariants.md#i-49)). Five facets at 83..87. |
| `originals` | cold owner-run blob map | `TypedColdBlobMap<IdVecKey, RuleOwnerRec>`: every installed implication chain (`premises ++ head` as `ruleInterner` ids) → the run of the rules that installed it (the same chain text at two scopes is two owners). Read by the integration premise matcher and the dump; a chain whose last owner leaves is erased. Five facets: key Lengths + Bytes (51/52), owner run 75..77. |

**Three key shapes, five maps.** Admission MAPS (`admissionMap`, `admissionMapIntegration`) are u_-prefixed templates. The admission SET (`admissionSetIntegration`) and rejection MAPS (`rejectedMap`, `rejectedMapIntegration`) are bare concrete instances. Lookups must transform at the boundary — u_-prefix when querying the maps, strip u_ when querying the set/rejection side. Storage keying: the algebra group (`admissionMap`, `admissionStatusMap`, `revisitInProgress`) is packed `(templateId, validityId)` via the dedicated `Memory::templateInterner` ([D-132](../40_decisions.md#d-132)) — write sites `mintTemplateKey`, probes the non-minting `lookupTemplateKey` (a never-interned template is a definitive miss); the key-SHAPE contract above is unchanged, the shapes are the interned strings. The algebra group + `varsInAdmissionMapKeys` AND the integration twins (`admissionMapIntegration` / `rejectedMapIntegration` / `varsIn{Admission,Rejected}MapIntegrationKeys`) are now COLD-stored on the cold-map family (`admissionMap` / `rejectedMap` / `admissionMapIntegration` / `rejectedMapIntegration` → `TypedColdBlobMap`, the satellites → `TypedColdSet` / `TypedColdMap`; [I-99](../30_invariants.md#i-99)). The admission SET side (`admissionSetIntegration` / `triggersForAdmissionSetIntegration`) stays on the heap. See [`10_pipeline/04_prover.md`](../10_pipeline/04_prover.md#admission-rejection-map-key-shapes) for the full table and [G-32](../50_gotchas.md#g-32) for the silent-miss pitfall.
...
1. **Generate hash requests.** One function does this — `generateEncodedRequestsStatic` ([`memory.cpp`](../../GL_Quick_VS/GL_Quick/src/memory.cpp)), and every caller now drives it the same way: with a list of **mandatory-containment terms**. The candidate is grown to a whole key and emitted only when it holds a statement from every view of at least one term. An **empty term list is a first-class input**, not the absence of one — it says no ingredient is mandatory, so every whole key is a request; the CE filter and batch 5 both pass it. The obligatory stump, its builder and the pairing merge are deleted ([D-295](../40_decisions.md#d-295)), so the target map is always `normalizedEncodedKeys`, the grow depth is always `maxKeyLength`, and there is no seed and no merge phase.

 It emits `IntNormalizedKey` objects that correspond to each candidate match.


 Under the LB split's second dimension the call also carries a **bucket of split stumps** ([D-203](../40_decisions.md#d-203)). The search then runs once per stump in the bucket over one shared filtered statement list. The stump IS the growing candidate: the run seeds its depth-first stack with that node and grows it onward, so it owns exactly that node's subtree ([D-283](../40_decisions.md#d-283)). The stump alone is probed once before the search, because unsplit that candidate is recorded in the loop of the candidate one level up, which a stumped sub-part never runs.
 **Each candidate is built from its predecessor ([D-296](../40_decisions.md#d-296)).** Inside one stack pop of the grow search the first k premises are fixed and only the appended one walks the candidate positions, and both per-node computations are monotone under that suffix append. The normalized key renumbers arguments in first-appearance order, so the prefix's key is a strict PREFIX of the child's and appending can only mint slots the prefix had not used; the request gates are order-free conjunctions, so each collapses to a summary a premise folds into one at a time. The search therefore prepares the prefix once per pop — `buildIntNormalizedKeyPrefix` into the shared key buffer, `foldPrefixIntoRequestGates` into the gate summary — and each position rewinds the carried counters and folds in its own premise. One per-expression body serves both the whole-candidate entries (`makeIntNormalizedKeyFromEncoded`, `requestGatesPass`) and the resume path, so the key language has no second implementation to drift. The rewind must cover every carried field or the keys silently stop matching a full rebuild ([I-202](../30_invariants.md#i-202)). The two owner-set probes below are untouched by this: a whole key is hashed and byte-compared once per node however it was built.
...
**Validity-comparability prune at request generation ([D-105](../40_decisions.md#d-105)) — RETIRED.** The prune and its predicate `ownerSetHasComparable` are deleted ([D-304](../40_decisions.md#d-304)): no probe reads an owner scope. The firing gate `checkLocalEncodedMemoryStatic` enforces scope comparability exactly, which is all the prune ever relied on.

**Rule-partition filter at request generation ([D-119](../40_decisions.md#d-119)) — RETIRED.** `partitionAccepts`, the composite owner ids and the `g_splitProcessID` / `g_splitCount` thread-locals are deleted ([D-304](../40_decisions.md#d-304)); production never left `g_splitCount == 1`, and the expression (bucket) split is the only split dimension.

**u_ literal prune at request generation ([D-120](../40_decisions.md#d-120)).** At the subkey growth probe alone, from `kSubkeyUCheckMinElements` premises on (the whole-key record probes read key presence only since [D-305](../40_decisions.md#d-305), and the whole-key record carries no signature at all since [D-304](../40_decisions.md#d-304)), `ExpressionAnalyzer::ownerSetUSatisfied(OwnerSet, exprs, count)` drops a request whose concrete argument values can satisfy no owner's u_ (unchangeable) literals. The two `normalizedEncoded*` maps are `ignoreU=false`, which normalizes every argument to a positional id and so **erases** a rule's u_ literal values — only the repetition pattern survives — so a structurally-matched request may be doomed by a u_ literal it can never match, a mismatch the firing gate (`encodedMap`, `ignoreU=true`) would otherwise catch only later. `recordUSignature` caches each owner's u_ signature — the `(linear-arg-slot, argFullId)` list of its unchangeable args, where `argFullId == NameMap::encode(arg[1])`, exactly the value the firing gate matches against the request's `argFullId` — at insert in `OwnerSet::uSignatures`. An owner with zero u_ args (or, conservatively, a u_ literal not yet interned at insert) sets `OwnerSet::hasLooseOwner`, which short-circuits the check to *keep* (the "0 u_, nothing to check" fast path). At match the request's flattened `argFullId` is compared against the cached signatures and the request is dropped only when no owner can be satisfied. The literal id is read via the non-minting `NameMap::lookup` at insert and never `encode`d at match, so the cache build mints nothing (byte-identical) and the match is read-only (safe on the shared LB the split executors read in parallel). Like D-105 it never changes which rules fire — the firing gate's `encodedMap` lookup enforces the exact u_ literal match — so it is a sound over-approximation ([I-79](../30_invariants.md#i-79)). Cost is O(1) on the common all-loose key; the subkey map is never scope-wiped, so a signature can go stale (a stale entry only weakens the prune, never makes it unsound).
...
**Both ends of the firing pipeline use `nm.comparable` for scope inheritance ([D-55](../40_decisions.md#d-55) Part C, 2026-05-09).** Request generation has always honoured ancestor/descendant inheritance via the `pairMap`-backed `nm.comparable` (see the four-call fan-out below). The firing site (`checkLocalEncodedMemoryStatic`'s head-LMV scope check) was on a stricter strict-eq + main-mask rule until `D-55`'s Part C: pre-fix, a rule at scope `S` could only fire on facts at scope `S` or at `"main"`, even though the matching request had already been emitted under the broader comparable rule. The mismatch was masked while every load-bearing rule of this shape was a v=main universal (chapter-6 / chapter-11 mirror reformulations). When `D-55` Part B removed those parents from `theorems.txt` and the K-impls took over the role at the OR's non-main scope, the strict-eq mismatch surfaced and rejected legitimate cross-scope firings (rung-1 stalled at burst 5 toBeProved=4). Part C realigned the firing-site check to `nm.comparable`, matching the request-generation rule. Both ends now agree on what scopes are mutually visible.

This is encoded in the fan-out at request generation. `performElem2` ([`prover.cpp`](../../GL_Quick_VS/GL_Quick/src/prover.cpp)) calls the one generator `generateEncodedRequestsStatic` four times, each call pairing a rule pool with a mandatory-element source, so that every emitted request contains at least one mandatory element and the mandatory source decides whether the request can be local-free.

The former batches 2 and 3 are now ONE call under the mandatory-containment control (`D-298`): they always read the same rule pool and the same statement universe and differed only in what they obliged a request to contain, which the terms now state directly. The other three batches are on the same control (`D-295`) but keep their own calls: each reads a DIFFERENT rule pool, and the rule pool decides the key length, the target map and every owner probe.

| # | Rule pool | Mandatory ingredient | Local mandatory? |
|---|---|---|---|
| 1 | `body.workingMemory` | `body.intLocalEncodedStatements` | ✓ |
| 2+3 | `body.overallHashMemory` | `body.intLocalEncodedStatementsDelta`, OR (`body.intExternalStatements` AND `body.intLocalEncodedStatements`) | mixed |
| 4 | `body.localHashMemory` | `body.intExternalStatements` | ✗ mail (v=main) |
| 5 | `body.localHashMemoryDelta` | none — empty term list | ✗ |

Batch 5 is the one with no mandatory ingredient at all. Its rule pool is what is new, and a rule that lands this burst has to meet every visible statement, not only the ones that arrived with it — so every whole key of that pool is a request. Naming the whole statement universe as a term would say the same thing and then pay a per-candidate mask test that can never fail.

Batches 4 and 5 are the asymmetric ones — **deliberately** non-local:

- **Batch 5** ([`prover.cpp`](../../GL_Quick_VS/GL_Quick/src/prover.cpp)). When a fresh rule lands in `localHashMemoryDelta` this cycle, it must fire against every pre-existing visible statement, not only against statements that arrived this cycle. Hence it names no mandatory ingredient at all.
...
**What this implies for disintegration.** Emission of these firings is fine. **Disintegration** of them at a non-anchor LB mints fresh `it_*` / `int_*` variables and historically exhausted the former 16-bit NameMap in incubator + `!ban_disintegration` mode. Name identifiers are now 32-bit, so that particular ceiling is gone, but the uncontrolled non-local disintegration remains a semantic and runtime fan-out error. The D-29 clause-2 gate is the architectural answer — block disintegration when none of the matched premises is in the LB's `intLocalEncodedStatementsSet` (an O(1) packed-key probe on the request's integer rows since D-129). The gate is *not* redundant with request generation; it corrects for Batches 4/5 at the disintegration site rather than tightening request emission.
...
### The install writes are staged per absorb window (D-333)

Every record an install produces for the seven index maps — the head and marker LMVs of `encodedMap`, the whole-key / originals / copy-owner / remaining-args-owner runs, the subkey `OwnerSet` records, the remaining-args `NormKey` runs — goes through a staging twin (`stageLmvIdsRecord`, `stageOwnerToRun`, `stageWholeKeyOwner`, `stageShortSubkeyOwner` / `stageSubkeySignatures`, `stageRemainingArgsNormKeyBatch`) into the worker slot's staging bound to the instance (`memory_infra/rule_index_staging.hpp`, `ruleStagings.acquire`), not into the cold map. The window's closer — the end of the two mail absorbs in `standardProcessing`, the end of `applyEquiClasses`, or the exit of the `addToHashMemory` that closes its LB's outermost window (`Memory::ruleStagingArmDepth` back to 0: LB seeding, CE facts, the compressor) — calls `ExpressionAnalyzer::closeRuleIndexStaging`, which flushes the LB's four instances (`flushRuleIndexStaging`; a window's nested rounds may have staged into any of them) and releases their bindings. The flush rebuilds each touched map once in key-id order — in place and backwards from the end (`HashMap::assignAllRunsGenerated` → `BlobCsrValueStore::rebuildRunsInPlace`: one emit pass buffers the produced runs, then only the tail behind the first produced key is rewritten; carried-over stretches move once as ranges) — with every key's run merged as the sequential doors would have merged it, then mints the new keys in first-touch order. A removal (`removeRuleFromHashMemory`) and the end-of-apply compaction (`finishRuleRemovals`) flush first and use the direct doors; the slot's stagings are asserted empty at every seam a reader reaches (the phase-2 producer, the gen-arena releases, the owner-less sweep). The cold bytes after a flush are byte-identical to the sequential result — the direct doors survive as the twin oracles the differential tests run (`test_rule_index_staging.cpp`).

Per multiplied copy, `KeyEncodeMemo` (memory.hpp) encodes each premise text once, lazily at the first read (the NameMap first-touch order is unchanged); every permutation, every prefix and every admission derivative reads it, and each permutation's normalized key is built once incrementally (`appendExprToIntNormalizedKey`) with the prefix keys taken as the slots written so far. A status-3 install (the incubator's external-mail absorb) writes `overallHashMemory` and the per-burst `workingMemory` from ONE such build: `addToHashMemory` takes the working instance as an optional `secondTarget` and stages every record to both instances' stagings (the second target's admission triggers and admission passes run against its own maps, after the first's, per copy).
...
**Path 2 — derivative LMV insert** (`memory.cpp`, inside `makeNormalizedKeysForAdmission` called from `addToHashMemory`). For each selectable non-anchor chain element, build an LMV keyed by a derivative sub-key and carrying either the marker-form missing premise (regular and `ordisOnly`) or the premise verbatim (`ordis2Demand`). When the actual rule has an `Anchor*` premise, every derivative sub-key retains it, including the regular route when the anchor mentions the selected output argument. The anchor is never selected as the missing premise. Regular, `ordisOnly`, and `ordis2Demand` qualification remains based on the same non-anchor premises; only the installed key gains the anchor context. Normalization is unchanged and runs over the complete derivative key, so equality to a particular anchor slot survives alpha-renaming ([I-193](../30_invariants.md#i-193)).

The two paths share the map but populate disjoint metadata. Head LMVs carry full `originalImplication` / `justification` / `validityName` for provenance; derivative LMVs leave those fields default-empty (the entry is structural — it represents the actual rule with one non-anchor premise missing). When adding new fields to `LocalMemoryValue`, ask: do I want this on head firings, on derivative probes, or both?
