...
1–3. The per-LB derived containers move onto the cold DELOADABLE arena: routing mail (`mailIn`/`mailOut` as `RoutingColdMail` — landed on the never-deloaded MAIL POOL, [I-101](30_invariants.md#i-101), because the commit barrier reads every body's `mailOut` incl. evicted LBs), `changedClassesThisStep` (tag 655) and `eqClassNameCaches` (tag 705) ([I-125](30_invariants.md#i-125), retiring [I-120](30_invariants.md#i-120)).
...
**Why cold, not hot, not the mail pool.** Both channels are non-empty at the deload seam — `nextIterationInternalMail` carries one-step-delayed emissions across it, and `sameIterationInternalMail` carries the end-of-burst `sanitizeHashMemory` rewrites (written after the post-burst clear) to the next step — so they MUST travel with the LB through deload. The HOT path (`mailIn`/`mailOut`) loses cross-burst content; the never-deloaded mail pool (`MailLog`) is for cross-LB resident-forever state. The cold deloadable path is the only fit. The one spot that is not a pure heap mirror, forced by cold storage: the cross-LB `nextIter` deposits (`updateGlobal*`) `ensureLoaded` the recipient and skip a `dischargedForever` target (dead deposit; `ensureLoaded` asserts on a discharged LB). The planned separate "residency guard" commit was folded into the flip — on the still-heap path that guard is premature (the heap write needs no residency), so it is only honest with the cold write.
