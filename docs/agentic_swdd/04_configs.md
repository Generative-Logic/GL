...
| `ConfigIncubatorGauss3.json` | Rung-1 incubator batch on the 9-slot `AnchorIncubator3` (i0..i3 only). Carries `EnumerationSet2`; sets `allow_disintegration=true`, `allow_multiplication=false`, `incubator_mode=true` — the combination needed for the FTA-ladder rung-1 §4.1 attempt. Renamed from `ConfigIncubatorGauss1.json` when the incubator anchor was split into a 9-slot AI3 and a 14-slot AI8. |
...
| `max_count_per_conjecture` | int | Cap on copies of this predicate in one conjecture. `0` means no cap / skip (interpretation context-dependent). |
...
| `max_size_expression` | int | Alternative single-limit form used in incubator configs (replaces the before/after split). |
...
| `allowed_combinations` | array | Sometimes-present whitelist of allowed arg-position combinations. |
...
| `maxNumberSecondaryVariables` | int | `2` | Cap on secondary variables produced during Pass B admission. |
...
| `maxIterationNumberVariable` | int | `1` | Max iteration index for Pass B-minted variables. |
| `standardMaxSecondaryNumber` | int | `1` | Standard admission-path secondary-variable cap. |
| `standardMaxAdmissionDepth` | int | `0` | Standard admission recursion depth. |
...
| `max_or_depth` | int | `1` | Max nesting depth of OR scopes (1 = no nested ORs). |
| `max_partition_size` | int | `5` | Cap on `multiplyImplication` Bell-partition size. |
| `maxNumberHashRequests` | int | `40000` | Per-part **submatch** cap (replaces the former emitted-request cap of `8192`). A hashburst part stops generating once its submatch count — `preEvaluateFromEncoded` matches, enforced by `BurstSink::canAccept` and the `growBaseCandidates` grow-DFS bail — reaches this; the busiest part's submatch count over this cap is the split-policy fill ratio. `int` so it can exceed the int16_t ceiling (32767). The CE filter ignores it (runs uncapped). See [D-109](40_decisions.md#d-109). |
| `disable_lb_split` | bool | `false` | Diagnostic / RT-profiling flag. When true, `proveKernel` runs every LB unsplit (one phase-2 part) so the per-call `RTTracker` homed in `performElem2` measures the LB's whole hashburst (one `.rt/<chain>.log` per LB). Equals the parallel path except a runaway LB exceeding `maxNumberHashRequests` truncates at the cap — a profiling run, not proof-complete. Default false leaves the production parallel path untouched. See [D-110](40_decisions.md#d-110), [I-59](30_invariants.md#i-59). |
| `fixed_number_splits` | int | `100` | Escalation target for the adaptive split: a main-path / compressor LB jumps to this many parts when its UNSPLIT burst hits `maxNumberHashRequests` (its truncated burst discarded and re-run at this split the same iteration), and stays here until it falls back. Every such LB starts unsplit (`numberOfParts` default 1). The **incubator** runs unsplit (gated on `incubator_mode`) and never escalates; the CE filter is unaffected (its own un-split loop, `splitCount = 1`). See [D-111](40_decisions.md#d-111). |
| `split_growth_factor` | int | `2` | **Unused** no-op field. Was the busy-band multiplier of the graduated `computeNextNumberOfParts` policy, removed in favour of the bang-bang `adaptiveSplitDecision` (1 ↔ `fixed_number_splits`). Retained only so existing config files keep parsing. See [D-111](40_decisions.md#d-111). |
| `max_number_splits` | int | `32` | **Unused** no-op field. Was the upper bound of the removed graduated `computeNextNumberOfParts`; the bang-bang `adaptiveSplitDecision` never exceeds `fixed_number_splits`, so a separate bound no longer applies. Retained only so existing config files keep parsing. See [D-111](40_decisions.md#d-111). |
| `split_fallback_ratio` | double | `0.10` | Fall-back threshold for the adaptive LB-split policy: a split LB (`numberOfParts > 1`) coarsens back to unsplit (`1`) for its next burst when its busiest part's submatch count is below this fraction of `maxNumberHashRequests`. The escalation direction is not ratio-gated — an unsplit LB jumps straight to `fixed_number_splits` the moment its single part hits the cap. Consumed by `adaptiveSplitDecision`. See [D-111](40_decisions.md#d-111). |
| `static_pool_bytes` | int64 | | Statification sizing: total bytes of the ONE static-memory reservation made at program start (never freed, never grown). Raised 2 GiB → 4 GiB by the strings campaign: with the interner tables cold, every LB that mints a string holds at least one block (broadcast rule installs mint into every LB), so large grids' floor consumption roughly doubled; later raised 4 GiB to 8 GiB for added working-set headroom. Must be a whole multiple of `static_block_bytes`; the pair is validated by `isValidStaticMemoryConfig` ([`parameters.hpp`](../GL_Quick_VS/GL_Quick/src/parameters.hpp)) and asserted in the `ExpressionAnalyzer` constructor. |
...
| Field | Type | Default | Purpose |
|---|---|---|---|
| `try_contradiction` | bool | `false` | Enables contradiction-attempt LBs for negative conjectures. Incubator-only. |
...
Array of string patterns. Conjectures whose stringified form matches any pattern are rejected by `patternInConjecture` at [`conjecturer.cpp`](../GL_Quick_VS/GL_Quick/src/conjecturer.cpp).
...
6. Run `gl_quick.exe Euclid` (or whatever the tag is) to smoke-test; then `main.py` for the full pipeline.
