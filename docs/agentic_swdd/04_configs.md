...
| `static_pool_bytes` | int64 | | Statification sizing: total bytes of the ONE static-memory reservation made at program start (never freed, never grown). The current 4 GiB cap provides 16384 blocks at `static_block_bytes = 256 KiB` and deliberately exercises the working-set pager on the heavy incubator batches ([D-209](40_decisions.md#d-209)). The historical sequence was 2 GiB → 4 GiB for cold-string floor consumption, then 4 GiB → 8 GiB for added headroom before this user-directed return to 4 GiB. Must be a whole multiple of `static_block_bytes`; the pair is validated by `isValidStaticMemoryConfig` ([`parameters.hpp`](../GL_Quick_VS/GL_Quick/src/parameters.hpp)) and asserted in the `ExpressionAnalyzer` constructor. |
