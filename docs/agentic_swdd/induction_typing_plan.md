...
Canonical Peano failure case (currently in `theorems.txt`):
...
- **Multi-batch theorems.txt.** Theorems from an earlier batch
 already in `theorems.txt` that relied on the old unsound
