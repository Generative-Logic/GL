...
*Seen:* throughout the codebase; definitions in `files/definitions/*.txt`; processed chapters in `files/processed_proof_graph/*.txt`.
...
*Seen:* `files/definitions/AnchorPeano.txt`; referenced by every Peano theorem. Signature computed via `makeAnchorSignature` at [`compiler.hpp`](../GL_Quick_VS/GL_Quick/src/compiler.hpp).
...
Anchor for the FTA milestone. Shape TBD as the ladder is climbed. *Seen:* `files/definitions/AnchorFTA.txt` (scaffolded on current branch).
...
A proved `(>[...](premise)(head))` expression. Survives the prover phase and enters `files/theorems/proved_theorems.txt`, then is further pruned by the compressor. Each theorem is emitted into the processed proof graph as one or more chapters (one for direct proof, three for induction, one each for mirrored/reformulated variants).
...
A theorem produced by swapping the output-var premise with the head, then permuting non-anchor premises. Method label: `mirrored statement`. Reference column: the source theorem in anchor-agnostic form.

*Seen:* `10_mirrored_statement.txt`:

```text
(>[i0,s,+,i1](AnchorPeano[N,i0,s,+,*,i1])(>[v1](in3[i1,v1,i0,+])(in2[v1,i0,s])))	main	mirrored from	(>[i0,s,+,i1](AnchorPeano[N,i0,s,+,*,i1])(>[v1](in2[v1,i0,s])(in3[i1,v1,i0,+])))	main
```
...
The unit of execution in the GL grid. Each LB has local memory (the `Memory` instance), its own hash index, its own queue of pending work, and its own mail. LBs communicate between cycles via `mailIn` / `mailOut`, not during them. In the current single-process execution, LBs are C++ objects dispatched sequentially; the *architecture* is designed for parallel distribution (ASIC roadmap).
...
| `wholeExpressions` | Set of expressions known to this LB. |
| `statementLevelsMap` | Per-expression levels (scope levels of admission). |
...
| `mailIn` / `mailOut` | Inter-LB messaging. |
...
Value type in `HashMemory::encodedMap`. Fields: `value` (normalised conclusion template), `levels`, `originalImplication`, `justification`, `key`, `remainingArgs`, `validityName`. Defined at [`memory.hpp`](../GL_Quick_VS/GL_Quick/src/memory.hpp).

### Mail

The inter-LB message buffer. Fields:

- `statements` — list of `(expr, levels)` pairs to be delivered.
- `implications` — list of implication tuples (chain, head, args, levels, theorem).
- `exprOriginMap` — provenance entries to merge into the recipient's origin map.

Defined at [`memory.hpp`](../GL_Quick_VS/GL_Quick/src/memory.hpp). Each `Memory` has a `mailIn` (inbox) and a `mailOut` (outbox). Main-scope-only by contract (every `mailOut.statements.insert` gated on `validityName == "main"`; absorb hard-codes `"main"`).

### InternalMail (retired 2026-05-07 — merged into Mail)

`InternalMail` was a separate struct on `Memory` (alongside `Mail mailIn`/`mailOut`) used as the per-LB integration-side revival inbox. Its `statements` was a `set<tuple<string, levels, validityName>>` carrying scope per element. As of 2026-05-07 ([D-53](40_decisions.md#d-53) unification, renumbered from main's D-46), the channel uses the same `Mail` struct as the routing channels — `Memory::internalMailIn` is now type `Mail` with `statements` element type `pair<ExpressionWithValidity, levels>`, and `struct InternalMail` has been deleted. The variable name `internalMailIn` and the function name `emitIntegrationRevivalToInternalMailIn` survive. See [`Mail`](#mail), [D-19](40_decisions.md#d-19), [D-53](40_decisions.md#d-53), [I-21](30_invariants.md#i-21).
...
Pre-parsed form of an expression. Fields: `name`, `negation`, `arguments` (with iteration/level), `maxIterationNumber`, `original`, `validityName`. Used as the key in `statementLevelsMap`. Defined at [`memory.hpp`](../GL_Quick_VS/GL_Quick/src/memory.hpp).

### levels

A pair of integers attached to every deposited statement, representing `(newStatementLevel, newEqualityLevel)`. Used to order admission and avoid re-firing. A monotonic counter (see [I-17](30_invariants.md#i-17)).

### statementLevelsMap

Per-validity map from `EncodedExpression` → levels pair. Records when each statement was admitted.

### intKnownStatements

The set of statements known to an LB at the integer-encoded level. Used for fast membership tests without string comparison.

### wholeExpressions

The set of expressions known to an LB at string level. String analogue of `intKnownStatements`.
...
Defined at [`prover.cpp`](../GL_Quick_VS/GL_Quick/src/prover.cpp) (near `revisitRejected2`). Integration-side counterpart. Called after `admissionMapIntegration` insert at `prover.hpp` (with u_-strip on the key) and after `admissionSetIntegration.insert` at `prover.cpp` (key already bare). Emits matching `rejectedMapIntegration` entries to `internalMailIn`. Does **not** call `addExprToMemoryBlock`; does **not** call `cleanAdmissionMap` ([I-22](30_invariants.md#i-22)).

### `applyEquivalenceClassToRejectedMapIntegration`

Defined at [`prover.hpp`](../GL_Quick_VS/GL_Quick/src/prover.hpp) (alongside `applyEquivalenceClass`). Equivalence-class-driven revival path. For each rmi entry whose key args overlap the class, iterates `allMappingsAna[(|indices|, |eqList|)]` (full permutation, same as main helper), dedups per-entry, probes each unique rewritten key against `admissionMapIntegration` (u_-transformed) and `admissionSetIntegration` (bare). On match: emit constituents to `internalMailIn`. On no-match: persist rewritten entry at the new key. Called at three sites mirroring `applyEquivalenceClass`: same-NS, ancestor-NS, fixpoint re-iter. Gated on `!parameters.skip_eq_classes`.
...
The per-LB encoder for validity names. Defined in [`memory.hpp`](../GL_Quick_VS/GL_Quick/src/memory.hpp). Internally maps canonical strings to `int16_t` IDs via `pairMap` verdicts and ancestor registration.

### NameMap::encodePush

The authoritative method for minting a new scope. Takes a `parentId` and a `payload`, registers ancestors, returns a fresh `int16_t`. **Every non-`main` validityName must be minted via `encodePush`** — raw string concatenation bypasses `pairMap` / `stackOfValidity` and produces orphan roots that break `comparable` / `deeperOf`. See [I-2](30_invariants.md#i-2).

### stackOfValidity

The per-LB ordered stack of active scope IDs. Used for scope-depth comparisons (`comparable`, `deeperOf`) and for unwinding on hypothesis discharge.

### pairMap

An internal NameMap table registering parent-child relationships between scope IDs. Consulted to answer "is scope A an ancestor of scope B?".
...
The C++ combinatorial enumerator that produces candidate theorems for a batch. Entry point: `Conjecturer::run` in [`conjecturer.cpp`](../GL_Quick_VS/GL_Quick/src/conjecturer.cpp). Invoked via `gl_quick.exe --conjecture <tag>`. Writes `files/theorems/theorems.txt`.

### reshuffled theorems

After the conjecturer emits candidate theorems, a normalisation pass canonicalises their argument order. The normalised forms are written to `files/theorems/reshuffled_theorems.txt`. Mirrors go to `files/theorems/reshuffled_mirrored_theorems.txt`.
...
| `mirrored from` | `check_mirrored_from` (2318) | This theorem is a mirror of the cited source. |
...
### `files/definitions/*.txt`

User-authored MPL definition files. Input to the compiler. Examples: `NaturalNumbers.txt`, `AnchorPeano.txt`, `fold.txt`, `in.txt`, `limitSet.txt`.
...
### `files/theorems/reshuffled_theorems.txt` / `reshuffled_mirrored_theorems.txt`

Post-normalisation canonical forms + their mirrors.

### `files/theorems/proved_theorems.txt`

Survivors of the prover phase, after compressor pruning. Input to stage 7 (raw proof graph emission).
...
Authoritative theorem registry for verifier. Three columns: `expression`, `method`, `reference`. Method ∈ `direct | induction | mirrored statement | reformulated statement`. Reference is `-1` for `direct`, the induction variable for `induction`, and the source-theorem expression for `mirrored statement` / `reformulated statement`.
