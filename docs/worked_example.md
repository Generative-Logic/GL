...
# Worked example — one theorem, all ten stages `[DRAFT]`
...
## Stage 1 — Definitions & anchor

The anchor `AnchorPeano` is defined in `files/definitions/AnchorPeano.txt` as:
...
`NaturalNumbers` in turn expands (from `files/definitions/NaturalNumbers.txt`) into a long conjunction enumerating every Peano axiom — including the ones we'll need for the proof.
...
## Stage 2 — Conjecturer emits it

The conjecturer enumerates candidates by combining expressions under the anchor. Our theorem appears in `files/theorems/theorems.txt` at line 37 in raw bound-variable-index form:
...
- Outer bound vars: `>[2,3,4,6]` — binds `i0, s, +, i1`. Slots `N` and `*` remain free (not used in the body).
...
Hat tip to [I-11](30_invariants.md#i-11): slots `N` and `*` are not bound because they are not mentioned in the body. Binding them would widen the theorem's quantifier universe unnecessarily.

The conjecturer also produces reshuffled / mirrored variants (canonicalised argument orders + the mirror with the output premise swapped to head). Those go to `files/theorems/reshuffled_theorems.txt` and `files/theorems/reshuffled_mirrored_theorems.txt`. Our theorem's mirror becomes theorem #5 in the global theorem list:
...
— "if `i0 + i1 = v1`, then `s(i0) = v1`". The mirror reference column uses the raw-integer form of the source theorem.

---

## Stage 3 & 4 — Normalisation & CE filter
...
No elimination. Theorem survives into the output `proved_theorems.txt`.

---

## Stage 7 — Raw proof graph
...
## Stage 8 — Process proof graph — the rename
...
- **Mirror / reformulation.** See `10_mirrored_statement.txt` or `103_reformulated_statement.txt`.
