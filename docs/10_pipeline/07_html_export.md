...
# Pipeline · Stage 9 — HTML export `[DRAFT]`

> **Input:** `files/processed_proof_graph/*.txt` (from stage 8).
...
- **Index** (`index.html`) — lists every theorem with a link to its chapter page. Groups theorems by method (direct / induction / mirrored / reformulated / OR) and by anchor.
...
**Truthfulness by construction (post `D-56`).** Chapter rows are emitted at the lifted (closest-to-`main`-with-origin) validity by `visualizer.cpp::buildStack` — see [`10_pipeline/04_prover.md`](04_prover.md#buildstack-chapter-walker-d-51-algorithm--lifting-per-d-pending-buildstack-lifting). Every `(<ns>)` label therefore names the scope where the derivation actually lives. Pre-lifting, a tag could name a deep boundary scope while the origin actually lived at `main` (a falsified label); post-lifting, the label is honest.
...
The generator's `_htmlify_readable` + `visu_helpers.format_implication` / `format_mirroring` / `format_reformulation` + the anchor-specific rename functions build these by:
...
- **OPEN-16 — RESOLVED.** Induction renders as **one HTML page per theorem**, not three. The generator aggregates the three source chapter files (`<N>_induction_typing.txt`, `<N+1>_check_zero.txt`, `<N+2>_check_induction_condition.txt`) into a single `chapter<M>.html` page with sub-sections. See [`generate_full_proof_graph.py–1489`](../../generate_full_proof_graph.py) (file aggregation) and [`:2199–2201`](../../generate_full_proof_graph.py) (render of both check_zero and check_induction_condition inside the same page with prefixes `c{idx}s0`, `c{idx}s1`). So: HTML page count = theorem count; induction-chapter-file count = 3 × induction theorems + 1 × direct theorems + 1 × mirrored theorems +...
