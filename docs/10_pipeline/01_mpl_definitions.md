...
# Pipeline · Stage 1 — MPL definitions & GL binary `[DRAFT]`

> **Input:** `files/definitions/*.txt` (user-authored MPL) + `files/config/Config<Tag>.json`.
...
- **No spaces.** Ever. Arguments are separated by bare commas. `(in2[a, b, c])` is **invalid** — it must be `(in2[a,b,c])`. Spaces in definition source files (as seen in [NaturalNumbers.txt](../../files/definitions/NaturalNumbers.txt)) are purely formatting; the compiler strips them before parsing.
...
You see this shape in every set-definition file (`fXY.txt`, `interval.txt`, `limitSet.txt`, …).
...
### `NaturalNumbers.txt` — the Peano axioms
...
2. **C++ startup.** The `ExpressionAnalyzer` constructor calls [`loadGlBinary`](../../GL_Quick_VS/GL_Quick/src/visualizer.cpp) on the per-batch file. Every entry is inserted into `compiledExpressions`; every entry whose category is in `{implication, existence, or, and}` also gets a row in `repetitionExclusionMap` (using the JSON's `elements` field directly as `splitNK` — that is exactly what `excludeRepetitions` stored at allocation time). The four shared counter members (`implCounter`, `existenceCounter`, `andCounter`, `orCounter`) are seeded from `max(N) + 1` so newly-allocated names start above the highest already in use.
...
4. **Post-batch (Python).** [`run_modes.py::_merge_into_shared`](../../run_modes.py) reads the per-batch file, filters to `{implication, existence, or, and}` entries (anchor and atomic entries are batch-local), and adds any name not already in shared. The shared file grows monotonically over the run. **Incubator-prefixed tags (`IncubatorPeano` / `IncubatorGauss` /...) are skipped** — their spontaneous allocations are batch-local and must not propagate into shared, otherwise the next main batch's counters and operator-name set would shift and theorem texts would change. Pre-2026-05-08 the merge was an accidental no-op for incubator tags because the visualizer wrote to a stale folder; the explicit skip preserves that behaviour now that the writer path is unified.

This guarantees [I-23](../30_invariants.md#i-23) for cross-batch *main* allocations: the same logical operator carries the same compact name in every main batch of a run, so `compiled_proved_theorems.txt`, `raw_proof_graph/global_theorem_list.txt`, and the proof-graph chapters all agree. Incubator allocations live in their own per-batch JSON files in the same directory and are scoped per-batch; the verifier filters by chapter context so incubator chapters never see main-batch spontaneous shapes (and vice versa).
...
| 4. Definition-file loading | Inside `modifyCoreExpressionMap` — reads from path under `files/definitions/` if `full_mpl` ends in `.txt` | |
...
- **Hand-written anchor definition files.** `AnchorPeano.txt`, `AnchorGauss.txt`, `AnchorIncubator.txt`, `AnchorFTA.txt` are each a small `and`-tree. The expected slot count (6 / 8 / 14 / TBD) is implicit in the file and hardcoded elsewhere (conjecturer, verifier, rename logic). Changing the slot count requires coordinated edits across files with no cross-check — the compiler does not reject a mismatch until much later in the pipeline.
- **`full_mpl` string vs file-path duality.** `modifyCoreExpressionMap` decides whether `full_mpl` is inline MPL or a filename based on whether it ends in `.txt`. A user error (e.g. copying a filename into the inline field without `.txt` suffix, or an inline string that ends with `.txt`) can silently corrupt the compilation.
