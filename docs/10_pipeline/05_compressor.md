...
# Pipeline · Stage 6 — Compressor `[DRAFT]`

> **Input:** the batch's proved-theorem set (in-memory after stage 5).
> **Output:** a minimal subset of proved theorems that preserves derivability of every individual theorem; persisted as the pruned `files/theorems/proved_theorems.txt` and (when externals are being rebuilt) the pruned `files/theorems/compressed_external_theorems.txt`.
