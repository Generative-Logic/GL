...
# Pipeline · Stage 2 — Conjecturer `[DRAFT]`

> **Input:** compiled `CoreExpressionConfig` map (from stage 1), `Config<Tag>.json`.
> **Output:** `files/theorems/theorems.txt` (and auxiliary output files depending on flags).
...
7. Writes the survivors to `files/theorems/theorems.txt`.

The prover then picks up `theorems.txt` (via the CE filter, stage 3→4) and tries to prove or refute each entry.

---

## The raw output — what `theorems.txt` looks like

A real line from the current branch's `theorems.txt`:
...
- Outer quantifier: `>[1,2,3,6]` — binds slots 1, 2, 3, 6. Slots 4 and 5 are not bound here because this conjecture does not mention `+` or `*`.
...
**Numbered-variable form.** In raw `theorems.txt`, bound variables are bare integers `1`, `2`, … — no `v1` yet. The renaming from integers to `v1`, `v2`, … happens in `process_proof_graphs.py` after the prover runs. See [I-10](../30_invariants.md#i-10).

**Reshuffled form.** After conjecturer emission, `reshuffled_theorems.txt` holds the argument-normalised canonical forms. Representative line from the current run:

```text
(>[1,2,3,4](AnchorPeano[1,2,3,6,7,4])(>[]!(=[2,4])!(>[5](in[5,1])!(in2[5,2,3]))))
```

Note the anchor slot pinning has changed — `AnchorPeano[1,2,3,6,7,4]` means slots 1..3 and 6 are bound, slots 4 and 5 are free placeholders (because the conjecture doesn't refer to them), and index 4 has taken slot 6's position as a bound slot. This is the reshuffling — a canonical alignment.
...
9. **Mirror generation** — `createReshuffledMirrored(expr, anchorFirst)` at [`conjecturer.cpp`](../../GL_Quick_VS/GL_Quick/src/conjecturer.cpp).
...
11. **Write output** — `theorems.txt`, `reshuffled_theorems.txt`, `reshuffled_mirrored_theorems.txt`.
...
Per-tag config rationale captured body: caps were chosen to match per-type maxes observed in `proved_theorems.txt`, so no baseline-proved shape is dropped. Observed effects on the immediate run after introduction:
...
`reshuffle(expr, deep)` at `conjecturer.cpp` canonicalises a survived conjecture into its `theorems.txt` / `reshuffled_theorems.txt` form. On the `rt_conjecturer` lineage the pipeline is:
...
3. **Contiguous-arg renumber post-connect** (commit, inside `connectExpressionsInt`). After a `connectExpressionsInt` merge via `subMap`, surviving arg-IDs may have holes in the shifted range. A renumber pass collects arg-IDs from atom args in block order, builds a `renumber` map (`oldID → contiguousID` starting at 1), then applies it across the expression. Downstream consumers (`theorems.txt`, anchor-connect inputs) see a contiguous `1..N` arg-ID space rather than a sparse one.
...
- [`10_pipeline/03_ce_filter.md`](03_ce_filter.md) — downstream consumer of `theorems.txt`.
...
- [I-11](../30_invariants.md#i-11) — anchor vars must not appear in `>[...]`.
