...
| `ConfigGauss.json` | Main-pipeline Gauss batch. **No longer carries `EnumerationSet2`** — that migrated to `ConfigIncubatorGauss1.json` on 2026-04-29 ([D-25](40_decisions.md#d-25)). |
| `ConfigIncubatorPeano.json` | Incubator batch over Peano anchor. |
| `ConfigIncubatorGauss.json` | Incubator batch over Gauss anchor. |
| `ConfigIncubatorGauss1.json` | Second incubator batch in the Gauss group (alphanumerically sequenced after `ConfigIncubatorGauss.json`). Sets `allow_disintegration=true`, `allow_multiplication=false`, `incubator_mode=true` — the combination needed for the FTA-ladder rung-1 §4.1 attempt with `EnumerationSet2`. |
...
| `full_mpl` | string | Inline MPL body, OR a filename (must end in `.txt`) to load from `files/definitions/`. |
...
Decoded: 6-ary. `full_mpl = "fold.txt"` means load the MPL body from [`files/definitions/fold.txt`](../files/definitions/fold.txt) — the complex folds-and-accumulates definition. Short form is the readable signature.
...
|---|---|---|
| `theorems_folder` | `"files/theorems_incubator"` | Override `files/theorems/` for this batch. Routes `theorems.txt`, `proved_theorems.txt`, `compressed_external_theorems.txt` to a separate tree. |
| `background_theorems_folder` | `"files/theorems"` | If set, `proved_set` is union-loaded from both `theorems_folder/proved_theorems.txt` and `background_theorems_folder/proved_theorems.txt`. Lets the incubator carry main-pipeline theorems as external axioms. |
...
| Expression | Peano | Gauss | IncubatorPeano | IncubatorGauss | IncubatorGauss1 | Visu |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| `AnchorPeano` | ✓ | — | ✓ | ✓ | ✓ | ✓ |
| `AnchorGauss` | — | ✓ | ✓ | ✓ | ✓ | ✓ |
| `AnchorIncubator` | — | — | ✓ | ✓ | ✓ | ✓ |
