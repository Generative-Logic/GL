...
**Primary dump.** `.debug/hashburst_trace.txt`. Each `HASHBURST`
...
section prove live memory. The `exprOriginMap` dump was disabled
(commit `429847c`) because its speculative provenance edges were
...
assigned by the C++ compiler. As of [D-22](../../docs/40_decisions.md#d-22)
...
**Atomic operators** (also documented in `CLAUDE.md → ConfigVisu.json
reference`):
- `in[x, S]` — `x ∈ S`.
...
**How confirmed** (commit `9162c4f`). 72 `symmetry of inequality`
...
`6068cb5`). The auxy comes from NaturalNumbers.txt — of the two
...
`b81725c`).** The function at `prover.cpp:1532` previously pushed the
...
`.debug/se2_step5_request_trap.txt`. Full-run result: **file not
...
`feedback_trap_hash_requests.md` in memory.
...
1. **`b6a62e9` — purity gate relaxed inside OR-branch scopes only.**
...
2. **`98664d3` — `preEvaluateFromEncoded` dedups secondary vars by
...
registry on `sandbox/reverse_direction2`), and implication21
...
### Step 8 — COMPLETE (commit `14d6175f`, D-30)
...
Diagnostic trail (debug branch `debug/local_premise_check`, then
trap on `sandbox/reverse_direction2`):
...
[D-33](../../docs/40_decisions.md#d-33)'s descendant-class direction
rewrites main-scope `(preorder[…])` facts using the branch-scope
equivalence class (`{2, repl_lev_1_0}` or `{6, repl_lev_1_0}`), and
[I-25](../../docs/30_invariants.md#i-25)'s single-channel routing
threads those cross-scope deposits through the kernel's discharge
loop. See also [D-31](../../docs/40_decisions.md#d-31) (initial OR-disint
unblock) and [D-32](../../docs/40_decisions.md#d-32) (sharpened gate).
...
two ordis branches mint, [D-33](../../docs/40_decisions.md#d-33)'s
descendant-class direction + [I-25](../../docs/30_invariants.md#i-25)'s
...
[D-34](../../docs/40_decisions.md#d-34) implements the convergence
