...
namespace ce {
    ...
    std::vector<std::vector<int>> generateBinarySequencesAsLists(int n) {
        ...
        if (n == 0) {
            out.emplace_back(); // single empty sequence for length 0
            return out;
        }
        ...
        for (std::size_t mask = 0; mask < total; ++mask) {
            ...
            // Reverse the bit order within each sequence: MSB -> LSB
            for (int i = n - 1; i >= 0; --i) {
    ...
    std::map<int, std::vector<std::vector<int>>> generateAllPermutations(int n) {
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            // current is already sorted; std::next_permutation enumerates lexicographically
            do {
